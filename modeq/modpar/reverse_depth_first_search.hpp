...
// Nonrecursive implementation of  reversed depth_first_visit_impl.
// Modified from the depth_first_visit_impl by P. Aronsson April. 2004.
//
...
#ifndef REVERSED_BOOST_GRAPH_RECURSIVE_DFS_HPP
#define REVERSED_BOOST_GRAPH_RECURSIVE_DFS_HPP
...
namespace boost {
  ...
  class ReverseDFSVisitorConcept {
  public:
    void constraints() {
...
// comparison.
    ...
    void reverse_depth_first_visit_impl
       ...
       ColorMap color, TerminatorFunc func = TerminatorFunc())
    {
      ...
      function_requires<ReverseDFSVisitorConcept<DFSVisitor, IncidenceGraph> >();
      typedef typename graph_traits<IncidenceGraph>::vertex_descriptor Vertex;
      function_requires< ReadWritePropertyMapConcept<ColorMap, Vertex> >();
      typedef typename property_traits<ColorMap>::value_type ColorValue;
      function_requires< ColorValueConcept<ColorValue> >();
      typedef color_traits<ColorValue> Color;
      typedef typename graph_traits<IncidenceGraph>::in_edge_iterator Iter;
       ...
       ColorMap color, TerminatorFunc func)
    {
      ...
      if (!fn(u, g))
        for (tie(ei, ei_end) = in_edges(u, g); ei != ei_end; ++ei) {
          Vertex v = source(*ei, g);           vis.examine_edge(*ei, g);
          ColorValue v_color = get(color, v);
          if (v_color == Color::white()) {     vis.tree_edge(*ei, g);
          reverse_depth_first_visit_impl(g, v, vis, color, func);
  ...
  void
  reverse_depth_first_search(const VertexListGraph& g, DFSVisitor vis, ColorMap color,
                     typename graph_traits<VertexListGraph>::vertex_descriptor start_vertex)
  {
    typedef typename graph_traits<VertexListGraph>::vertex_descriptor Vertex;
    function_requires<ReverseDFSVisitorConcept<DFSVisitor, VertexListGraph> >();
    typedef typename property_traits<ColorMap>::value_type ColorValue;
    typedef color_traits<ColorValue> Color;
    ...
    if (start_vertex != implicit_cast<Vertex>(*vertices(g).first)){ vis.start_vertex(start_vertex, g);
      detail::reverse_depth_first_visit_impl(g, start_vertex, vis, color,
                                     detail::reverse_nontruth2());
    }

    for (tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui) {
      ColorValue u_color = get(color, *ui);
      if (u_color == Color::white()) {       vis.start_vertex(*ui, g);
        detail::reverse_depth_first_visit_impl(g, *ui, vis, color, detail::reverse_nontruth2());
      }
    }
  }
  ...
  reverse_depth_first_search(const VertexListGraph& g, DFSVisitor vis, ColorMap color)
  {
    reverse_depth_first_search(g, vis, color, *vertices(g).first);
  }

  namespace detail {
    template <class ColorMap>
    struct reverse_dfs_dispatch {
      ...
      apply(const VertexListGraph& g, DFSVisitor vis, Vertex start_vertex,
            ...
            ColorMap color)
      {
        reverse_depth_first_search(g, vis, color, start_vertex);
      }
    };

    template <>
    struct reverse_dfs_dispatch<detail::error_property_not_found> {
      template <class VertexListGraph, class Vertex, class DFSVisitor,
                class P, class T, class R>
      static void
      apply(const VertexListGraph& g, DFSVisitor vis, Vertex start_vertex,
  ...
  class reverse_dfs_visitor {
  public:
    reverse_dfs_visitor() { }
    reverse_dfs_visitor(Visitors vis) : m_vis(vis) { }
    ...
    BOOST_GRAPH_EVENT_STUB(on_initialize_vertex,reverse_dfs)
    BOOST_GRAPH_EVENT_STUB(on_start_vertex,reverse_dfs)
    BOOST_GRAPH_EVENT_STUB(on_discover_vertex,reverse_dfs)
    BOOST_GRAPH_EVENT_STUB(on_examine_edge,reverse_dfs)
    BOOST_GRAPH_EVENT_STUB(on_tree_edge,reverse_dfs)
    BOOST_GRAPH_EVENT_STUB(on_back_edge,reverse_dfs)
    BOOST_GRAPH_EVENT_STUB(on_forward_or_cross_edge,reverse_dfs)
    BOOST_GRAPH_EVENT_STUB(on_finish_vertex,reverse_dfs)

  protected:
    Visitors m_vis;
  };
  template <class Visitors>
  reverse_dfs_visitor<Visitors>
  make_reverse_dfs_visitor(Visitors vis) {
    return reverse_dfs_visitor<Visitors>(vis);
  }
  typedef reverse_dfs_visitor<> default_reverse_dfs_visitor;


  // Named Parameter Variant
  template <class VertexListGraph, class P, class T, class R>
  void
  reverse_depth_first_search(const VertexListGraph& g,
                     const bgl_named_params<P, T, R>& params)
  {
    typedef typename property_value< bgl_named_params<P, T, R>,
      vertex_color_t>::type C;
    detail::reverse_dfs_dispatch<C>::apply
      (g,
       choose_param(get_param(params, graph_visitor),
                    make_reverse_dfs_visitor(null_visitor())),
  ...
  template <class IncidenceGraph, class DFSVisitor, class ColorMap>
  void reverse_depth_first_visit
    (const IncidenceGraph& g,
     typename graph_traits<IncidenceGraph>::vertex_descriptor u,
     DFSVisitor vis, ColorMap color)
  {
    vis.start_vertex(u, g);
    detail::reverse_depth_first_visit_impl(g, u, vis, color, detail::reverse_nontruth2());
  }
  ...
  {
    vis.start_vertex(u, g);
    detail::reverse_depth_first_visit_impl(g, u, vis, color, func);
  }
