...
has been available for many years, but are not often used by other
than researchers.
...
hybrid DAEs\cite{dae} is not very familiar to the engineers who do the
modeling.

\subsection{The class concept}

In Modelica, the basic means of structuring models is the
\firstref{class} concept.  Everthing in Modelica is declared as a class
...
The Programming Environment Laboratory, PELAB, (\fixme{say what?}
professor Peter Fritzson) is concerned with research in software
...
program transformation systems, etc.  The view of programming
...
\item Class names can only denote classes, which means that it is not
  ...
  A type is a property of a peice of data.  Instantiating a class
...
\item Functions are implemented in too simplistic way.  A simple
  type-checking of arguments and a single return value is perform, but
  it only allows complete types, meaning that the sizes of array
  ...
  The function semantics are still uncertain.  There has been though
  about a proposal where function calls are defined in terms of
  ``implicit instantiation'', where each function definition would be
  instantiated once for each function call in the model.  This would
  make it possible to determine the full type of the arguments for
  each function call.
  
  However, this is currently hard to implement in this semantic
  specification because of RML limitations.  The RML module which does
  the instantiation is called \code{Inst} (see section
  \ref{src:Inst}).  It needs to do static analysize of expressions,
  and for that purpose it uses the relations in the module
  \code{Static}.  But if the \code{Static} module encounters a
  function call it would need to do an implicit instantiation, which
  would require the use of the \code{Inst} module.  Unfortunately,
  circular dependencies between modules are not allowed in RML.

  This can be worked around in several ways, but all of them would
  make the specification less structured or harder to understand.
...
\item Unused modifications are simply discarded.  They should produce
  an error.  This is also true for out-of-bounds array modifications.
...
\item When instantiation an \code{extends} element, the extended class
...
The word \term{instantiate} is used in many language descriptions,
with slightly different meaning.  In the specification it is used in
two related, but slightly different, meanings.
...
\item A special kind of equation \codebox{ident ::= expression} is
  ...
  problem but just create a new, similar problem.
...
\item Due to the extensions a flat model representation is not valid
  ...
  There are plans for a modified version of the translator which
  produces valid Modelica.  This will be less readable than the
  current Flat Modelica, but will allow us to feed the output of the
  translator to another Modelica system.
...
\item Parsing of the flat Modelica has not been explored. It should
  ...
  An extension of the translator is currently being developed which
  will allow it to produce valid Modelica.
...
clearer.  Each file contains one module.  The names of the files are
the names of the module it contains, with all letters in lower case,
and with the suffex \code{.rml} appended.
