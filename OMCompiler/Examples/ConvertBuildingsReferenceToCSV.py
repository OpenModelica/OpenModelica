...
def _write_csv(file_name, d):
    ...
    for key in list(d.keys()):
        if len(d[key]) != n:
            if key == "time":
                d[key] = np.linspace(
                    np.float64(d[key][0]), np.float64(d[key][-1]), n
                ).tolist()
            else:
...
def main():
    ...
    parser.add_option(
        "--input-dir",
        help="Directory containing .txt reference files in the Buildings package format",
        type="string",
        dest="input_dir",
    ...
    parser.add_option(
        "--output-dir",
        help="Directory to generate csv-files in",
        type="string",
        dest="output_dir",
        default=os.path.abspath("."),
    )
