...
function genRecordDecls
  ...
  input list<MidCode.Record> extraRecordDecls;
algorithm
  List.map_0(extraRecordDecls, genRecordDecl);
end genRecordDecls;

function genRecordDecl
  "Generates a record declaration.
  Follows the same naming scheme as the existing codegenerator for C in the backend."
  input MidCode.Record rDecl;
algorithm
  () := match rDecl
    case MidCode.RECORD_DECLARATION(__)
    algorithm
      genRecordDeclDef(rDecl.definitionPath
                       ,rDecl.encodedPath
                       ,rDecl.fieldNames);
      then ();
    else
    algorithm
      Error.addInternalError("Unsupported MidCode.Record type at", sourceInfo());
      then fail();
    end match;
end genRecordDecl;
...
algorithm
  /*First generate field e.g the char** (Similar to CodegenCFunctions) */
  ...
  () := match numFields
    ...
    then();
     ...
     algorithm
      ...
      EXT_LLVM.genGlobalStructConstant("struct.record_description"
      /*Do not change this name (If so change generateIntial... in llvm_gen.hpp).*/
                                       ,encName + "__desc");
     then();
  end match;
...
algorithm
  if Flags.isSet(Flags.JIT_DUMP_IR) then
    print("Dumping generated LLVM-IR:\n");
    EXT_LLVM.dumpIR();
  end if;
  EXT_LLVM.jitCompile();
...
algorithm
  ...
  for inp in inputs loop
    ...
    if typeDescriptor == MODELICA_TUPLE then
      EXT_LLVM.genFunctionArg(MODELICA_METATYPE, genVarName(inp));
      return;
    end if;
    EXT_LLVM.genFunctionArg(typeDescriptor, genVarName(inp));
  end for;
end genFunctionArgs;
...
algorithm
  () := match listLength(outputs)
    ...
    else /* Function returning several values */
      algorithm
...
"
  Generates a MetaModelica literal.
  For instance the value of a MetaModelica List such as:
  {1,2 ,3}
"
...
algorithm
  tyDescInt := match ty
    ...
    case DAE.T_TUPLE()  then MODELICA_METATYPE;
...
function getTupleTypes
  "TODO: move to DAE Util"
  input DAE.Type ty;
  output list<DAE.Type> types;
algorithm
  types := match ty
    case DAE.T_TUPLE(__) then ty.types;
    case DAE.T_METATUPLE(__) then ty.types;
    else fail();
  end match;
end getTupleTypes;
