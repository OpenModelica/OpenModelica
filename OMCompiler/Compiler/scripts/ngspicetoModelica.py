    ...
    for eachline in compInfo:
        ...
        if eachline[0] == "r":
            ...
            if len(words) > 3:
                ...
                vt = str(vt_temp)
                stat = (
                    "Analog.Semiconductors.Diode "
                    + words[0]
                    + "(Ids = "
                    + modelInfo[words[3]]["is"]
                    + ", Vt = "
                    + vt
                    + ", R = 1e12"
                    + ");"
            ...
            stat = (
                ...
                + splitIntoVal(ld)
                + ", U0 = "
                + str(float(splitIntoVal(uo)) * 0.0001)
                + ", LAMBDA = "
                + lam
                + ", TOX = "
                + splitIntoVal(tox)
            ...
            if typ[0] == "pulse":
                ...
                #         elif IfMOS == '1':
                stat = (
                    "Analog.Sources.TrapezoidVoltage "
                    + words[0]
                    + "(rising = "
                    + words[6]
                    + ", V = "
            ...
            if len(temp_line) > point + 1:
                ...
                rem_new = ",".join(rem)
                stat = (
                    subname + " " + subname + "_instance" + index + "(" + rem_new + ");"
                )
            else:
        ...
        if len(nonprotectedNode) > 0:
            for i in range(0, len(nonprotectedNode), 1):
                if i != len(nonprotectedNode) - 1:
                    pinProtectedInit = (
                        pinProtectedInit + nodeDic[nonprotectedNode[i]] + ","
                    )
                else:
...
modelicaCompInit, numNodesSub = compInit(
...
def procesSubckt(subcktName, dir_name):
    ...
    if len(subcktName) > 0:
        for eachsub in subcktName:
            ...
            subModel, subModelInfo, subsubName, subParamInfo, subtransInfo = addModel(
                subOptionInfo, dir_name
            )
            IfMOSsub = "0"
            for eachline in subSchemInfo:
                #      words = eachline.split()
            ...
            nodeSub, nodeDicSub, pinInitSub, pinProtectedInitSub = nodeSeparate(
                subSchemInfo, "1", eachsub, subsubName
            )
            modelicaSubCompInit, numNodesSubsub = compInit(
                subSchemInfo, nodeSub, subModelInfo, subsubName, dir_name, subtransInfo
            )
            modelicaSubParamNew = getSubParamLine(
                eachsub, numNodesSub, modelicaSubParam, dir_name
            )
...
if len(subcktName) > 0:
    (
        ...
        pinInitSub,
        connSubInfo,
    ) = procesSubckt(subcktName, dir_name)
