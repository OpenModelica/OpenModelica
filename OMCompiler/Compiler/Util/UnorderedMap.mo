...
public
  ...
  function tryUpdate
    "Updates the value associated with a key if the key exists in the map,
     otherwise does nothing. Returns whether the value was updated or not,
     i.e. if the key exists in the map or not."
    input K key;
    input V value;
    input UnorderedMap<K, V> map;
  ...
  function addMerge
    "Adds a key and associated value to the map. If the key already exists the
     merge function is called with the new and the old value and its result is
     stored. Unlike addUpdate this takes a plain function rather than a closure,
     which matters on paths that add millions of entries."
    input K key;
    input V value;
    input MergeFn fn;
    input UnorderedMap<K, V> map;

    partial function MergeFn
      input V newValue;
      input V oldValue;
      output V value;
    end MergeFn;
  ...
  algorithm
    ...
    if index > 0 then
      Vector.updateNoBounds(map.values, index, fn(value, Vector.getNoBounds(map.values, index)));
    else
      addEntry(key, value, hash, map);
    end if;
  end addMerge;

  function tryAddUpdate
    "Adds a key and associated value to the map, where the value is generated by
     calling the given function. If the key already exists the function is given
