...
algorithm
  outTree := matchcontinue(inTree, inKey)
    local
      Key key, rkey;
      Val val;
      Node<Key,Val> node;
    ...
    case (TREE(node, cf, kf, vf, uf, n), key)
      algorithm
        node := addNode(inTree, node, key, inVal); // send the tree down to the nodes for compare function and update check
      then
...
algorithm
  outVal := match(inNode, inKeyComp)
    ...
    // found match.
    case (NODE(item = ITEM(val = val)), 0)
      then val;

    // search to the right.
    case (NODE(right = r), 1)
      then getNode(inTree, r, inKey);

    // search to the left.
    case (NODE(left = l), -1)
      then getNode(inTree, l, inKey);
...
algorithm
  (outTree, outItem) := matchcontinue(inTree, inKey)
    local
      Key key, rkey;
      Val val;
      Node<Key,Val> node;
    ...
    case (TREE(node, cf, kf, vf, uf, n), key)
      algorithm
        (node, item) := addNodeUnique(inTree, node, key, inVal); // send the tree down to the nodes for compare function and update check
      then
...
"Helper function to addNode."
  ...
  output Item<Key,Val> outItem;
algorithm
  (outNode, outItem) := match(inNode, inKeyComp)
    local
      Key key;
      Val val;
      Node<Key,Val> l, r, n;
    ...
    // insert into right subtree.
    case (NODE(item = i, height = h, left = l, right = r), 1)
      algorithm
        n := emptyNodeIfNoNode(r);
        (n, it) := addNodeUnique(inTree, n, inKey, inVal);
      then
        (NODE(i, h, l, n), it);
