...
constant ConfigFlag INTERACTIVE_PORT = CONFIG_FLAG(141, "interactivePort",
  NONE(), EXTERNAL(), INT_FLAG(0), NONE(),
  Gettext.gettext("Sets the port used by the interactive server."));
constant ConfigFlag ALLOW_NON_STANDARD_MODELICA = CONFIG_FLAG(142, "allowNonStandardModelica",
  NONE(), EXTERNAL(), STRING_LIST_FLAG({
    }),
  SOME(STRING_DESC_OPTION({
...
constant ConfigFlag EXPORT_CLOCKS_IN_MODELDESCRIPTION = CONFIG_FLAG(143, "exportClocksInModelDescription",
  NONE(), EXTERNAL(), BOOL_FLAG(false), NONE(),
  Gettext.gettext("exports clocks in modeldescription.xml for fmus, The default is false."));
constant ConfigFlag LINK_TYPE = CONFIG_FLAG(144, "linkType",
...
constant ConfigFlag TEARING_ALWAYS_DERIVATIVES = CONFIG_FLAG(145, "tearingAlwaysDer",
  NONE(), EXTERNAL(), BOOL_FLAG(false), NONE(),
  Gettext.gettext("Always choose state derivatives as iteration variables in strong components."));
constant ConfigFlag DUMP_FLAT_MODEL = CONFIG_FLAG(146, "dumpFlatModel",
...
constant ConfigFlag SIMULATION = CONFIG_FLAG(147, "simulation",
  SOME("u"), EXTERNAL(), BOOL_FLAG(false), NONE(),
  Gettext.gettext("Simulates the last model in the given Modelica file."));
constant ConfigFlag OBFUSCATE = CONFIG_FLAG(148, "obfuscate",
  NONE(), EXTERNAL(), STRING_FLAG("none"),
  SOME(STRING_DESC_OPTION({
...
constant ConfigFlag FMU_RUNTIME_DEPENDS = CONFIG_FLAG(149, "fmuRuntimeDepends",
  NONE(), EXTERNAL(), STRING_FLAG("default"),
  SOME(STRING_DESC_OPTION({
  ...
  Gettext.gettext("Defines if runtime library dependencies are included in the FMU."));
constant ConfigFlag FRONTEND_INLINE = CONFIG_FLAG(150, "frontendInline",
  NONE(), EXTERNAL(), BOOL_FLAG(false), NONE(),
  Gettext.gettext("Enables inlining of functions in the frontend."));
constant ConfigFlag EXPOSE_LOCAL_IOS = CONFIG_FLAG(151, "exposeLocalIOs",
  NONE(), EXTERNAL(), INT_FLAG(0), NONE(),
...
constant ConfigFlag BASE_MODELICA_FORMAT = CONFIG_FLAG(152, "baseModelicaFormat",
...
constant ConfigFlag BASE_MODELICA_OPTIONS = CONFIG_FLAG(153, "baseModelicaOptions",
...
constant ConfigFlag DEBUG_FOLLOW_EQUATIONS = CONFIG_FLAG(154, "debugFollowEquations",
  NONE(), EXTERNAL(), STRING_LIST_FLAG({}), NONE(),
...
constant ConfigFlag MAX_SIZE_LINEARIZATION = CONFIG_FLAG(155, "maxSizeLinearization",
  NONE(), EXTERNAL(), INT_FLAG(1000), NONE(),
  Gettext.gettext("Sets the maximum system size for which linearization code is generated."));
constant ConfigFlag RESIZABLE_ARRAYS = CONFIG_FLAG(156, "resizableArrays",
  NONE(), EXTERNAL(), BOOL_FLAG(false), NONE(),
  Gettext.gettext("Assumes all arrays are resizable. Only works with the new backend --newBackend."));
constant ConfigFlag EVALUATE_STRUCTURAL_PARAMETERS = CONFIG_FLAG(157, "evaluateStructuralParameters",
...
constant ConfigFlag LOAD_MISSING_LIBRARIES = CONFIG_FLAG(158, "loadMissingLibraries",
  NONE(), EXTERNAL(), BOOL_FLAG(true), NONE(),
  Gettext.gettext("Automatically try to load a matching library if a name can't be found during name lookup."));
constant ConfigFlag CAUSALIZE_DAE_MODE = CONFIG_FLAG(159, "causalizeDaeMode",
  NONE(), EXTERNAL(), BOOL_FLAG(true), NONE(),
...
constant ConfigFlag SIM_CODE_SCALARIZE = CONFIG_FLAG(160, "simCodeScalarize",
  NONE(), EXTERNAL(), BOOL_FLAG(true), NONE(),
