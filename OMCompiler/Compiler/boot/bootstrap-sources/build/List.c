...
if (!listEmpty(tmp4_1)) goto tmp3_end;
tmpMeta6 = mmc_mk_cons(_inListNameStr, mmc_mk_cons(_inBeginStr, mmc_mk_cons(_inEndStr, MMC_REFSTRUCTLIT(mmc_nil))));
...
tmpMeta7 = mmc_mk_cons(_inListNameStr, mmc_mk_cons(_inBeginStr, mmc_mk_cons(_str, mmc_mk_cons(_inEndStr, MMC_REFSTRUCTLIT(mmc_nil)))));
...
_n = ((modelica_integer) 1) + ldiv(((modelica_integer) -1) + _length,tmp3).quot;
tmp4 = _n;
if (tmp4 == 0) {MMC_THROW_INTERNAL();}
_outPartitions = omc_List_partition(threadData, _lst, ((modelica_integer) 1) + ldiv(((modelica_integer) -1) + _length,tmp4).quot);
_return: OMC_LABEL_UNUSED
...
tmp5 = ((modelica_integer) 1); tmp6 = 1; tmp7 = ldiv(_length,tmp8).quot;
...
tmp3 = _inParts;
if (tmp3 == 0) {MMC_THROW_INTERNAL();}
_outParts = omc_List_partition(threadData, _inList, ldiv(_length,tmp3).quot);
}
_return: OMC_LABEL_UNUSED
return _outParts;
}
...
_middle = ldiv(listLength(_inList),tmp11).quot;
