  ...
  case (stmt::tail)
    algorithm
      ...
      case DAE.STMT_ASSIGN(ty, exp1 as DAE.CREF(__), exp, _)
        algorithm
         cref := ComponentReferenceBasics.crefLastCref(exp1.componentRef);
         () := match cref
      ...
      algorithm
        ...
        varIndex := match exp1.sub
          local
            DAE.Exp indexexp;
          case {DAE.INDEX(exp = indexexp)} then rValueToVar(ExpToMid(indexexp, state), state);
          case {DAE.SLICE(exp = indexexp)} then rValueToVar(ExpToMid(indexexp, state), state);
          case {DAE.WHOLE_NONEXP(exp = indexexp)} then rValueToVar(ExpToMid(indexexp, state), state);
        end match;
...
algorithm
  ...
  then rvalue;
  case DAE.ASUB(exp1, subscripts) //Array subscripts are not supported.
  algorithm
    varExp := rValueToVar(ExpToMid(exp1, state), state);
    varExp2 := match subscripts
      local
        DAE.Exp indexexp;
      case {DAE.INDEX(exp = indexexp)} then rValueToVar(ExpToMid(indexexp, state), state);
      case {DAE.SLICE(exp = indexexp)} then rValueToVar(ExpToMid(indexexp, state), state);
