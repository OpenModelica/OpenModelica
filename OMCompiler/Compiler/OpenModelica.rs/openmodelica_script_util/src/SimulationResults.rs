...
//
// The readers and the tube comparison live in `openmodelica_result_files`
// (shared with the OMPlot web module); this file adds the scripting-error
// reporting and the per-process reader cache.
...
/// the readDataset branch emits when a variable cannot be read (`file_msg`
/// already in display form).
fn get_data(reader: &mut ResultReader, varname: &str, file_msg: &ArcStr) -> Result<Option<Vec<f64>>> {
    if let Some(v) = reader.trajectory(varname) {
        return Ok(Some(v));
    }
    match reader {
        ResultReader::Plt(_) => err(&ERROR_PLT_VAR_NOT_FOUND, [ArcStr::from(varname)])?,
        _ => err(&ERROR_COULD_NOT_READ_VAR, [ArcStr::from(varname), file_msg.clone()])?,
    }
    Ok(None)
}

fn read_vars_filter_aliases(reader: &ResultReader) -> Vec<ArcStr> {
    reader.vars_filter_aliases().into_iter().map(ArcStr::from).collect()
}
...
pub fn cmpSimulationResults(mut runningTestsuite: bool, mut filename: ArcStr, mut reffilename: ArcStr, mut logfilename: ArcStr, mut refTol: metamodelica::Real, mut absTol: metamodelica::Real, mut vars: Arc<metamodelica::List<ArcStr>>) -> Result<Arc<metamodelica::List<ArcStr>>> {
    ...
    let mut reader_c = match open_reporting(&filename)? {
        Some(r) => r,
        None => {
    ...
    let timeref_name = time_var_name(&allvarsref);
    ...
    for var in &cmpvars {
        ...
        drop_leading_dups(&mut data, offset);
