...
pub fn newFill<T: Clone + 'static>(size: i32, value: T) -> metamodelica::Ref<Vector<T>> {
...
pub fn fromArray<T: Clone + 'static>(arr: Array<T>) -> metamodelica::Ref<Vector<T>> {
...
pub fn toArray<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> Array<T> {
...
pub fn fromArrayNoCopy<T: Clone + 'static>(arr: Array<T>, size: i32) -> metamodelica::Ref<Vector<T>> {
...
pub fn rawArray<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> Array<T> {
    (*v).clone()
}

pub fn fromList<T: Clone + 'static>(l: List<T>) -> metamodelica::Ref<Vector<T>> {
    metamodelica::Ref::new(metamodelica::arrayFromVec(l.into_iter().cloned().collect()))
}

pub fn toList<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> List<T> {
    let data = v.borrow();
...
pub fn push<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, value: T) {
    v.borrow_mut().push(value);
}

pub fn insert<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, value: T, index: i32) -> Result<()> {
...
pub fn append<T: Clone + 'static>(v1: metamodelica::Ref<Vector<T>>, v2: metamodelica::Ref<Vector<T>>) {
...
pub fn appendList<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, l: List<T>) -> Result<()> {
...
pub fn appendArray<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, arr: Array<T>) {
    let extension: Vec<T> = arr.borrow().clone();
    v.borrow_mut().extend(extension);
}

pub fn pop<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) {
...
pub fn clear<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) {
    // MetaModelica's `clear` preserves capacity; `Vec::clear` does the same.
    v.borrow_mut().clear();
}

pub fn shrink<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, newSize: i32) {
    let mut data = v.borrow_mut();
...
pub fn grow<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, newSize: i32, fillValue: T) {
    let mut data = v.borrow_mut();
    let new_len = newSize.max(0) as usize;
    if new_len > data.len() {
...
pub fn resize<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, newSize: i32, fillValue: T) {
    v.borrow_mut().resize(newSize.max(0) as usize, fillValue);
}

pub fn remove<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, index: i32) -> Result<()> {
    let mut data = v.borrow_mut();
...
pub fn update<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, index: i32, value: T) -> Result<()> {
    let mut data = v.borrow_mut();
    let sz = data.len() as i32;
    if index <= 0 || index > sz {
...
pub fn updateNoBounds<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, index: i32, value: T) {
    v.borrow_mut()[idx(index)] = value;
}

pub fn get<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, index: i32) -> Result<T> {
    let data = v.borrow();
    let sz = data.len() as i32;
    if index <= 0 || index > sz {
...
pub fn getNoBounds<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, index: i32) -> T {
    v.borrow()[idx(index)].clone()
}

pub fn last<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> Result<T> {
    let data = v.borrow();
    match data.last() {
        Some(e) => Ok(e.clone()),
...
pub fn size<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> i32 {
    v.borrow().len() as i32
}

pub fn capacity<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> i32 {
    v.borrow().capacity() as i32
}

pub fn isEmpty<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> bool {
    v.borrow().is_empty()
}

pub fn reserve<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, newCapacity: i32) {
    let mut data = v.borrow_mut();
...
pub fn trim<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) {
    v.borrow_mut().shrink_to_fit();
}

pub fn fill<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>, value: T, from: i32, to: i32) -> Result<()> {
    let mut data = v.borrow_mut();
    let sz = data.len() as i32;
...
fn elem_at<T: Clone + 'static>(v: &metamodelica::Ref<Vector<T>>, i: usize) -> Option<T> {
...
pub fn copy<T: Clone + 'static>(v: metamodelica::Ref<Vector<T>>) -> metamodelica::Ref<Vector<T>> {
...
pub fn swap<T: Clone + 'static>(v1: metamodelica::Ref<Vector<T>>, v2: metamodelica::Ref<Vector<T>>) {
