//! FMI 3.0 C API over an [fmi-ls-wasm](https://github.com/modelica/fmi-ls-wasm)
...
//!
//! The library is model-agnostic — one build per platform serves every FMU. The
//! model arrives as a resource next to it:
//!
//! | file | used when |
//! | --- | --- |
//! | `resources/<platform>.cwasm` | always, if present: a precompiled artifact, `deserialize`d in ~1 ms |
//! | `resources/model.wasm` | `jit` builds only: the component, compiled at instantiate |
//! | `binaries/wasm32-wasip2/*.wasm` | `jit` builds only, last resort — the wasm binary the FMU already ships |
//!
...
//!
//! Every `fmi3*` below is an FMI 3.0 C entry point: unsafe by signature, with the
...
impl Host {
    fn log(&mut self, status: i32, category: &str, message: &str) {
        let Some(cb) = self.log else { return };
        let (Ok(cat), Ok(msg)) = (CString::new(category), CString::new(message)) else { return };
        cb(self.env, status, cat.as_ptr(), msg.as_ptr());
    }
}
...
/// textually identical but resolve to different generated types.
macro_rules! on_instance {
    ($c:expr, |$store:ident, $g:ident, $h:ident, $st:ident| $body:expr) => {{
        let Some(inst) = (unsafe { ($c as *mut Instance).as_mut() }) else { return ERROR };
...
fn component_source(res: &Path) -> Option<PathBuf> {
    let direct = res.join("model.wasm");
    if direct.is_file() {
        return Some(direct);
    }
    let binaries = res.parent()?.join("binaries/wasm32-wasip2");
...
fn new_store(engine: &Engine, res: &str, env: *mut c_void, log: LogCb, iu: IntermediateUpdateCb) -> wasmtime::Result<Store<Host>> {
...
fn report(name: &str, env: *mut c_void, log: LogCb, e: impl std::fmt::Display) -> *mut c_void {
    if let Some(cb) = log {
        let msg = CString::new(format!("{name}: {e}")).unwrap_or_else(|_| c"instantiation failed".into());
        cb(env, ERROR, c"error".as_ptr(), msg.as_ptr());
    }
    std::ptr::null_mut()
}
...
pub unsafe extern "C" fn fmi3InstantiateModelExchange(
    ...
    let token = cstr(instantiation_token);
    let res = cstr(resource_path);
    let built = (|| -> wasmtime::Result<Box<Instance>> {
        ...
        let mut store = new_store(&engine, &res, instance_environment, log_message, None)?;
        ...
        let handle = world
            ...
            .ok_or_else(|| wasmtime::Error::msg("fmi3InstantiateModelExchange: the FMU refused to instantiate"))?;
        Ok(Box::new(Instance { store, kind: Kind::Me { world, handle }, strings: Vec::new(), binaries: Vec::new() }))
    })();
    match built {
        Ok(b) => Box::into_raw(b) as *mut c_void,
        Err(e) => report(&name, instance_environment, log_message, e),
    }
}
...
pub unsafe extern "C" fn fmi3InstantiateCoSimulation(
    ...
    let built = (|| -> wasmtime::Result<Box<Instance>> {
        ...
        let mut store = new_store(&engine, &res, instance_environment, log_message, intermediate_update)?;
        ...
        let handle = world
            ...
            .ok_or_else(|| wasmtime::Error::msg("fmi3InstantiateCoSimulation: the FMU refused to instantiate"))?;
        Ok(Box::new(Instance { store, kind: Kind::Cs { world, handle }, strings: Vec::new(), binaries: Vec::new() }))
    })();
    match built {
        Ok(b) => Box::into_raw(b) as *mut c_void,
        Err(e) => report(&name, instance_environment, log_message, e),
...
pub unsafe extern "C" fn fmi3InstantiateScheduledExecution(
    ...
    let name = cstr(instance_name);
    report(&name, instance_environment, log_message, "Scheduled Execution is not supported by this FMU")
}
...
macro_rules! nullary {
    ($cfn:ident, $wfn:ident) => {
        ...
        pub unsafe extern "C" fn $cfn(c: *mut c_void) -> i32 {
            on_instance!(c, |store, g, h, st| match g.$wfn(store, h) {
                Ok(s) => st(s),
                Err(_) => ERROR,
            })
        }
...
pub unsafe extern "C" fn fmi3EnterInitializationMode(
    ...
    on_instance!(c, |store, g, h, st| match g.call_enter_initialization_mode(store, h, tol, start_time, stop) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
pub unsafe extern "C" fn fmi3SetDebugLogging(
    ...
    on_instance!(c, |store, g, h, st| match g.call_set_debug_logging(store, h, logging_on, &cats) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
pub unsafe extern "C" fn fmi3UpdateDiscreteStates(
    ...
    on_instance!(c, |store, g, h, st| match g.call_update_discrete_states(store, h) {
        Ok(Ok(info)) => out!(info),
        Ok(Err(s)) => st(s),
        Err(_) => ERROR,
    })
...
macro_rules! getter {
    ($cfn:ident, $wfn:ident, $ty:ty) => {
        ...
        pub unsafe extern "C" fn $cfn(
            ...
            on_instance!(c, |store, g, h, st| match g.$wfn(store, h, refs) {
                Ok(Ok(v)) => {
...
macro_rules! setter {
    ($cfn:ident, $wfn:ident, $ty:ty) => {
        ...
        pub unsafe extern "C" fn $cfn(
            ...
            on_instance!(c, |store, g, h, st| match g.$wfn(store, h, refs, vals) {
                Ok(s) => st(s),
                Err(_) => ERROR,
            })
        }
...
pub unsafe extern "C" fn fmi3GetClock(
    ...
    let refs = vrs(value_references, n_value_references);
    on_instance!(c, |store, g, h, st| match g.call_get_clock(store, h, refs) {
        Ok(Ok(v)) => {
...
pub unsafe extern "C" fn fmi3SetClock(
    ...
    on_instance!(c, |store, g, h, st| match g.call_set_clock(store, h, refs, vals) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
pub unsafe extern "C" fn fmi3SetString(
    ...
    on_instance!(c, |store, g, h, st| match g.call_set_string(store, h, refs, &vals) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
pub unsafe extern "C" fn fmi3SetBinary(
    ...
    let vals: Vec<Vec<u8>> = (0..n_values)
        ...
        .collect();
    on_instance!(c, |store, g, h, st| match g.call_set_binary(store, h, refs, &vals) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
pub unsafe extern "C" fn fmi3GetFMUState(c: *mut c_void, state: *mut *mut c_void) -> i32 {
    ...
    on_instance!(c, |store, g, h, st| match g.call_get_fmu_state(store, h) {
        Ok(Ok(bytes)) => {
...
pub unsafe extern "C" fn fmi3SetFMUState(c: *mut c_void, state: *mut c_void) -> i32 {
    ...
    on_instance!(c, |store, g, h, st| match g.call_set_fmu_state(store, h, bytes) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
macro_rules! derivative {
    ($cfn:ident, $wfn:ident) => {
        ...
        pub unsafe extern "C" fn $cfn(
            ...
            on_instance!(c, |store, g, h, st| match g.$wfn(store, h, u, k, s) {
                Ok(Ok(v)) => {
...
pub unsafe extern "C" fn fmi3GetNumberOfVariableDependencies(
    ...
    on_instance!(c, |store, g, h, st| match g.call_get_number_of_variable_dependencies(store, h, value_reference) {
        Ok(Ok(n)) => {
...
pub unsafe extern "C" fn fmi3GetVariableDependencies(
    ...
    on_instance!(c, |store, g, h, st| match g.call_get_variable_dependencies(store, h, dependent) {
        Ok(Ok(deps)) => out!(deps),
        Ok(Err(s)) => st(s),
        Err(_) => ERROR,
    })
...
pub unsafe extern "C" fn fmi3GetIntervalDecimal(
    ...
    let refs = vrs(value_references, n_value_references);
    on_instance!(c, |store, g, h, st| match g.call_get_interval_decimal(store, h, refs) {
        Ok(Ok(v)) => {
...
pub unsafe extern "C" fn fmi3GetIntervalFraction(
    ...
    let refs = vrs(value_references, n_value_references);
    on_instance!(c, |store, g, h, st| match g.call_get_interval_fraction(store, h, refs) {
        Ok(Ok(v)) => {
            if v.len() != n_value_references {
                return ERROR;
            }
...
pub unsafe extern "C" fn fmi3GetShiftDecimal(
    ...
    let refs = vrs(value_references, n_value_references);
    on_instance!(c, |store, g, h, st| match g.call_get_shift_decimal(store, h, refs) {
        Ok(Ok(v)) => {
...
pub unsafe extern "C" fn fmi3GetShiftFraction(
    ...
    let refs = vrs(value_references, n_value_references);
    on_instance!(c, |store, g, h, st| match g.call_get_shift_fraction(store, h, refs) {
        Ok(Ok(v)) => {
            if v.len() != n_value_references {
                return ERROR;
            }
...
pub unsafe extern "C" fn fmi3SetIntervalDecimal(
    ...
    on_instance!(c, |store, g, h, st| match g.call_set_interval_decimal(store, h, refs, v) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
pub unsafe extern "C" fn fmi3SetShiftDecimal(
    ...
    on_instance!(c, |store, g, h, st| match g.call_set_shift_decimal(store, h, refs, v) {
        Ok(s) => st(s),
        Err(_) => ERROR,
    })
}
...
pub unsafe extern "C" fn fmi3GetOutputDerivatives(
    ...
    let Some(requests) = requests(value_references, n_value_references, orders) else { return ERROR };
    on_instance!(c, |store, g, h, st| match g.call_get_output_derivatives(store, h, &requests) {
        Ok(Ok(v)) => {
            if v.len() != n_values || values.is_null() {
                return ERROR;
            }
