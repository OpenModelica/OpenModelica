//! faer 0.24.4's `src/linalg/evd/schur/real_schur.rs`, verbatim apart from the
...
fn lasy2<T: RealField>(
	...
	for i in 0..3usize {
		...
		if ipsv != i {
			faer::perm::swap_rows_idx(t16.rb_mut(), ipsv, i);
			let temp = btmp[(i, 0)].copy();
			btmp[(i, 0)] = btmp[(ipsv, 0)].copy();
			btmp[(ipsv, 0)] = temp;
		}
		if jpsv != i {
			faer::perm::swap_cols_idx(t16.rb_mut(), jpsv, i);
		}
		jpiv[i] = jpsv;
...
pub(crate) fn schur_swap_12<T: RealField>(
	...
	let (mut head, tail) = v1.rb_mut().split_at_row_mut(1);
	let HouseholderInfo { tau: tau1, .. } =
		make_householder_in_place(&mut head[0], tail);
	...
	let (mut head, tail) = v2.rb_mut().split_at_row_mut(1);
...
fn multishift_qr_sweep<T: RealField>(
	...
	assert!(n >= 12);
	let (mut v, _stack) =
		temp_mat_zeroed(3, s_re.nrows() / 2, stack);
...
// faer's own `mod tests` is dropped here; ours is in lib.rs.
