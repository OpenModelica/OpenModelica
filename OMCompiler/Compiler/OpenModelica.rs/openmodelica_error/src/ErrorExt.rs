...
/// `RUNTIME`/`Error`. For our own ModelicaUtilities (the wasm-jit/FFI simulation
/// environment); see `libopenmodelica_compiler::modelica_utilities`.
pub fn runtime_error(msg: &str) {
    add_runtime_error_message(msg);
}

/// Panic payload marking an expected runtime error (a `ModelicaError` / failed
/// `omc_assert` from an external simulation function). Its message is already in
/// the Error buffer; [`catch_runtime_error`] turns this into an ordinary failure
/// and the panic hook installed by [`install_runtime_panic_hook`] suppresses the
/// default backtrace print for it.
pub struct RuntimeAbort;

/// Raise a runtime-error panic. Called from the C interception shims
/// (`runtime_error_shim.c`, panic mode) after they record the message: the
/// simulation runtime's own throw is a `longjmp` into an unset jump buffer, so in
/// Rust mode we unwind with a panic instead, caught by [`catch_runtime_error`].
/// `extern "C-unwind"` so the panic may propagate out through the calling C frame.
#[unsafe(no_mangle)]
pub extern "C-unwind" fn omrs_runtime_abort() -> ! {
    std::panic::panic_any(RuntimeAbort)
}

/// Suppress the default panic hook's backtrace print for [`RuntimeAbort`] (an
/// expected, reported error), chaining to the previous hook for real panics.
/// Idempotent.
pub fn install_runtime_panic_hook() {
    use std::sync::Once;
    static ONCE: Once = Once::new();
    ONCE.call_once(|| {
        let prev = std::panic::take_hook();
        std::panic::set_hook(Box::new(move |info| {
            if info.payload().is::<RuntimeAbort>() {
                return;
            }
            prev(info);
        }));
...
/// Run `f`, catching a [`RuntimeAbort`] panic raised by [`omrs_runtime_abort`]
/// (i.e. an external function's `ModelicaError`). Returns `Err(())` on such a
/// runtime error (message already in the Error buffer); any other panic is
/// resumed. Used to wrap the libffi call to an external simulation function.
pub fn catch_runtime_error<F: FnOnce()>(f: F) -> Result<(), ()> {
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(f)) {
        Ok(()) => Ok(()),
        Err(payload) if payload.is::<RuntimeAbort>() => Err(()),
        Err(payload) => std::panic::resume_unwind(payload),
    }
}

/// Route `ModelicaMessage`/`ModelicaFormatMessage` to the buffer as a
/// `RUNTIME`/`Notification`.
pub fn runtime_message(msg: &str) {
    add_runtime_message_at(msg, Severity::NOTIFICATION);
}

/// Route `ModelicaWarning`/`ModelicaFormatWarning` to the buffer as a
/// `RUNTIME`/`Warning`.
pub fn runtime_warning(msg: &str) {
    add_runtime_message_at(msg, Severity::WARNING);
}
