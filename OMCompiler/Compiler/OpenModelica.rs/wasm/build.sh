...
# build profile. Output goes to wasm/omc/ for the browser (matching index.html's
# `./omc/` import and the cmake install layout) and wasm/pkg-nodejs/ for Node
# (matching omc-cli.js), so the two hosts don't clobber each other.
#
...
# The LLVM rustc backend is forced for wasm32 in .cargo/config.toml (the
# workspace dev profile's cranelift backend cannot target wasm32).
