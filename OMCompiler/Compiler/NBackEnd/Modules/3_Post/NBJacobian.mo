...
protected
  ...
  algorithm
    ...
    Lfg_jacobian := func(name, JacobianType.OPT, seedCandidates, partialCandidates, part.equations, all_knowns, part.strongComponents, funcMap, init);
  ...
  algorithm
    ...
    if Partition.Partition.isODEorDAE(part) then
      ...
      if Flags.getConfigBool(Flags.MOO_DYNAMIC_OPTIMIZATION) then
        ...
        (Lfg_jacobian, Mr_jacobian) := partJacobianDynamicOptimization(part, knowns, name, func, funcMap);
      end if;
