...
public
  ...
  */
  ...
  uniontype Matrix
    ...
    // do a different one for LINEAR MATCHING and SORTING so we can "upgrade" them
    // EMPTY -> FULL -> LINEAR -> MATCHING -> SORTING (least work to do)
    ...
    algorithm
      (adj, funcTree) := match (adj, idx_lst)
        ...
        case (FINAL(), {-1})    then create(vars, eqns, adj.st);
