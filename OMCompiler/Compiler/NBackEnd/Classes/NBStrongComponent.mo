...
public
  ...
  record GENERIC_EQUATION
    "has no status since this is generated by the Solve module and is allways status=EXPLICIT."
    Slice<EquationPointer> eqn  "sliced equation";
  end GENERIC_EQUATION;
  ...
  algorithm
    str := match comp
      ...
      case GENERIC_EQUATION() algorithm
        str := StringUtil.headline_3("BLOCK" + indexStr + ": Generic Equation (status = Solve.EXPLICIT)");
        str := str + "### Equation:\n" + Slice.toString(comp.eqn, function Equation.pointerToString(str = "\t")) + "\n";
      then str;
  ...
  algorithm
    i := match comp
      ...
      case GENERIC_EQUATION()       then Equation.hash(Slice.getT(comp.eqn), mod);
  ...
  algorithm
    b := match(comp1, comp2)
      ...
      case (GENERIC_EQUATION(), GENERIC_EQUATION())               then Slice.isEqual(comp1.eqn, comp2.eqn, Equation.equalName);
