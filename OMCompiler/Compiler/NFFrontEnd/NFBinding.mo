...
public
  ...
  type Source = enumeration(
    ...
    TYPE      "The binding is an attribute modifier of a type, see Inst.markTypeModifier.",
...
public
    ...
    () := match binding
      case RAW_BINDING()
        algorithm
          ...
          if binding.source <> Source.TYPE then
  ...
  algorithm
    () := match binding
      case RAW_BINDING() algorithm binding.confidence := confidence; then ();
      case UNTYPED_BINDING() algorithm binding.confidence := confidence; then ();
      case TYPED_BINDING() algorithm binding.confidence := confidence; then ();
      case FLAT_BINDING() algorithm binding.confidence := confidence; then ();
      else ();
    end match;
  end setConfidence;
  ...
  function isFromType
    "Returns true if the binding is an attribute modifier of a type rather than
     of a component."
    input Binding binding;
    output Boolean fromType = source(binding) == Source.TYPE;
  end isFromType;

  function compareStartConfidence
    "Compares two start attributes by MLS 8.6.2 priority: a start set on a
     component beats one set by its type, then the lower actual confidence wins,
     then the lower raw confidence. Negative if b1 is stronger, 0 if tied."
    input Binding b1;
    input Binding b2;
    output Integer cmp;
  algorithm
    if isFromType(b1) <> isFromType(b2) then
      cmp := if isFromType(b1) then 1 else -1;
    else
      cmp := actualConfidence(b1) - actualConfidence(b2);

      if cmp == 0 then
        cmp := confidence(b1) - confidence(b2);
      end if;
    end if;
  end compareStartConfidence;
  ...
  function actualConfidence
    "Returns the actual confidence for a binding, which if the binding consists
     ...
     per 8.6.2. For any other kind of binding, and for the attribute modifiers
     of a type, the actual confidence is just the confidence stored in the
     binding."
    input Binding binding;
