      ...
      algorithm
        if isSome(binding) then
          ...
          if Binding.isBound(b) then
            b := flattenBinding(b, ComponentRef.nodeType(prefix), ComponentRef.rest(prefix));
...
algorithm
  ...
  if isSome(outerBinding) then
    ...
    binding := flattenBinding(binding, ty, prefix);
    unfix := false;
  end if;
  ...
  name := ComponentRef.prefixCref(comp_node, ty, {}, prefix);
  ty_attrs := list(flattenTypeAttribute(m, ty, name) for m in typeAttrs);
  ...
  if unfix then
    ty_attrs := Binding.setAttr(ty_attrs, "fixed",
      Binding.FLAT_BINDING(Expression.fillType(ty, Expression.BOOLEAN(false)), Variability.CONSTANT, info));
  end if;
...
protected
  Binding binding;
algorithm
  binding := flattenBinding(Modifier.binding(attr), parentType, prefix, isTypeAttribute = true);
  ...
  recordBindings := match binding_exp
    case Expression.RECORD()
      then list(if Expression.isEmpty(e) then
               ...
               Binding.FLAT_BINDING(e, var, Binding.getInfo(binding))
...
algorithm
  binding := match binding
    ...
    case Binding.TYPED_BINDING()
      algorithm
        bind_exp := flattenBindingExp(binding.bindingExp, prefix, isTypeAttribute);

        // If the binding is some kind of each binding, lift it so that it gets
        // the same dimensions as the expected type. E.g.:
        // Real x[3](each start = 1) => Real x[3](start = {1, 1, 1})
        if binding.eachType <> NFBinding.EachType.NOT_EACH then
          dims := Type.arrayDims(componentType);
          dims := List.firstN(dims, listLength(dims) - Type.dimensionCount(Expression.typeOf(bind_exp)));
          bind_exp := Expression.liftArrayList(dims, bind_exp);
        end if;
      then
        Binding.FLAT_BINDING(bind_exp, binding.variability, binding.info);
...
algorithm
  binding := Binding.mapExp(var.binding,
