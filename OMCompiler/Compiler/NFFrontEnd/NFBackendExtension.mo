...
public
  uniontype BackendInfo
    record BACKEND_INFO
      ...
      Option<PointerCyclic<Variable>> var_pre      "Pointer (var -> pre) or (pre -> var) if existent.";
      Option<PointerCyclic<Variable>> var_seed     "Pointer (var -> seed) or (seed -> var) if existent.";
      Option<PointerCyclic<Variable>> var_pder_res "Pointer (var -> pder, result var in Jacobian) or (pder -> var) if existent.";
      Option<PointerCyclic<Variable>> var_pder_tmp "Pointer (var -> pder, tmp var in Jacobian) or (pder -> var) if existent.";
      Option<PointerCyclic<Variable>> var_start    "Pointer (var -> start) or (start -> var) if existent.";
      Option<PointerCyclic<Variable>> parent       "record parent if it is part of a record.";
    end BACKEND_INFO;
    ...
    partial function setPartner
      input output BackendInfo binfo;
      input Option<PointerCyclic<Variable>> var_ptr;
    end setPartner;
  ...
  uniontype VariableKind
    ...
    record STATE
      ...
      Option<PointerCyclic<Variable>> derivative  "pointer to the derivative";
      Boolean natural                       "false if it was forced by StateSelect.always or StateSelect.prefer or generated by index reduction";
    end STATE;
    record STATE_DER
      PointerCyclic<Variable> state               "Original state";
      Option<PointerCyclic<Expression>> alias     "Optional alias state expression. Result of differentiating the state if existant!";
    end STATE_DER;
    record DUMMY_DER
      PointerCyclic<Variable> dummy_state         "corresponding dummy state";
    end DUMMY_DER;
    record DUMMY_STATE
      PointerCyclic<Variable> dummy_der           "corresponding dummy derivative";
    ...
    record START
      PointerCyclic<Variable> original            "Pointer to the corresponding original variable.";
    end START;
    record EXTOBJ
