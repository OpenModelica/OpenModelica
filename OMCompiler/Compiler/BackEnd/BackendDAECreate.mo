...
algorithm
  outEqns := matchcontinue(cond, conditions, conditions1, theneqns1)
    local
      list<DAE.Exp> explst;
      list<list<BackendDAE.Equation>> beqnslst;
      list<BackendDAE.Equation> beqns,breqns,bieqns;
...
protected function selectAlias
  ...
  output list<BackendDAE.Equation> oEqns;
algorithm
  (oVars, outGlobalKnownVars, oExtVars, oAVars, oRepl, oEqns) := matchcontinue (exp1, exp2, iAVars)
    local
      BackendDAE.Variables vars, globalKnownVars, extvars, avars;
      BackendVarTransform.VariableReplacements repl;
      list<BackendDAE.Equation> eqns;
    ...
    // array array case
    case (DAE.ARRAY(array=explst1), DAE.ARRAY(array=explst2), _)
      algorithm
    ...
    case (DAE.CREF(componentRef=cr1, ty=DAE.T_ARRAY(dims = dims1)), DAE.ARRAY(array=explst2), _)
      algorithm
    ...
    case (DAE.ARRAY(array=explst1), DAE.CREF(componentRef=cr2, ty=DAE.T_ARRAY(dims = dims2)), _)
      algorithm
    ...
    case (DAE.CREF(componentRef=cr1, ty=DAE.T_ARRAY(dims = dims1)), DAE.CREF(componentRef=cr2, ty=DAE.T_ARRAY(dims = dims2)), _)
      algorithm
        crefs1 := ComponentReference.expandArrayCref(cr1, dims1);
        explst1 := List.map(crefs1, Expression.crefExp);
        crefs2 := ComponentReference.expandArrayCref(cr2, dims2);
    ...
    // matrix matrix case
    case (DAE.MATRIX(matrix=explstlst1), DAE.MATRIX(matrix=explstlst2), _)
      algorithm
...
algorithm
  (oVars, outGlobalKnownVars, oExtVars, oAVars, oRepl) :=
   match (v1, index1, arrayIndx1, e1, v2, index2, arrayIndx2, source, inGlobalKnownVars, iExtVars, iAVars)
    local
      BackendDAE.Variables vars, globalKnownVars, extvars, avars;
      BackendVarTransform.VariableReplacements repl;
    ...
    case (BackendDAE.VAR(varKind=BackendDAE.STATE()), _, 1, _,
          BackendDAE.VAR(varName=cr2), _, 1, _, _, _, _)
      algorithm
        // check if replacable
    ...
    case (BackendDAE.VAR(varName=cr1), _, 1, _,
          BackendDAE.VAR(varKind=BackendDAE.STATE()), _, 1, _, _, _, _)
      algorithm
        // check if replacable
...
"This function updates the variable kind to discrete
  ...
  output BackendDAE.Variables outVariables;
algorithm
  outVariables := matchcontinue (inEquation, inVariables)
    local
      DAE.ComponentRef cr;
      list<DAE.ComponentRef> crefs;
      DAE.Exp e;
    ...
    case (BackendDAE.WHEN_EQUATION(whenEquation = BackendDAE.WHEN_STMTS(whenStmtLst = {BackendDAE.ASSIGN(left=DAE.CREF(componentRef=cr))})), _)
      algorithm
    ...
    case (BackendDAE.WHEN_EQUATION(whenEquation = BackendDAE.WHEN_STMTS(whenStmtLst = {BackendDAE.ASSIGN(left=e)})), _)
      algorithm
        ...
        vars := List.map1(vars, BackendVariable.setVarKind, BackendDAE.DISCRETE());
      then BackendVariable.addVars(vars, inVariables);
    case (BackendDAE.ALGORITHM(alg=DAE.ALGORITHM_STMTS(statementLst = statementLst)), _)
...
protected function detectImplicitDiscreteAlgsStatemensFor
  ...
  output BackendDAE.Variables outVariables;
algorithm
  outVariables := matchcontinue (inIteratorExp, inExplst, inVariables, inGlobalKnownVars, inStatementLst)
    local
    ...
    case (ie, e::{}, v, globalKnownVars, statementLst)
      ...
      then
        v_1;
    case (ie, e::rest, v, globalKnownVars, statementLst)
      algorithm
        (statementLst1, _) := DAEUtil.traverseDAEEquationsStmts(statementLst, Expression.replaceExpTpl, ((ie, e)));
        v_1 := detectImplicitDiscreteAlgsStatemens(v, globalKnownVars, statementLst1, true);
        v_2 := detectImplicitDiscreteAlgsStatemensFor(ie, rest, v_1, globalKnownVars, statementLst, insideWhen);
      then
        v_2;
    case (ie, e::rest, v, globalKnownVars, statementLst)
      algorithm
