...
algorithm
   outRepl := match(inSrcs,inDsts,inFuncTypeExpExpToBooleanOption)
     local
       DAE.ComponentRef cr;
...
protected
  list<DAE.Ident> iv;
algorithm
  (REPLACEMENTS(iterationVars=iv),_) := (repl, inVar);
  is := listMember(inVar, iv);
end isIterationVar;
...
"This function takes a list of equations ana a set of variable
  ...
  output Boolean replacementPerformed;
algorithm
  (outEqns,replacementPerformed) := matchcontinue()
    local
...
algorithm
  ...
  match (inBackendDAEEquationLst,inVariableReplacements)
    local
      ...
      Boolean b;
    case ({},_) then (listReverse(inAcc),iReplacementPerformed);
    case (a::es,_)
      algorithm
...
protected function optimizeIfEquation
  ...
  output list<BackendDAE.Equation> outEqns;
algorithm
  outEqns := matchcontinue(conditions,theneqns,elseenqs,conditions1,theneqns1)
    local
      DAE.Exp e;
      list<DAE.Exp> explst;
      list<list<BackendDAE.Equation>> eqnslst;
...
algorithm
  (outElse,replacementPerformed) := matchcontinue (inElse)
    local
      ...
      Boolean b1,b2;
    case (DAE.ELSEIF(exp=e1,statementLst=statementLst,else_=else_))
      algorithm
        (e1_1,b1) := replaceExp(e1, inVariableReplacements,inFuncTypeExpExpToBooleanOption);
        (e1_2,_) := ExpressionSimplify.condsimplify(b1,e1_1);
        (else_1,b2) := replaceElse1(e1_2,statementLst,else_,inVariableReplacements,inFuncTypeExpExpToBooleanOption);
        true := b1 or b2;
      then
        (else_1,true);
    case (DAE.ELSE(statementLst=statementLst))
      algorithm
        (statementLst_1,true) := replaceStatementLst(statementLst, inVariableReplacements,inFuncTypeExpExpToBooleanOption,{},false);
      then
