  ...
  matchcontinue (inCache,inEnv,className,outputFileIn,dumpSteps)
    ...
    case (cache,graph,_,outputFile,_)
      equation
        ...
        //(dlow_1,funcs1) = BackendDAEUtil.getSolvedSystem(dlow, funcs,SOME({"removeSimpleEquations","removeFinalParameters", "comSubExp", "expandDerOperator"}), NONE(), NONE(),NONE());
        Flags.setConfigBool(Flags.DEFAULT_OPT_MODULES_ORDERING, false);
        (dlow_1) = BackendDAEUtil.getSolvedSystem(dlow, "", SOME({"removeSimpleEquations","removeUnusedVariables","comSubExp","expandDerOperator"}), NONE(), NONE(), SOME({}));
