...
::=
match program
  /*Program without classes. Do nothing.*/
...
match restriction
  ...
  else error(sourceInfo(), 'Unknown restriction encountered')
end dumpRestriction;
...
match alg
  ...
  case ALG_WHEN_A(__) then
   "TODO: If alg when occurs.. It just did. Should it be translated to a simple if?"
  ...
  case ALG_TRY(__) then
    ...
    try
      <%arg1%>
    except Execption //MM does not really have specialised exceptions to my understanding
      <%arg2%>
    end;
    >>
...
"Just calls the corresponding function in AbsynDumptpl for most things.
 Matrix operations are probably not used that much in the compiler code. We can probably just provide suitable overloads for them."
::= match op
      case AND(__) then '&&'
      case OR(__) then '||'
      case NOT(__) then '!'
