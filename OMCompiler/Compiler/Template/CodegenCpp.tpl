...
case SIMCODE(modelInfo=MODELINFO(__), makefileParams=MAKEFILE_PARAMS(__), simulationSettingsOpt = sopt) then
  ...
  # /TP - Use C++ Compiler
  !IF "$(PCH_FILE)" == ""
  CFLAGS=  $(SYSTEM_CFLAGS) /I"$(OMHOME)/include/omc/cpp/" /I. <%makefileParams.includes%>  /I"$(BOOST_INCLUDE)" /I"$(UMFPACK_INCLUDE)" /I"$(SUNDIALS_INCLUDE)" /DNOMINMAX /TP /DNO_INTERACTIVE_DEPENDENCY <%additionalCFlags_MSVC%>
  !ELSE
  CFLAGS=  $(SYSTEM_CFLAGS) /I"$(OMHOME)/include/omc/cpp/" /I. <%makefileParams.includes%>  /I"$(BOOST_INCLUDE)" /I"$(UMFPACK_INCLUDE)" /I"$(SUNDIALS_INCLUDE)" /DNOMINMAX /TP /DNO_INTERACTIVE_DEPENDENCY  /Fp$(OMHOME)/include/omc/cpp/Core/$(PCH_FILE)  /YuCore/$(H_FILE) <%additionalCFlags_MSVC%>
  !ENDIF
  CPPFLAGS =
  # /ZI enable Edit and Continue debug info
  CDFLAGS = /ZI
  !IF "$(PCH_FILE)" == ""
  LDSYSTEMFLAGS=  /link /DLL  /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/cpp/msvc" /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/msvc" /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/msvc/debug"  /LIBPATH:"$(OMHOME)/bin" /LIBPATH:"$(BOOST_LIBS)" OMCppSystem.lib OMCppModelicaUtilities.lib  OMCppMath.lib OMCppDataExchange.lib  OMCppOMCFactory.lib <%timeMeasureLink%> WSock32.lib Ws2_32.lib
  LDMAINFLAGS=/link /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/cpp/msvc" /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/msvc" OMCppOMCFactory.lib OMCppModelicaUtilities.lib  <%timeMeasureLink%> /LIBPATH:"$(OMHOME)/bin" /LIBPATH:"$(BOOST_LIBS)"
  !ELSE
  LDSYSTEMFLAGS=  /link /DLL  /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/cpp/msvc" /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/msvc" /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/msvc/debug"  /LIBPATH:"$(OMHOME)/bin" /LIBPATH:"$(BOOST_LIBS)" OMCppSystem.lib OMCppModelicaUtilities.lib  OMCppMath.lib OMCppDataExchange.lib  OMCppOMCFactory.lib $(PCH_LIB) <%timeMeasureLink%> WSock32.lib Ws2_32.lib
  LDMAINFLAGS=/link /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/cpp/msvc" /LIBPATH:"$(OMHOME)/lib/<%Autoconf.triple%>/omc/msvc" OMCppOMCFactory.lib OMCppModelicaUtilities.lib $(PCH_LIB) <%timeMeasureLink%> /LIBPATH:"$(OMHOME)/bin" /LIBPATH:"$(BOOST_LIBS)"
  !ENDIF
