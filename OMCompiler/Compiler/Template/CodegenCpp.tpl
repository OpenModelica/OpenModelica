...
case "gcc" then
    match simCode
        case SIMCODE(modelInfo=MODELINFO(__), makefileParams=MAKEFILE_PARAMS(__), simulationSettingsOpt = sopt) then
            ...
            LDSYSTEMFLAGS_COMMON=-L"$(OMHOME)/lib/<%Config.targetTriple()%>/omc/cpp" $(BASE_LIB) <%additionalLinkerFlags_GCC%>  -Wl,-rpath,"$(OMHOME)/lib/<%Config.targetTriple()%>/omc/cpp" <%timeMeasureLink%> -L"$(BOOST_LIBS)" $(BOOST_LIBRARIES) $(LINUX_LIB_DL)
...
case SIMCODE(modelInfo = MODELINFO(__), modelStructure = fmims) then
  ...
  <%(clockedPartitions |> partition =>
    ...
    case CLOCKED_PARTITION(__) then
      ...
      let isEventBased = match baseClock case EVENT_CLOCK() then "true"
