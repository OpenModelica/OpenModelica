...
algorithm
  outSourceInfo  :=
  match (inStartLineColumnNumber, inEndLineInfo)
    local
      Integer startL, startC, endL, endC;
      LineInfo endlinfo;
      String  fileName;

    case ((startL, startC), endlinfo as LINE_INFO(parseInfo = PARSE_INFO(fileName = fileName)))
      algorithm
...
algorithm
  (outChars, outLineInfo) := matchcontinue (inChars, inLineInfo)
    ...
    case (chars as ("/" :: "*" :: charsRest), linfo)
      algorithm
        failure(comment(charsRest, linfo));
...
algorithm
  (outChars, outLineInfo, outError) := matchcontinue (inChars, inLineInfo)
    ...
    case (startChars as (_ :: chars), linfo)
      algorithm
        failure(newLine(startChars, linfo));
        (chars, linfo, optErr) := stringCommentRest(chars, linfo);
      then (chars, linfo, optErr);
...
public function checkConstantType
  ...
  output LineInfo outLineInfo;
algorithm
  (outChars, outLineInfo) := match (inChars, inLineInfo, inConstType, inConstTypeLiteral)
    local
      list<String> chars;
      LineInfo linfo;
    ...
    case (chars, linfo, TplAbsyn.UNRESOLVED_TYPE(_), _)
      then (chars, linfo);

    case (chars, linfo, _, TplAbsyn.UNRESOLVED_TYPE(_))
      then (chars, linfo);

    //types resolved, but not equal
    case (chars, linfo, ctype, litType) guard not (valueEq(ctype, litType))
      algorithm
...
public function exponent
  ...
  output TplAbsyn.TypeSignature outLitType;
algorithm
  (outChars,outExponent,outLitType) := matchcontinue inChars
    local
      String ex,pm;
      list<String> chars, ds;
  ...
  (outChars, outLineInfo, outExpression) := matchcontinue (inChars, inLineInfo, inLeftEsc, inRightEsc)
   ...
   case ("<"::"<":: startChars, startLInfo as LINE_INFO(startOfLineChars = solChars), lesc, resc)
      algorithm
        ...
        failure( takeSpaceAndNewLine(startChars, startLInfo) );
...
algorithm
  ...
  := matchcontinue (inChars, inLineInfo, inAccStringChars)
    local
      list<String> chars;
      LineInfo linfo;
      Integer lineInd;
