...
::=
  ...
  void <%symbolName(modelNamePrefix, name)%>_DAG(DATA* data, threadData_t* threadData)
  {
    ...
    'buildEvalDAG_<%name%>(data->modelData, 0, NULL);' /* MSVC rejects an empty array initializer */
    %>
  }
  >>
end functionXXX_DAG;
...
::=
  ...
  void <%symbolName(modelNamePrefix, "Jac")%><%name%>_DAG(DATA* data, threadData_t* threadData, JACOBIAN* jacobian)
  {
    ...
    'buildEvalDAG_Jac(jacobian, data->modelData, 0, NULL);' /* MSVC rejects an empty array initializer */
    %>
  }
  >>
end functionJac_DAG;
...
case SIMCODE(modelInfo=MODELINFO(varInfo=varInfo as VARINFO(__)), delayedExps=DELAYED_EXPRESSIONS(maxDelayedIndex=maxDelayedIndex), makefileParams=MAKEFILE_PARAMS(__), simulationSettingsOpt = sopt) then
  ...
  let debugFlags = if boolOr(Testsuite.isRunning(), Flags.isSet(Flags.GEN_DEBUG_SYMBOLS)) then "/Od /Zi" else "/O2"
  let extraCflags = match sopt case SOME(s as SIMULATION_SETTINGS(__)) then
    match s.method case "dassljac" then "/D_OMC_JACOBIAN "
  let libpath = '<%makefileParams.omhome%>/lib/<%Autoconf.triple%>/omc'
  <<
  # Makefile generated by OpenModelica
  # Platform: <%makefileParams.platform%> (MSVC / nmake)
  #
  # The generated model code links against the CMake-built OpenModelica runtime:
  #   SimulationRuntimeC.dll  - the simulation runtime (re-exports the whole
  #                             MetaModelica runtime absorbed from OpenModelicaRuntimeC)
  #   omcgc.dll               - the Boehm GC
  # IMPORT_INTO makes openmodelica.h's DLLDirection resolve the runtime's data
  # symbols (mmc_emptystring, record descriptions, ...) as __declspec(dllimport).
  ...
  # define OMC_CFLAGS_OPTIMIZATION env variable to override the optimization level
  OMC_CFLAGS_OPTIMIZATION=<%debugFlags%>
  CFLAGS_BASED_ON_INIT_FILE=<%extraCflags%>

  # /MD  - link against the DLL C runtime (matches the OpenModelica runtime DLLs)
  # /TC  - compile the generated code as C (it relies on implicit void* casts and
  #        C99 designated initializers; MSVC C++ before C++20 rejects both)
  # /EHsc- exception model (for the C++ runtime headers reached through the .inc)
  # /DOM_HAVE_PTHREADS - must match the threadData_t layout the runtime DLL was
  #        built with (parentMutex, mmc_init_once, ...)
  # /DNOMINMAX, /D_USE_MATH_DEFINES - Windows.h / math.h behaviour
  CFLAGS=/nologo /MD /TC /EHsc $(OMC_CFLAGS_OPTIMIZATION) $(CFLAGS_BASED_ON_INIT_FILE) /DOM_HAVE_PTHREADS /DNOMINMAX /D_USE_MATH_DEFINES /DIMPORT_INTO=1 /DOPENMODELICA_XML_FROM_FILE_AT_RUNTIME<% if Flags.isSet(Flags.OMC_RELOCATABLE_FUNCTIONS) then " /DOMC_GENERATE_RELOCATABLE_CODE"%> /DOMC_MODEL_PREFIX=<%modelNamePrefix(simCode)%> /DOMC_NUM_MIXED_SYSTEMS=<%varInfo.numMixedSystems%> /DOMC_NUM_LINEAR_SYSTEMS=<%varInfo.numLinearSystems%> /DOMC_NUM_NONLINEAR_SYSTEMS=<%varInfo.numNonLinearSystems%> /DOMC_NDELAY_EXPRESSIONS=<%maxDelayedIndex%> /DOMC_NVAR_STRING=<%varInfo.numStringAlgVars%><% if Config.simCodeRustRuntime() then " /DOMC_RUST_SIMULATION_RUNTIME"%> <%makefileParams.includes ; separator=" "%> /I"$(OMHOME)/include/omc/c" /I"$(OMHOME)/include/omc" /I. <%match sopt case SOME(s as SIMULATION_SETTINGS(__)) then '<%s.cflags%> ' /* From the simulate() command */%><%if (Flags.isSet(Flags.HPCOM)) then '/openmp'%>

  # /LIBPATH: - directories the linker searches for the .lib import/static libs
  # SimulationRuntimeC.dll re-exports the whole MetaModelica runtime (it absorbs
  # the static OpenModelicaRuntimeC via /WHOLEARCHIVE), so the model links only
  # its import lib - linking OpenModelicaRuntimeC.lib too would give the model a
  # second, divergent copy of threadData / the GC roots.
  LDFLAGS=/nologo /INCREMENTAL:NO /STACK:16777216 /LIBPATH:"<%libpath%>" /LIBPATH:"$(OMHOME)/bin" <%dirExtra%> <%ParModelicaExpLibs%> <%libsPos1%> <%libsPos2%> SimulationRuntimeC.lib omcgc.lib openblas.lib pthreadVC3.lib ws2_32.lib user32.lib
  ...
  # omc regenerates every source before invoking nmake (with /a), so no explicit
  # dependency tracking is needed - a single cl invocation compiles and links.
  omc_main_target: $(MAINFILE) $(CFILES)
  <%\t%>$(CC) $(CFLAGS) /Fe"<%fileNamePrefix%>$(EXEEXT)" $(MAINFILE) $(CFILES) /link $(LDFLAGS)

  omc_dll_target: $(MAINFILE) $(CFILES)
  <%\t%>$(CC) $(CFLAGS) /LD /Fe"<%fileNamePrefix%>$(DLLEXT)" $(MAINFILE) $(CFILES) /link $(LDFLAGS)
