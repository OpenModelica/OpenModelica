...
algorithm
  ...
  daePurity := getFunctionRestrictionPurity(SCodeUtil.getFunctionRestrictionPurity(fres), inheritedComment);
  ...
  attr := matchcontinue fres
    ...
    case SCode.FR_KERNEL_FUNCTION()
      ...
      algorithm
        ...
        if daePurity == DAE.Purity.UNDEFINED and Config.languageStandardAtLeast(Config.LanguageStandard.'3.3') then
          daePurity := if SCodeUtil.isExternalFunctionRestriction(fres) then DAE.Purity.IMPURE else DAE.Purity.PURE;
        end if;
      then
...
algorithm
  ...
  if outPurity <> DAE.Purity.PURE and
     (SCodeUtil.commentHasBooleanNamedAnnotation(cmt, "__ModelicaAssociation_Impure") or
      SCodeUtil.commentHasBooleanNamedAnnotation(cmt, "__OpenModelica_Impure")) then
