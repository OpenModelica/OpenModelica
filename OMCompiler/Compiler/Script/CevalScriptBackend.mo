...
protected function getTotalProgramNF
  "Returns the loaded program reduced to the given class and what it uses. The
   class is kept whole, and it and every class in it are instantiated with the
   new frontend to find what they use. That also works for packages, and for
   classes that fail to instantiate everything used up to the failure is kept."
  input Absyn.Path classPath;
...
algorithm
  ...
  // Only the used elements are wanted from the instantiations, not their messages.
  ErrorExt.setCheckpoint(getInstanceName());
  NFInst.startRecordingUsedClasses();
  // The new frontend needs its own builtin classes, also with -d=nonewInst.
  nf_inst := FlagsUtil.set(Flags.SCODE_INST, true);

  for path in classPath :: getNestedClassPaths(cls, classPath) loop
    try
      runFrontEndWorkNF(path, relaxedFrontend = true);
    else
    end try;
  end for;

  FlagsUtil.set(Flags.SCODE_INST, nf_inst);
  used := NFInst.stopRecordingUsedClasses();
  ErrorExt.rollBack(getInstanceName());
  markElementUsed(cls, used);

  // Instantiation may have loaded libraries, so fetch the program afterwards.
...
algorithm
  UnorderedSet.add(NFInst.usedElementKey(element), used);
...
algorithm
  for e in elements loop
    if SCodeUtil.elementIsClass(e) then
      if UnorderedSet.contains(NFInst.usedElementKey(e), used) then
...
algorithm
  try
    unused := not UnorderedSet.contains(NFInst.usedElementKey(
