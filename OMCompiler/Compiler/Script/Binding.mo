...
algorithm
  model_def := ProgramUtil.getPathedClassInProgram(model_path, env);
  ...
  out_model_def := ProgramUtil.updateProgram(Absyn.PROGRAM({out_vmodel}, ProgramUtil.buildWithin(model_path)), env);
end inferBindings;
...
algorithm
  ...
  // get mediators
  ms := getMediatorDefsElements(scode_def, {});
  package_def := ProgramUtil.getPathedClassInProgram(package_path, in_env);
    autogen_model_list := {};
    i:=0;
   ...
   out_env  := ProgramUtil.updateProgram(Absyn.PROGRAM({out_vmodel}, ProgramUtil.buildWithin(package_path)), in_env);
...
algorithm
  out_class := match in_class
    ...
    case out_class as Absyn.CLASS(body=Absyn.PARTS(typeVars, classAttrs, _, ann, comment))
      algorithm
        out_class.body := Absyn.PARTS(typeVars, classAttrs, {Absyn.PUBLIC(ag_elems)}, ann, comment);
      then
        out_class;
  end match;
end updatePackage;
...
public function updateClass
  ...
  output Absyn.Class  out_class;
algorithm
  out_class as Absyn.CLASS() := in_class;
  out_class.body := parseClassDef(out_class.body, defs, typeSpec, rootType, exp, instance_name, hasPreferred, preferred, path + out_class.name + ".");
end updateClass;
...
algorithm
  out_elems := matchcontinue in_elems
   ...
   list<Absyn.ElementItem> rest, e_list;
   list<Absyn.ComponentItem> components;
   TypeSpec tSpec;
   Boolean newName;
   case {}
...
algorithm
  ...
  local
    ...
    case name::rest then Absyn.MODIFICATION(false, Absyn.NON_EACH(),
          AbsynUtil.stringPath(name), SOME(Absyn.CLASSMOD({}, Absyn.EQMOD(exp, Absyn.dummyInfo))), NONE(), Absyn.dummyInfo)::buildComponentModifiers2(rest, exp);
        end match;
...
algorithm
  out_eq := match in_eq
    local
      ...
      Absyn.Operator op;
      list<tuple<Absyn.Exp, Absyn.Exp>> elif;
...
output list<tuple<Absyn.Exp, String>>  out_es;
 algorithm
   ...
   local
     ...
     Absyn.Exp exp;
    case {} then in_es;
    case PROVIDER(className, _, template)::rest // TODO fix handling instance
      algorithm
    ...
    case Absyn.ELEMENTITEM(Absyn.ELEMENT(_,_,_, Absyn.COMPONENTS(_,typeSpec, components), _, _))::rest
      algorithm
         ...
         // if Interactive.isPrimitive(path, env) then fail(); end if;
          def := ProgramUtil.getPathedClassInProgram(path,env); // load the element
...
algorithm
  ...
  local
    ...
    String template, mType, str1, str2;
    list<Client> cls;
    list<Provider> prvs;
    list<Preferred> pref;
