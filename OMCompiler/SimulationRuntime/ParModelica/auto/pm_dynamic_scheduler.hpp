...
public:
    TaskLauncher(long id, FunctionType** f, void** d) :
      task_id(id)
      , function_systems(f)
      , data(d)
    {}
...
public:
    ...
    DynamicScheduler(TaskSystem<TaskTypeT>& task_system) :
        ...
        for ( ; vert_iter != vert_end; ++vert_iter) {
            ...
            flow::continue_node<flow::continue_msg>* curr_f_node =
                    new flow::continue_node<flow::continue_msg>(dynamic_graph,
                        TaskLauncher<TaskType>(curr_b_node.node_id, &function_systems, &data));
