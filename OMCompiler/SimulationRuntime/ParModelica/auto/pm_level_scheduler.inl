...
void LevelSchedulerThreadOblivious<TaskTypeT>::execute() {
    ...
    for( ; level_iter != levels.end(); ++level_iter) {
        ...
        if(level_iter->level_cost > 0.1) {
            tbb::parallel_for(
                tbb::blocked_range<std::vector<long>::const_iterator>(
                l_task_ids.begin(), l_task_ids.end())
                , level_executor);
        }
        else {
...
void LevelSchedulerThreadAware<TaskTypeT>::schedule(int number_of_processors) {
    ...
    for(level_iter = this->levels.begin() + 1; level_iter != this->levels.end(); ++level_iter, ++level_number) {
        ...
        while(current_level.size() != 0) {
            // utility::test_log("Info") << "vertices remaining. " << current_level.size() << " " << system_graph[current_level.front()].index << newl;
            pqueue_iter = std::min_element(current_group.begin(), current_group.end(),
                                TaskQueueType::cost_comparator);
        ...
        current_group.parallel_cost = std::max_element(current_group.begin(), current_group.end(),
                                    TaskQueueType::cost_comparator)->total_cost;
        ...
        pqueue_iter = std::find_if(current_group.begin(), current_group.end(),
                            std::mem_fun_ref(&TaskQueueType::empty));
...
void LevelSchedulerThreadAware<TaskTypeT>::execute() {
    ...
    for( ; pq_level_iter != processor_queue_levels.end(); ++pq_level_iter) {
        ...
        if(current_group.parallel_cost > 0.05) {
            tbb::parallel_for(
                tbb::blocked_range<typename ConcurrentQueuesType::const_iterator>(
