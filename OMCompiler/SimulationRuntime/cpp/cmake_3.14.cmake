...
elseif(APPLE OR CMAKE_VERSION VERSION_LESS "3.30")
 ...
 find_package(Boost COMPONENTS program_options filesystem REQUIRED)
  ...
  # In CONFIG mode, BoostConfig.cmake does not set Boost_INCLUDE_DIR / Boost_LIBRARY_DIRS.
  # Extract them from the imported targets.
  if(TARGET Boost::headers)
    get_target_property(_boost_inc Boost::headers INTERFACE_INCLUDE_DIRECTORIES)
    if(_boost_inc AND NOT _boost_inc STREQUAL "_boost_inc-NOTFOUND")
      list(GET _boost_inc 0 Boost_INCLUDE_DIR)
    endif()
  elseif(TARGET Boost::boost)
    get_target_property(_boost_inc Boost::boost INTERFACE_INCLUDE_DIRECTORIES)
    if(_boost_inc AND NOT _boost_inc STREQUAL "_boost_inc-NOTFOUND")
      list(GET _boost_inc 0 Boost_INCLUDE_DIR)
    endif()
  endif()
  ...
  if(NOT Boost_LIBRARY_DIRS)
    foreach(_comp IN ITEMS filesystem program_options)
      if(TARGET Boost::${_comp})
        get_target_property(_loc Boost::${_comp} IMPORTED_LOCATION_RELEASE)
        if(NOT _loc OR _loc STREQUAL "_loc-NOTFOUND")
          get_target_property(_loc Boost::${_comp} IMPORTED_LOCATION)
        endif()
        if(NOT _loc OR _loc STREQUAL "_loc-NOTFOUND")
          get_target_property(_loc Boost::${_comp} IMPORTED_LOCATION_DEBUG)
        endif()
        if(_loc AND NOT _loc STREQUAL "_loc-NOTFOUND")
          get_filename_component(Boost_LIBRARY_DIRS "${_loc}" DIRECTORY)
          break()
        endif()
      endif()
    endforeach()
  endif()
  if(Boost_LIBRARY_DIRS AND NOT Boost_LIBRARY_DIR)
    set(Boost_LIBRARY_DIR "${Boost_LIBRARY_DIRS}")
  endif()
  if(Boost_LIBRARY_DIRS AND NOT Boost_Library_folder)
