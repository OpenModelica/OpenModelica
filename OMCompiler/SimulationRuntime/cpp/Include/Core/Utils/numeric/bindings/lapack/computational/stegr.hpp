...
//
// The LAPACK-backend for stegr is the netlib-compatible backend.
//
...
struct stegr_impl< Value, typename boost::enable_if< is_real< Value > >::type > {
...
//
template< typename Value >
struct stegr_impl< Value, typename boost::enable_if< is_complex< Value > >::type > {
...
// Calls to these functions are passed to the stegr_impl classes. In the
