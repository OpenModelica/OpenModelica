...
public:
  ...
  virtual shared_ptr<IAlgLoopSolverFactory> createAlgLoopSolverFactory(IGlobalSettings* globalSettings)
  {
    ...
    std::map<std::string, factory<IAlgLoopSolverFactory,IGlobalSettings*,PATH,PATH> >& algloopsolver_factory(_simobject_type_map->get());
  ...
  {
    std::map<std::string, factory<IHistory,IGlobalSettings&,size_t > >::iterator writer_iter;
    std::map<std::string, factory<IHistory,IGlobalSettings&,size_t > >& writer_factory(_simobject_type_map->get());
