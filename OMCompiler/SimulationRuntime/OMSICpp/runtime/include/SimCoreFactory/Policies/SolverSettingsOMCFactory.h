...
public:
                                                             ...
                                                             shared_ptr<IGlobalSettings> globalSettings)
    {
        ...
        else if (solvername.compare("euler") == 0)
        {
            ...
            LOADERRESULT result = ObjectFactory<CreationPolicy>::_factory->LoadLibrary(
                euler_path.string(), *_solver_type_map);
            if (result != LOADER_SUCCESS)
            {
        ...
        else if (solvername.compare("rk12") == 0)
        {
            ...
            rk12_path /= rk12_name;
            LOADERRESULT result = ObjectFactory<CreationPolicy>::_factory->LoadLibrary(
                rk12_name.string(), *_solver_type_map);
            if (result != LOADER_SUCCESS)
            {
        ...
        else if (solvername.compare("peer") == 0)
        {
            ...
            peer_path /= peer_name;
            LOADERRESULT result = ObjectFactory<CreationPolicy>::_factory->LoadLibrary(
                peer_name.string(), *_solver_type_map);
            if (result != LOADER_SUCCESS)
            {
        ...
        else if (solvername.compare("rtrk") == 0)
        {
            ...
            rtrk_path /= rtrk_name;
            LOADERRESULT result = ObjectFactory<CreationPolicy>::_factory->LoadLibrary(
                rtrk_path.string(), *_solver_type_map);
            if (result != LOADER_SUCCESS)
            {
        ...
        else if (solvername.compare("RTEuler") == 0)
        {
            ...
            LOADERRESULT result = ObjectFactory<CreationPolicy>::_factory->LoadLibrary(
                RTEuler_path.string(), *_solver_type_map);
            if (result != LOADER_SUCCESS)
            {
        ...
        else if (solvername.compare("ida") == 0)
        {
            ...
            ida_path /= ida_name;
            LOADERRESULT result = ObjectFactory<CreationPolicy>::_factory->LoadLibrary(
                ida_name.string(), *_solver_type_map);
            if (result != LOADER_SUCCESS)
            {
        ...
        else if ((solvername.compare("cvode") == 0) || (solvername.compare("dassl") == 0))
        {
            ...
            LOADERRESULT result = ObjectFactory<CreationPolicy>::_factory->LoadLibrary(
                cvode_path.string(), *_solver_type_map);
            if (result != LOADER_SUCCESS)
            {
        ...
        if (iter == factories.end())
        {
            ...
            std::cerr << "Available solverfactories:" << std::endl;
            for (std::map<std::string, factory<ISolverSettings, IGlobalSettings*>>::iterator iter = factories.begin();
            ...
            throw ModelicaSimulationError(MODEL_FACTORY,
                                          "No such Solver " + solvername + ". Available solver factories:" +
                                          factoryStr);
        }
        shared_ptr<ISolverSettings> solver_settings = shared_ptr<ISolverSettings>(
            iter->second.create(globalSettings.get()));
