...
SimSettings OMCFactory::readSimulationParameter(int argc, const char* argv[])
{
    ...
    desc.add_options()
        ...
        ("nls-continue", po::bool_switch()->default_value(false),
         "non linear solver will continue if it can not reach the given precision")
        ...
        ("input-path", po::value<string>(),
         "directory with input files, like init xml (defaults to modelica-system-library)")
        ("output-path", po::value<string>(),
         "directory for output files, like results (defaults to modelica-system-library)")
        ...
        ("log-settings,V", po::value<vector<string>>(),
         "log information: init, nls, ls, solver, output, events, model, other")
        ...
        ("output-type,O", po::value<string>()->default_value("all"),
         "the points in time written to result file: all (output steps + events), step (just output points), none")
        ("output-format,P", po::value<string>()->default_value("mat"),
         "simulation results output format: csv, mat, buffer, empty")
        ...
        ("init-file,i", po::value<string>()->default_value("public"),
         "path for osu or fmu 2.0 ME initialisation xml-file");
    ...
    po::variables_map vm;
    boost::function < pair<string, string>(const string &) > parserFunction(
        boost::bind(&OMCFactory::parseIngoredAndWrongFormatOption, this, _1));
    ...
    if (vm.count("help"))
    {
        cout << desc << endl;
        throw ModelicaSimulationError(MODEL_FACTORY,
                                      "Cannot parse command line arguments correctly, because the help message was requested.",
                                      "", true);
    }
    ...
    SimSettings settings = {
        solver, linSolver, nonLinSolver, starttime, stoptime, stepsize, 1e-24, 0.01, tolerance, resultsfilename,
        timeOut, outputPointType, logSettings, nlsContinueOnError, solverThreads, outputFormat, emitResults, osu_name,
        osu_path, init_file_path, inputPath, outputPath
    };
...
shared_ptr<ISimController> OMCFactory::loadSimControllerLib(PATH simcontroller_path, type_map simcontroller_type_map)
{
    ...
    if (result != LOADER_SUCCESS)
        throw ModelicaSimulationError(MODEL_FACTORY,
                                      string("Failed loading SimConroller library!") + simcontroller_path);
