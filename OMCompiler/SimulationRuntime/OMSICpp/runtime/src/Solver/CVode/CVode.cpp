...
void Cvode::initialize()
{
    ...
    else
    {
        ...
        // Set Tolerances
        _idid = CVodeSVtolerances(_cvodeMem, dynamic_cast<ISolverSettings*>(_cvodesettings)->getRTol(), _CV_absTol);
        // RTOL and ATOL
        if (_idid < 0)
            throw ModelicaSimulationError(SOLVER, "CVode::initialize()");
        ...
        _idid = CVodeSetMinStep(_cvodeMem, dynamic_cast<ISolverSettings*>(_cvodesettings)->getLowerLimit());
        // MINIMUM STEPSIZE
        if (_idid < 0)
            throw ModelicaSimulationError(SOLVER, "CVode::initialize()");
    ...
    while ((_solverStatus & ISolver::CONTINUE) && !_interrupt)
    {
        ...
        if (writeOutput)
        {
            ...
            catch (std::exception& ex)
            {
                if (!(_cv_rt == CV_ROOT_RETURN))
                    // if a zero crossing was dected before the event iteration was called and writeoutput throws and error
        ...
        if ((_cv_rt == CV_ROOT_RETURN) && !isInterrupted())
        {
            ...
            else
            {
                ...
                throw ModelicaSimulationError(EVENT_HANDLING,
                                              "Number of events of zero function(s) " + zeros.str() +
                                              "exceeded in time interval " + to_string(_abs) + " at time " + to_string(
                                                  _tCurrent));
            }
        ...
        if ((_zeroFound || state_selection) && !isInterrupted())
        {
            ...
            // If event time is in endtime (time-event) stop solver. Otherwise CVODE throws warning
            if (_tCurrent == _tEnd)
        ...
        // Increase counter for output steps
        ++_outStps;
...
void Cvode::writeCVodeOutput(const double& time, const double& h, const int& stp)
{
    ...
    if (stp > 0)
    {
        if (_cvodesettings->getDenseOutput())
        {
            ...
            while (_tLastWrite + dynamic_cast<ISolverSettings*>(_cvodesettings)->getGlobalSettings()->gethOutput() <=
                time)
            {
                if (!_bWritten)
                {
                ...
                _bWritten = true;
                _tLastWrite = _tLastWrite + dynamic_cast<ISolverSettings*>(_cvodesettings)
...
int Cvode::CV_JCallback(long int N, double t, N_Vector y, N_Vector fy, DlsMat Jac, void* user_data, N_Vector tmp1,
                        N_Vector tmp2, N_Vector tmp3)
{
    return ((Cvode*)user_data)->calcJacobian(t, N, tmp1, tmp2, tmp3, NV_DATA_S(y), fy, Jac);
}

int Cvode::calcJacobian(double t, long int N, N_Vector fHelp, N_Vector errorWeight, N_Vector jthCol, double* y,
                        N_Vector fy, DlsMat Jac)
{
    try
    {
        ...
        minInc = (fnorm != 0.0) ? (1000.0 * abs(h) * UROUND * N * fnorm) : 1.0;
...
void Cvode::writeSimulationInfo()
{
    ...
    flag = CVodeGetIntegratorStats(_cvodeMem, &nst, &nfe, &nsetups, &netf, &qlast, &qcur, &h0u, &hlast, &hcur,
                                   &tcur);
    ...
    LOGGER_WRITE("Cvode: number of evaluateODE calls 'eODE' = " + to_string(_numberOfOdeEvaluations), LC_SOLVER,
                 LL_INFO);
