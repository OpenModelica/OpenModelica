...
void Ida::initialize()
{
    ...
    _idaMem = IDACreate(_sunctx);
        ...
        /* IDASetErrHandlerFn is gone since SUNDIALS 7; the error handler is pushed
           onto the SUNContext instead. */
        _idid = SUNContext_PushErrHandler(_sunctx, errOutputIDA, _data);
        if (_idid < 0)
            throw std::invalid_argument("IDA::initialize()");
        // Set Tolerances
        _idid = IDASVtolerances(_idaMem, dynamic_cast<ISolverSettings*>(_idasettings)->getRTol(), _CV_absTol);
        // RTOL and ATOL
        if (_idid < 0)
            throw std::invalid_argument("IDA::initialize()");
        ...
        if ((_cv_rt == IDA_ROOT_RETURN) && !isInterrupted())
        {
            ...
            else if ((_abs >= 1e-3))
            {
                ...
                throw std::runtime_error(
                    "Number of events exceeded  in time interval " + to_string(_abs) + " at time " + to_string(
                        _tCurrent));
    ...
    if (stp > 0)
    {
        if (_idasettings->getDenseOutput())
        {
            ...
            )
            {
                ...
                _bWritten = true;
                _tLastWrite = _tLastWrite + dynamic_cast<ISolverSettings*>(_idasettings)
