...
ublas::vector<double> Reduction::getError(ublas::matrix<double>& R, ublas::matrix<double>& R2,
...
std::vector<unsigned int> Reduction::cancelTerms(label_list_type& labels, ublas::matrix<double>& Ro,
                                                 ...
                                                 , SimSettings simsettings, string modelKey,
                                                 vector<string> output_names, double timeout,
        ...
        FOREACH(label_type label, labels)
        {
                ...
                *(get < 2 > (label)) = 1;
...
#endif
                std::cout << " time of simulation for reducing label " << get < 0 > (label) << " is " << simtime <<
                    " seconds" << std::endl;
                ...
                else
                {
                    ...
                    *(get < 2 > (label)) = 0;
                    ...
                    {
                        cout << "Redution stoped at step " << reductionStep + 1 <<
                            " because of exceeding max number of reduction fails" << std::endl;
            ...
            catch (ModelicaSimulationError& ex)
            {
                ...
                *(get < 2 > (label)) = 0;
                ...
                if ((nfail) > _settings->getNFail())
                {
                    cout << "Redution failed for " << nfail << " times. So, it stoped at step " << reductionStep + 1 <<
                        std::endl;
                    //stop looking for terms to delete
                    break;
                }
...
#endif
        std::cout << " time of reduction: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).
...
bool Reduction::isLess(ublas::vector<double>& v1, ublas::vector<double>& v2, vector<int> indexes,
                       vector<string> output_names)
{
    //cout << " v1 "<<v1<<std::endl;
    ...
    {
        cout << indexes[i] << " error variable " << output_names[indexes[i]] << " with error bound " << v2[i] << " is "
            << v1[indexes[i]] << std::endl;
