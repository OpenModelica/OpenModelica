...
weak_ptr<IMixedSystem> SimController::LoadSystem(string modelLib, string modelKey)
{
    ...
    if (iter != _systems.end())
    {
        ...
        shared_ptr<IExtendedSimObjects> extended_simObjects = dynamic_pointer_cast<IExtendedSimObjects>(simObjects);

        if (extended_simObjects)
            extended_simObjects->eraseSimData(modelKey);
...
weak_ptr<IMixedSystem> SimController::LoadOSUSystem(string osu_name, string osu_key)
{
    ...
    if (iter != _systems.end())
    {
        ...
        if (extended_simObjects)
            extended_simObjects->eraseSimData(osu_key);
        else
        {
            string error = string("Simulation data was not found for model: ") + osu_key;
            throw ModelicaSimulationError(SIMMANAGER, error);
        }
    ...
    else
    {
        try
        {
            ...
            if (global_settings->getOutputFormat() == BUFFER)
            {
                ...
                shared_ptr<IExtendedSimObjects> extended_simObjects = dynamic_pointer_cast<IExtendedSimObjects>(_sim_objects);

                if (!extended_simObjects)
                {
                    string error = string("Simulation data was not found for model: ") + simsettings.outputfile_name;
                    throw ModelicaSimulationError(SIMMANAGER, error);
                }
