...
MeasureTimeValuesSolver::MeasureTimeValuesSolver(unsigned long long functionEvaluations,
...
MeasureTimeValuesSolver::MeasureTimeValuesSolver(const MeasureTimeValuesSolver& timeValues) :
    MeasureTimeValues(), _functionEvaluations(timeValues._functionEvaluations),
    _errorTestFailures(timeValues._errorTestFailures)
{
}
...
MeasureTimeData::MeasureTimeData(const MeasureTimeData& data): _id(data._id),
                                                               _sumMeasuredValues(data._sumMeasuredValues->clone())
{
}
...
void MeasureTime::addResultContentBlock(std::string modelName, std::string blockName,
                                        std::vector<MeasureTimeData*>* data)
{
...
void MeasureTime::writeToJson()
{
    ...
    date << date_t->tm_year + 1900 << "-" << date_t->tm_mon + 1 << "-" << date_t->tm_mday << " " << date_t->tm_hour <<
        ":" << date_t->tm_min << ":" << date_t->tm_sec;
    ...
    for (file_map::iterator model = _valuesToWrite.begin(); model != _valuesToWrite.end(); ++model) // iterate files
    {
        ...
        for (block_map::iterator block = model->second.begin(); block != model->second.end(); ++block) // iterate blocks
        {
            ...
            for (unsigned i = 0; i < (data->size() == 0 ? 1 : data->size()) - 1; ++i)
            {
                if ((*data)[i] == NULL)
                {
                    LOGGER_WRITE("Skipped a measured block in '" + block->first + "' because it is null.", LC_OUTPUT,
                                 LL_ERROR);
                    continue;
                }
            ...
            if (data->size() > 0) os << "{\"id\":\"" << (*data)[data->size() - 1]->_id << "\"," << (*data)[data->size()
                - 1]->serializeToJson() << "}]";
            else os << "]";
        } // end blocks
