        ...
        else //time > delay time
        {
            ...
            else
            {
                ...
                buffer_type::iterator pos = find_if(_time_buffer.begin(), _time_buffer.end(),
                                                    bind2nd(std::greater_equal<double>(), ts));
...
void SystemDefaultImplementation::setRealStartValue(BaseArray<double>& avar, const BaseArray<double>& aval,
...
void SystemDefaultImplementation::setBoolStartValue(BaseArray<bool>& avar, const BaseArray<bool>& aval,
                                                    bool overwriteOldValue)
{
    bool* varp = avar.getData();
...
void SystemDefaultImplementation::setIntStartValue(BaseArray<int>& avar, const BaseArray<int>& aval,
                                                   bool overwriteOldValue)
{
    int* varp = avar.getData();
...
void SystemDefaultImplementation::setStringStartValue(BaseArray<string>& avar, const BaseArray<string>& aval,
                                                      bool overwriteOldValue)
{
    string* varp = avar.getData();
...
double SystemDefaultImplementation::computeNextTimeEvents(double currTime, std::pair<double, double>* timeEventPairs)
{
    ...
    for (int timerIdx = 0; timerIdx < _dimTimeEvent; timerIdx++)
    {
        ...
        {
            pastIntervalls = std::floor(
                (currTime - timeEventPairs[timerIdx].first + 1e4 * UROUND) / timeEventPairs[timerIdx].second);
            _currTimeEvents[timerIdx] = timeEventPairs[timerIdx].first + (pastIntervalls) * timeEventPairs[timerIdx].
                second;
