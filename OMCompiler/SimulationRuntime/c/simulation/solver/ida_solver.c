                       ...
                       SOLVER_INFO* solverInfo, IDA_SOLVER* idaData)
{
  ...
  idaData->errwgt = N_VNew_Serial(idaData->N, idaData->sunctx);
  ...
  flag = IDASVtolerances(idaData->ida_mem, data->simulationInfo->tolerance,
                         N_VMake_Serial(idaData->N, tmp, idaData->sunctx));
  ...
  if (idaData->daeMode) {
    ...
    flag = IDASetId(idaData->ida_mem, N_VMake_Serial(idaData->N, tmp, idaData->sunctx));
