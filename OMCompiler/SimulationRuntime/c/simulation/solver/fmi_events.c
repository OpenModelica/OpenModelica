...
modelica_boolean checkForDiscreteChanges(DATA *data, threadData_t *threadData)
{
  ...
  if (OMC_ACTIVE_STREAM(OMC_LOG_EVENTS_V)) {
    ...
    infoStreamPrint(OMC_LOG_EVENTS_V, 1, "check for discrete changes at time=%.12g", data->localData[0]->timeValue);
    ...
    for (i=realStartIndex; i<realStopIndex; i++) {
      if(strncmp(modelData->realVarsData[i].info.name, "$cse", 4)) {
        ...
        if (v1 != v2) {
          infoStreamPrint(OMC_LOG_EVENTS_V, 0, "discrete var changed: %s from %g to %g", modelData->realVarsData[i].info.name, v1, v2);
          needToIterate = TRUE;
        }
      }
    }
    for (i=0; i<modelData->nVariablesInteger; i++) {
      if(strncmp(modelData->integerVarsData[i].info.name, "$cse", 4)) {
        ...
        if (v1 != v2) {
          infoStreamPrint(OMC_LOG_EVENTS_V, 0, "discrete var changed: %s from %ld to %ld", modelData->integerVarsData[i].info.name, (long) v1, (long) v2);
          needToIterate = TRUE;
        }
      }
    }
    for (i=0; i<modelData->nVariablesBoolean; i++) {
      if(strncmp(modelData->booleanVarsData[i].info.name, "$cse", 4)) {
        ...
        if (v1 != v2) {
          infoStreamPrint(OMC_LOG_EVENTS_V, 0, "discrete var changed: %s from %d to %d", modelData->booleanVarsData[i].info.name, v1, v2);
          needToIterate = TRUE;
        }
      }
    }
    for (i=0; i<modelData->nVariablesString; i++) {
      if(strncmp(modelData->stringVarsData[i].info.name, "$cse", 4)) {
        ...
        if (0 != strcmp(MMC_STRINGDATA(v1),MMC_STRINGDATA(v2))) {
          infoStreamPrint(OMC_LOG_EVENTS_V, 0, "discrete var changed: %s from %s to %s", modelData->stringVarsData[i].info.name, MMC_STRINGDATA(v1), MMC_STRINGDATA(v2));
          needToIterate = TRUE;
        }
      }
    }
    if (OMC_ACTIVE_STREAM(OMC_LOG_EVENTS_V)) messageClose(OMC_LOG_EVENTS_V);
