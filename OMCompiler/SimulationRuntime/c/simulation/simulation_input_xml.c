...
#define READ_VARIABLES(out, in, attributeKind, read_var_attribute, debugName, start, nStates, mapAlias) \
  ...
  { \
    ...
    setFilterOuput(v, out[j], info->name, endsWith(info->info.filename, ".moc")); \
  ...
  for(i=0; i<modelData->nAliasReal; i++)
  {
    ...
    setFilterOuput(*findHashLongVar(mi.rAli,i), modelData->realAlias[i], modelData->realAlias[i].info.name, endsWith(modelData->realAlias[i].info.info.filename, ".moc"));
  ...
  for(i=0; i<modelData->nAliasInteger; i++)
  {
    ...
    setFilterOuput(*findHashLongVar(mi.iAli,i), modelData->integerAlias[i], modelData->integerAlias[i].info.name, endsWith(modelData->integerAlias[i].info.info.filename, ".moc"));
  ...
  for(i=0; i<modelData->nAliasBoolean; i++)
  {
    ...
    setFilterOuput(*findHashLongVar(mi.bAli,i), modelData->booleanAlias[i], modelData->booleanAlias[i].info.name, endsWith(modelData->booleanAlias[i].info.info.filename, ".moc"));
  ...
  for(i=0; i<modelData->nAliasString; i++)
  {
    ...
    setFilterOuput(*findHashLongVar(mi.sAli,i), modelData->stringAlias[i], modelData->stringAlias[i].info.name, endsWith(modelData->stringAlias[i].info.info.filename, ".moc"));
