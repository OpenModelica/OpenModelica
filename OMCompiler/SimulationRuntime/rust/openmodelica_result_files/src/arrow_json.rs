//! Reader for the pre-`arrow.modelica` layout `openmodelica_arrow_writer::json`
//! writes (one IPC file, the variable table as JSON in the schema metadata).
//! Never released; kept behind the `json-layout` feature for the result-format
//! benchmark only. The record batches are decoded once into per-field `f64`
//! columns and the variable table becomes the `.mat`-shaped `MatVariable`
//! list, so everything built on [`ResultTable`] reads it like the others. An
...
impl ResultTable for ArrowJsonReader {
...
mod tests {
    use super::*;
    use openmodelica_arrow_writer::json::{ArrowStream, write_arrow};
    use openmodelica_arrow_writer::{Affine, ArrowKind, ArrowVar, ColTy, FileMeta, VarTy, no_strings};
    ...
    fn a_dictionary_encoded_enumeration_reads_the_same() {
        ...
        drop(w);
        let mut r = ArrowJsonReader::from_bytes(out).expect("readable");
    ...
    fn a_file_without_footer_reads_up_to_the_last_block() {
        ...
        assert_eq!(ArrowJsonReader::from_bytes(out).expect("truncated file").nrows, 3);
    }
}
