...
//! ring buffer per `delay(...)` expression.
//!
//! Only the buffers, the interpolation and the event search; allocating the
//! state and reporting a model error are the runtime's business.
...
use crate::omclog;
use crate::{format_e, format_g};

static THROW_HOOK: AtomicUsize = AtomicUsize::new(0);

/// Install C's `throwStreamPrint` for a delay evaluation. Each runtime reports a
/// model error its own way -- the in-wasm one traps, the native one longjmps out
/// of the model -- so the buffers only say what happened. Unset, the message goes
/// to the log and the evaluation carries on with the caller's value.
...
fn throw_stream(s: &str) {
    ...
    if p == 0 {
        omclog::error(omclog::STDOUT, false, s);
        return;
    }
...
impl DelayState {
    pub fn new(n: usize, start_time: f64) -> Self {
    ...
    /// that boundary).
    pub fn store(&mut self, idx: usize, time: f64, value: f64, delay_time: f64) {
    ...
    pub fn zc(&self, idx: usize, time: f64, delay_time: f64, zc_pre: f64) -> f64 {
        let buf = &self.buffers[idx];
        if buf.is_empty() {
            return zc_pre;
        }
