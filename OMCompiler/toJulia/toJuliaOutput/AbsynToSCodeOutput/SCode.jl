...
function algorithmEqual2(ai1::Statement, ai2::Statement)::Bool
  ...
  equal = begin
    ...
    @matchcontinue (ai1, ai2) begin
      ...
      (ALG_ASSIGN(assignComponent = e11, value = e12), ALG_ASSIGN(assignComponent = Absyn.TUPLE(_), value = e22))  => begin
        @assert isa(e11, Absyn.TUPLE)
        @assert isa(e21, Absyn.TUPLE)
      ...
      (a1, a2)  => begin
        alg1 = statementToAlgorithmItem(a1)
        alg2 = statementToAlgorithmItem(a2)
        equality(alg1, alg2)
        true
      end
...
function findIteratorIndexedCrefsInEEquation(inEq::EEquation, inIterator::String, inCrefs::Lst = list())::Lst
  local outCrefs::Lst
  f = (inIterator) -> AbsynUtil.findIteratorIndexedCrefs(inIterator)
  outCrefs = SCode.foldEEquationsExps(inEq, f, inCrefs)
  outCrefs
end
...
function findIteratorIndexedCrefsInStatement(inStatement::Statement, inIterator::String, inCrefs::Lst = list())::Lst
  local outCrefs::Lst
  f = (inIterator) -> AbsynUtil.findIteratorIndexedCrefs(inIterator)
  outCrefs = SCode.foldStatementsExps(inStatement, f, inCrefs)
  outCrefs
end
...
function argument. =#
function traverseNamedArgExps(inArg::Absyn.NamedArg, inTuple::Tuple{TraverseFunc, Absyn.NamedArg})::Tuple{TraverseFunc, Absyn.NamedArg}
  local outTuple::Tuple
...
function lookupNamedAnnotation(ann::Annotation, name::String)::Mod
  ...
  mod = begin
    @match ann begin
      Annotation.ANNOTATION(Mod.MOD(subModLst))  => begin
        for sm in subModLst
          (id, mod) =  @match sm begin
            SubMod.NAMEMOD(id, mod) => (id, mod)
            _ => failure()
          end
          if id == name
            return
...
function hasBooleanNamedAnnotationInClass(inClass::Element, namedAnnotation::String)::Bool
  ...
  hasAnn = begin
    ...
    @match (inClass, namedAnnotation) begin
      (CLASS(COMMENT(SOME(ann))), _)  => begin
...
function hasBooleanNamedAnnotationInComponent(inComponent::Element, namedAnnotation::String)::Bool
  ...
  hasAnn = begin
    ...
    @match (inComponent, namedAnnotation) begin
      (COMPONENT(COMMENT(SOME(ann))), _)  => begin
        hasBooleanNamedAnnotation(ann, namedAnnotation)
      end
      _  => begin
        false
...
function hasBooleanNamedAnnotation(inAnnotation::Annotation, inName::String)::Bool
  ...
  @match ANNOTATION(MOD(subModLst = submods)) = inAnnotation
...
function appendAnnotationToComment(inAnnotation::Annotation, inComment::Comment)::Comment
  ...
  outComment = begin
    ...
    @match (inAnnotation, inComment) begin
      ...
      (ANNOTATION(MOD(mods1)), COMMENT(SOME(ANNOTATION(MOD(fp, ep, mods2, b, info))), cmt))  => begin
