    ...
    FuncType = Function
        ...
        function traverseEquation(inEquation::Equation, inFunc::FuncTplToTpl, inTypeA::TypeA)::Tuple
              ...
              outTpl = begin
                  ...
                  local eqilst1::IList
                  local eqilst2::IList
                  local eqilst_1::IList
                  local eqilst1_1::IList
                  local eqilst2_1::IList
                  local eeqitlst::IList
                  local eeqitlst_1::IList
        ...
        function traverseEquationItemList(inEquationItemList::IList, inFunc::FuncTplToTpl, inTypeA::TypeA)::Tuple
              local outTpl::Tuple
        ...
        function traverseExpEqItemTupleList(inList::IList, inFunc::FuncTplToTpl, inTypeA::TypeA)::Tuple
              local outTpl::Tuple
        ...
        function traverseAlgorithm(inAlgorithm::Algorithm, inFunc::FuncTplToTpl, inTypeA::TypeA)::Tuple
              ...
              outTpl = begin
                  ...
                  local ailst2::IList
                  local ailst_1::IList
                  local ailst1_1::IList
                  local ailst2_1::IList
                  local eaitlst::IList
                  local eaitlst_1::IList
                  local rel::FuncTplToTpl
        ...
        function traverseAlgorithmItemList(inAlgorithmItemList::IList, inFunc::FuncTplToTpl, inTypeA::TypeA)::Tuple
              local outTpl::Tuple
         ...
         #= traverses a list of Exp * AlgorithmItem list tuples
          mostly used for else-if blocks =#
        function traverseExpAlgItemTupleList(inList::IList, inFunc::FuncTplToTpl, inTypeA::TypeA)::Tuple
              local outTpl::Tuple
        ...
        function traverseExpList(inExpList::IList, inFunc::FuncTplToTpl, inArg::Type_a)::Tuple{Type_a, IList}
              local outArg::Type_a
              local outExpList::IList
         ...
         #= Traverses a list of expressions, calling traverseExpBidir on each
          expression. =#
        function traverseExpListBidir(inExpl::IList, enterFunc::FuncType, exitFunc::FuncType, inArg::Argument)::Tuple{Argument, IList}
        ...
        function traverseExpBidirSubExps(inExp::Exp, enterFunc::FuncType, exitFunc::FuncType, inArg::Argument)::Tuple{Argument, Exp}
              ...
              (e, arg) = begin
                  ...
                  local else_ifs1::IList
                  local else_ifs2::IList
                  local expl1::IList
                  local expl2::IList
                  local mat_expl::IList
                  ...
                  local match_decls::IList
                  local match_cases::IList
                  local cmt::Option
                @match (inExp, enterFunc, exitFunc, inArg) begin
                  (INTEGER(), _, _, _)  => begin
                    (inExp, inArg)
        ...
        function traverseExpBidirFunctionArgs(inArgs::FunctionArgs, enterFunc::FuncType, exitFunc::FuncType, inArg::Argument)::Tuple{Argument, FunctionArgs}
              ...
              (outArgs, outArg) = begin
                  ...
                  local expl1::IList
                  local expl2::IList
                  local named_args1::IList
                  local named_args2::IList
        ...
        function traverseExpBidirNamedArg(inArg::NamedArg, enterFunc::FuncType, exitFunc::FuncType, inExtra::Argument)::Tuple{Argument, NamedArg}
              ...
              #= This is crazy code =#NAMEDARG(name, value1) = inArg
        ...
        function traverseExpBidirIterator(inIterator::ForIterator, enterFunc::FuncType, exitFunc::FuncType, inArg::Argument)::Tuple{Argument, ForIterator}
              ...
              #= This is crazy code =#ITERATOR(name = name, guardExp = guardExp1, range = range1) = inIterator
        ...
        function traverseEquationItemListBidir(inEquationItems::IList, enterFunc::FuncType, exitFunc::FuncType, inArg::Argument)::Tuple{Argument, IList}
              local outArg::Argument
              local outEquationItems::IList
        ...
        function traverseAlgorithmItemListBidir(inAlgs::IList, enterFunc::FuncType, exitFunc::FuncType, inArg::Argument)::Tuple{Argument, IList}
              local outArg::Argument
              local outAlgs::IList
        ...
        function className(cl::Class)::Path
              ...
              #= This is crazy code =#CLASS(name = id) = cl
              name = IDENT(id)
          name
        end
        ...
        function expString(exp::Exp)::String
              ...
              #= This is crazy code =#STRING(str) = exp
          str
        end
        ...
        function expCref(exp::Exp)::ComponentRef
              ...
              #= This is crazy code =#CREF(cr) = exp
          cr
        end
        ...
        function optArrayDimEqual(oad1::Option, oad2::Option)::Bool
              local b::Bool
        ...
        function stringListPath(paths::IList)::Path
              local qualifiedPath::Path

              qualifiedPath = begin
                  local str::String
                  local rest_str::IList
                  local p::Path
         ...
         #= Converts a list of strings into a qualified path, in reverse order.
           Ex: {'a', 'b', 'c'} => c.b.a =#
        function stringListPathReversed(inStrings::IList)::Path
              local outPath::Path
        ...
        function stringListPathReversed2(inStrings::IList, inAccumPath::Path)::Path
              local outPath::Path
        ...
        function pathToStringList(path::Path)::IList
              local outPaths::IList

              outPaths = listReverse(pathToStringListWork(path, list()))
          outPaths
        end

        function pathToStringListWork(path::Path, acc::IList)::IList
              local outPaths::IList
        ...
        function addSubscriptsLast(icr::ComponentRef, i::IList)::ComponentRef
        ...
        function getCrefsFromSubs(isubs::IList, includeSubs::Bool #= include crefs from array subscripts =#, includeFunctions::Bool #= note that if you say includeSubs = false then you won't get the functions from array subscripts =#)::IList
              local crefs::IList
        ...
        function getCrefFromExp(inExp::Exp, includeSubs::Bool #= include crefs from array subscripts =#, includeFunctions::Bool #= note that if you say includeSubs = false then you won't get the functions from array subscripts =#)::IList
              local outComponentRefLst::IList
        ...
        function getCrefFromFarg(inFunctionArgs::FunctionArgs, includeSubs::Bool #= include crefs from array subscripts =#, includeFunctions::Bool #= note that if you say includeSubs = false then you won't get the functions from array subscripts =#)::IList
              local outComponentRefLst::IList
        ...
        function iteratorName(iterator::ForIterator)::String
              ...
              #= This is crazy code =#ITERATOR(name = name) = iterator
          name
        end
        ...
        function iteratorRange(iterator::ForIterator)::Option
              ...
              #= This is crazy code =#ITERATOR(range = range) = iterator
          range
        end
        ...
        function iteratorGuard(iterator::ForIterator)::Option
              ...
              #= This is crazy code =#ITERATOR(guardExp = guardExp) = iterator
          guardExp
        end
        ...
        function getNamedFuncArgNamesAndValues(inNamedArgList::IList)::Tuple{IList, IList}
              local outExpList::IList
              local outStringList::IList
        ...
        function getCrefFromNarg(inNamedArg::NamedArg, includeSubs::Bool #= include crefs from array subscripts =#, includeFunctions::Bool #= note that if you say includeSubs = false then you won't get the functions from array subscripts =#)::IList
              local outComponentRefLst::IList
        ...
        function pathAppendList(inPathLst::IList)::Path
              local outPath::Path
        ...
        function pathToCrefWithSubs(inPath::Path, inSubs::IList)::ComponentRef
              local outComponentRef::ComponentRef
        ...
        function crefLastSubs(inComponentRef::ComponentRef)::IList
              local outSubscriptLst::IList
        ...
        function crefSetLastSubs(inCref::ComponentRef, inSubscripts::IList)::ComponentRef
        ...
        function getSubsFromCref(cr::ComponentRef, includeSubs::Bool #= include crefs from array subscripts =#, includeFunctions::Bool #= note that if you say includeSubs = false then you won't get the functions from array subscripts =#)::IList
              ...
              outComponentRef = begin
                ...
                @match (inComponentRef1, inComponentRef2) begin
                  (CREF_IDENT(name = id, subscripts = sub), cr2)  => begin
                      throw("#= This is crazy code =#CREF_FULLYQUALIFIED() = cr2")
        ...
        function lastClassname(inProgram::Program)::Path
              ...
              #= This is crazy code =#PROGRAM(classes = lst) = inProgram
              #= This is crazy code =#CLASS(name = id) = List.last(lst)
              outPath = IDENT(id)
          outPath
        end
        ...
        function classFilename(inClass::Class)::String
              ...
              #= This is crazy code =#CLASS(info = SOURCEINFO(fileName = outFilename)) = inClass
          outFilename
        end
        ...
        function subscriptsEqual(inSubList1::IList, inSubList2::IList)::Bool
              local outIsEqual::Bool
        ...
        function getClassName(inClass::Class)::String
              ...
              #= This is crazy code =#CLASS(name = outName) = inClass
          outName
        end
        ...
        function findIteratorIndexedCrefs(inExp::Exp, inIterator::String, inCrefs::IList = list())::IList
        ...
        function findIteratorIndexedCrefs_traverser(inExp::Exp, inCrefs::IList, inIterator::String)::Tuple{IList, Exp}
              local outCrefs::IList
        ...
        function getIteratorIndexedCrefs(inCref::ComponentRef, inIterator::String, inCrefs::IList)::IList
              local outCrefs::IList = inCrefs
        ...
        function getFileNameFromInfo(inInfo::SourceInfo)::String
              ...
              #= This is crazy code =#SOURCEINFO(fileName = inFileName) = inInfo
          inFileName
        end
        ...
        function onlyLiteralsInAnnotationMod(inMod::IList)::Bool
        ...
        function onlyLiteralsInExpEnter(inExp::Exp, inLst::IList)::Tuple{IList, Exp}
              local outLst::IList
              local outExp::Exp
         ...
         Visitor function for checking if Exp contains only literals, NO CREFS!
         It returns an empty list if it doesn't contain any crefs! =#
        function onlyLiteralsInExpExit(inExp::Exp, inLst::IList)::Tuple{IList, Exp}
              local outLst::IList
              local outExp::Exp
        ...
        function crefIdent(cr::ComponentRef)::String
              ...
              #= This is crazy code =#CREF_IDENT(str, list()) = cr
          str
        end
        ...
        function crefInsertSubscriptLstLst(inExp::Exp, inLst::IList)::Tuple{IList, Exp}
              local outLst::IList
              local outExp::Exp
        ...
        function crefInsertSubscriptLstLst2(inCref::ComponentRef, inSubs::IList)::ComponentRef
              local outCref::ComponentRef
        ...
        function isDerCrefFail(exp::Exp)
              #= This is crazy code =#CALL(CREF_IDENT("der", list()), FUNCTIONARGS(list(CREF()), list())) = exp
        end
        ...
        function getExpsFromArrayDim(inAd::ArrayDim)::Tuple{IList, Bool}
              local outExps::IList
         ...
         #= author: adrpo
          returns all the expressions from array dimension as a list
          also returns if we have unknown dimensions in the array dimension =#
        function getExpsFromArrayDimOpt(inAdO::Option)::Tuple{IList, Bool}
              local outExps::IList
              local hasUnknownDimensions::Bool
         ...
         #= author: adrpo
          returns all the expressions from array dimension as a list
          also returns if we have unknown dimensions in the array dimension =#
        function getExpsFromArrayDim_tail(inAd::ArrayDim, inAccumulator::IList)::Tuple{IList, Bool}
              local outExps::IList
              local hasUnknownDimensions::Bool
              ...
              o = begin
                ...
                @match cl begin
                  CLASS(name, partialPrefix, finalPrefix, encapsulatedPrefix, R_FUNCTION(funcRest), PARTS(typeVars, classAttr, classParts, _, _), info)  => begin
                      @assert (@match _, _ = elts) == (List.fold(listReverse(classParts), getFunctionInterfaceParts, list()))
        ...
        function getFunctionInterfaceParts(part::ClassPart, elts::IList)::IList
              local oelts::IList
              ...
              o = begin
                ...
                @match cl begin
                  CLASS(name, partialPrefix, finalPrefix, encapsulatedPrefix, restriction, PARTS(typeVars, classAttrs, classParts, annotations, comment), info)  => begin
                      @assert (@match _, _ = classParts) == (List.fold(listReverse(classParts), filterNestedClassesParts, list()))
        ...
        function filterNestedClassesParts(classPart::ClassPart, inClassParts::IList)::IList
              local outClassPart::IList
        ...
        function getExternalDecl(inCls::Class)::ClassPart
              ...
              local cp::ClassPart
              local class_parts::IList

              #= This is crazy code =#CLASS(body = PARTS(classParts = class_parts)) = inCls
        ...
        function makeClassElement(cl::Class)::ElementItem
              ...
              #= This is crazy code =#CLASS(finalPrefix = fp, info = info) = cl
        ...
        function componentName(c::ComponentItem)::String
              ...
              #= This is crazy code =#COMPONENTITEM(component = COMPONENT(name = name)) = c
          name
        end
        ...
        function mergeAnnotations2(oldmods::IList, newmods::IList)::IList
              local res::IList = listReverse(oldmods)
              ...
              for mod in newmods
                #= This is crazy code =#MODIFICATION(path = p) = mod
                try
              ...
              mod = begin
                ...
                @match (oldmod, newmod) begin
                  ...
                  (MODIFICATION(modification = SOME(CLASSMOD(args1, _))), arg2 && MODIFICATION(modification = SOME(CLASSMOD(args2, eq2))))  => begin
                      ...
                      res = list()
                      for arg1 in args1
                        #= This is crazy code =#MODIFICATION(path = p) = arg1
        ...
        function annotationToElementArgs(ann::Annotation)::IList
              local args::IList

              #= This is crazy code =#ANNOTATION(args) = ann
          args
        end
        ...
        function typeSpecStringNoQualNoDimsLst(inTypeSpecLst::IList)::String
              local outString::String
        ...
        function removeCrefFromCrefs(inAbsynComponentRefLst::IList, inComponentRef::ComponentRef)::IList
              local outAbsynComponentRefLst::IList
              ...
              outAbsynComponentRefLst = begin
                ...
                @matchcontinue (inAbsynComponentRefLst, inComponentRef) begin
                  ...
                  (cr1 <| rest, cr2)  => begin
                      #= This is crazy code =#CREF_IDENT(name = n1, subscripts = list()) = cr1
                      #= This is crazy code =#CREF_IDENT(name = n2, subscripts = list()) = cr2
                  ...
                  (cr1 <| rest, cr2)  => begin
                      #= This is crazy code =#CREF_QUAL(name = n1) = cr1
                      #= This is crazy code =#CREF_IDENT(name = n2) = cr2
                      @assert true == (stringEq(n1, n2))
                      rest_1 = removeCrefFromCrefs(rest, cr2)
                    rest_1
                  end
              ...
              outString = begin
                ...
                @matchcontinue (inClass, id, f) begin
                  (CLASS(body = PARTS(ann = ann)), _, _)  => begin
                      ...
                      #= This is crazy code =#SOME(str) = getNamedAnnotationStr(annlst, id, f)
                    SOME(str)
                  end
                  ...
                  (CLASS(body = DERIVED(comment = SOME(COMMENT(SOME(ANNOTATION(annlst)), _)))), _, _)  => begin
                      #= This is crazy code =#SOME(res) = getNamedAnnotationStr(annlst, id, f)
                    SOME(res)
                  end
        ...
        function getNamedAnnotationStr(inAbsynElementArgLst::IList, id::Path, f::ModFunc)::Option
              local outString::Option
              ...
              outCref = begin
                ...
                @match (inCref, inMapFunc) begin
                  (CREF_QUAL(name, subs, rest_cref), _)  => begin
                      ...
                      #= This is crazy code =#CREF_IDENT(name, subs) = inMapFunc(cref)
        ...
        function getDefineUnitsInElements(elts::IList)::IList
              local outElts::IList
        ...
        function getElementItemsInClass(inClass::Class)::IList
        ...
        function getElementItemsInClassPart(inClassPart::ClassPart)::IList
              local outElements::IList
        ...
        T = Any
        ArgT = Any
        function traverseListGeneric(inList::IList, inFunc::FuncType, inArg::ArgT)::Tuple{Bool, ArgT, IList}
        ...
        function crefExplode(inCref::ComponentRef, inAccum::IList = list())::IList
              local outCrefParts::IList
        ...
        function traverseExpShallowIterator(inIterator::ForIterator, inArg::ArgT, inFunc::FuncT)::ForIterator
              ...
              #= This is crazy code =#ITERATOR(name, guard_exp, range_exp) = inIterator
        ...
        function getAnnotationsFromConstraintClass(inCC::Option)::IList
              local outElArgLst::IList
        ...
        function getAnnotationsFromItems(inComponentItems::IList, ccAnnotations::IList)::IList
              local outLst::IList = list()
        ...
        function stripGraphicsAndInteractionModification(inAbsynElementArgLst::IList)::Tuple{IList, IList}
              local outAbsynElementArgLst2::IList
              local outAbsynElementArgLst1::IList
        ...
        function traverseClasses2(inClasses::IList, inPath::Option, inFunc::FuncType, inArg::Type_a #= extra argument =#, inVisitProtected::Bool #= visit protected elements =#)::Tuple
              local outTpl::Tuple
        ...
        function partsHasLocalClass(inParts::IList)::Bool
              local res::Bool
        ...
        function eltsHasLocalClass(inElts::IList)::Bool
              local res::Bool
        ...
        function traverseInnerClass(inClass::Absyn.Class, inPath::Option, inFunc::FuncType, inArg::Type_a #= extra value =#, inVisitProtected::Bool #= if true, traverse protected elts =#)::Tuple
              local outTpl::Tuple
        ...
        function traverseInnerClassParts(inClassParts::IList, inPath::Option, inFunc::FuncType, inArg::Type_a #= extra argument =#, inVisitProtected::Bool #= visist protected elts =#)::Tuple
              local outTpl::Tuple
        ...
        function traverseInnerClassElements(inElements::IList, inPath::Option, inFuncType::FuncType, inArg::Type_a, inVisitProtected::Bool #= visit protected elts =#)::Tuple
              local outTpl::Tuple
        ...
        function traverseInnerClassElementspec(inElementSpec::Absyn.ElementSpec, inPath::Option, inFuncType::FuncType, inArg::Type_a, inVisitProtected::Bool #= visit protected elts =#)::Tuple
              local outTpl::Tuple
        ...
        function getComponentItemsFromElementSpec(elemSpec::Absyn.ElementSpec)::IList
              local componentItems::IList
        ...
        function getComponentItemsFromElementItem(inElementItem::Absyn.ElementItem)::IList
              local componentItems::IList
