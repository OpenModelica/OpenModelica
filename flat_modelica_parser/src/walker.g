...
//	void* attr = 0;
	...
	:
      ...
      while( !(var_stack.empty()) ){
        modSimPackTest::FlatVariable(variable_type,
          var_stack.top(),
          (int)(pfx.flow), 
          (int)(pfx.variability), 
          (int)(pfx.direction), 
...
unary_arithmetic_expression returns [void* ast]
	:
		(#(UNARY_PLUS ast = term) { eq_stack.push("UNARY_PLUS");
						/*ast = Absyn__UNARY(Absyn__UPLUS,ast);*/ }
		|#(UNARY_MINUS ast = term) {  eq_stack.push("UNARY_MINUS");
...
string_concatenation returns [void * ast] :
	...
	|#(p:PLUS ast=string_concatenation s2:STRING)
		{
			//ast = to_rml_str(s2);
		}
	;
