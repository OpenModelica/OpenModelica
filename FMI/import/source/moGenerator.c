...
static char* getDecompPath(const char * omPath, const char* mid){
	...
	if (!GetEnvironmentVariable("OPENMODELICAHOME", omPath, PATHSIZE)) {
        if (GetLastError() == ERROR_ENVVAR_NOT_FOUND) {
            printf("#### %s Error: Environment variable \"OPENMODELICAHOME\" not defined\n",QUOTEME(__LINE__));
        }
        else {
...
fmiScalarVariableType getElementType(ScalarVariable* sv){
	...
	switch(elm){
		...
		case elm_Enumeration:sv_type = sv_enum;
							 return sv_type;
		default: printf("#### unknown element type in ScalarVaraible...\n"); exit(EXIT_FAILURE);
	}
}
...
void* allocateElmSV(fmiScalarVariable fmisv){
	switch(fmisv.type){
		...
		default: printf("#### Line %s: unknown element type in allocateElmSV()...\n",QUOTEME(__LINE__)); exit(EXIT_FAILURE);
	}
}
...
void instElmSV(ScalarVariable* sv, fmiScalarVariable fmisv){
	...
	switch(fmisv.type){
		...
		default: printf("#### Line %s: unknown error in instElmSV()...\n",QUOTEME(__LINE__)); exit(EXIT_FAILURE);
	}
}
...
static void instScalarVariable(ModelDescription* md,fmiScalarVariable* list){
	...
	if(md->modelVariables){
		...
		printf("#### instScalarVariable failed: no modelVariable defined or memory error...\n");
		exit(EXIT_FAILURE);
	}
}
...
void tmpcodegen(size_t nx, size_t nz, const char* mid, const char* guid, const char* decompPath){
	...
	if(!pfile){
		...
		while(!feof(pf)){
			...
			if(ferror(pf)){
				printf("#### Error: Reading lines from %s failed...\n",FMU_TEMPLATE);
				exit(EXIT_FAILURE);
			}
			fputs(tmpstr,pfile);
		}
...
void blockcodegen(ModelDescription* md,size_t nx, size_t nz, const char* mid, const char* guid,
				  const char* decompPath, const char* fmudllpath){
	// Allocated memory needed to be freed
	char * id;
	FILE * pfile;	
	int j;
