...
#define WHERESTR_1  "[%s, %s, %s, %s(...), line: %d]: "
#define WHEREARG_1  __DATE__, __TIME__, __FILE__, __func__, __LINE__
// #define ERRORPRINT2(...)       fprintf(errLogFile, __VA_ARGS__)
// #define ERRORPRINT(_fmt, ...)  ERRORPRINT2(WHERESTR _fmt, WHEREARG, __VA_ARGS__)
#define ERRORPRINT       fprintf(errLogFile, WHERESTR_1, WHEREARG_1)
// end
...
#define LOGPRINT       fprintf(logFile, WHERESTR_1, WHEREARG_1)
// end
 ...
 * What's done here?
#if 0
  fprintf(errLogFile, "fmudllpath=%s\n", fmudllpath);
...
fmiVariability getFMIVariability(ScalarVariable* sv){
	...
	switch(fmi_var){
		...
		default: ERRORPRINT; fprintf(errLogFile,"#### Unknown error in getFMIVariability() for fmiScalarVariable: %s...\n",getString(sv,att_name)); exit(EXIT_FAILURE);
	}
	return ret;
}
...
fmiCausality getFMICausality(ScalarVariable* sv){
	...
	switch(fmi_cau){
		...
		default: ERRORPRINT; fprintf(errLogFile,"#### Unknown error in getFMICausality() for fmiScalarVariable: %s...\n",getString(sv,att_name)); exit(EXIT_FAILURE);
	}
	return ret;
}
...
fmiAlias getFMIAlias(ScalarVariable* sv){
	...
	switch(fmi_als){
		...
		default: ERRORPRINT; fprintf(errLogFile,"#### Unknown error in getFMIAlias() for fmiScalarVariable: %s...\n",getString(sv,att_name)); exit(EXIT_FAILURE);
	}
	return ret;
}
...
// function that returns the variable naming convention
...
// function that creates and returns the default experiment data strucutre
...
fmiTypeSpec *getFmiTypeSpec(Element *tp){
	...
	const char *tmp_str;
	fmiBooleanXML tmp_bl;
	fmiTypeSpec * fmitps = NULL;	
	
	switch(tp->type){
		case elm_RealType:{
			fmitps = (fmiTypeSpec *)calloc(1,sizeof(fmiTypeSpec));
			fmitps->type = tp->type;
			fmitps->tp = calloc(1,sizeof(fmiRealType));
			
			tmp_str = getString(tp,att_quantity);
			((fmiRealType *)fmitps->tp)->quantity = (tmp_str!=NULL ? tmp_str : "");
			
			tmp_str = getString(tp,att_unit);
			((fmiRealType *)fmitps->tp)->unit = (tmp_str!=NULL ? tmp_str : "");
			
			tmp_str = getString(tp,att_displayUnit);
			((fmiRealType *)fmitps->tp)->displayUnit = (tmp_str!=NULL ? tmp_str : "");
			
			tmp_bl = getBoolean(tp,att_relativeQuantity,&vs);
			((fmiRealType *)fmitps->tp)->relQuantity = (vs==valueDefined ? tmp_bl : 0);
			
			tmp_db = getDouble(tp,att_min,&vs);
			if(vs==valueDefined){
				((fmiRealType *)fmitps->tp)->min = tmp_db;
				((fmiRealType *)fmitps->tp)->defMin = 1;
			}
			else ((fmiRealType *)fmitps->tp)->defMin = 0;
			
			tmp_db = getDouble(tp,att_max,&vs);
			if(vs==valueDefined){
				((fmiRealType *)fmitps->tp)->max = tmp_db;
				((fmiRealType *)fmitps->tp)->defMax = 1;
			}
			else ((fmiRealType *)fmitps->tp)->defMax = 0;

			tmp_db = getDouble(tp,att_nominal,&vs);
			if(vs==valueDefined){
				((fmiRealType *)fmitps->tp)->nominal = tmp_db;
				((fmiRealType *)fmitps->tp)->defNorminal = 1;
			}
			else ((fmiRealType *)fmitps->tp)->defNorminal = 0;	
			break;
		}
		case elm_IntegerType:{
			fmitps = (fmiTypeSpec *)calloc(1,sizeof(fmiTypeSpec));
			fmitps->type = tp->type;
			fmitps->tp = calloc(1,sizeof(fmiIntegerType));
			
			tmp_str = getString(tp,att_quantity);
			((fmiIntegerType *)fmitps->tp)->quantity = (tmp_str!=NULL ? tmp_str : "");
			
			tmp_i = getInt(tp,att_min,&vs);
			if(vs==valueDefined){
				((fmiIntegerType *)fmitps->tp)->min = tmp_i;
				((fmiIntegerType *)fmitps->tp)->defMin = 1;
			}
			else ((fmiIntegerType *)fmitps->tp)->defMin = 0;
			
			tmp_i = getInt(tp,att_max,&vs);
			if(vs==valueDefined){
				((fmiIntegerType *)fmitps->tp)->max = tmp_i;
				((fmiIntegerType *)fmitps->tp)->defMax = 1;
			}
			else ((fmiIntegerType *)fmitps->tp)->defMax = 0;

			break;
		}
		case elm_BooleanType: break;
		
		case elm_StringType: break;
		
		case elm_EnumerationType:{
			fmitps = (fmiTypeSpec *)calloc(1,sizeof(fmiTypeSpec));
			fmitps->type = tp->type;
			fmitps->tp = calloc(1,sizeof(fmiEnumerationType));
			
			tmp_str = getString(tp,att_quantity);
			((fmiEnumerationType *)fmitps->tp)->quantity = (tmp_str!=NULL ? tmp_str : "");
			
			tmp_i = getInt(tp,att_min,&vs);
			if(vs==valueDefined){
				((fmiEnumerationType *)fmitps->tp)->min = tmp_i;
				((fmiEnumerationType *)fmitps->tp)->defMin = 1;
			}
			else ((fmiEnumerationType *)fmitps->tp)->defMin = 0;
			
			tmp_i = getInt(tp,att_max,&vs);
			if(vs==valueDefined){
				((fmiEnumerationType *)fmitps->tp)->max = tmp_i;
				((fmiEnumerationType *)fmitps->tp)->defMax = 1;
			}
			else ((fmiEnumerationType *)fmitps->tp)->defMax = 0;
									
			((fmiEnumerationType *)fmitps->tp)->items = NULL;
...
// function that returns the pointer pointed to the
// allocated memory for an fmiTypeElement structure
fmiTypeElement *getFmiTypeElement(Type *type){
	fmiTypeElement *fmiType = NULL;
	int len;

	fmiType = (fmiTypeElement *)calloc(1,sizeof(fmiTypeElement));
	
	fmiType->name = getName(type);
	len = strlen(fmiType->name);
	fmiType->flatName = (char*)calloc(len+1, sizeof(char));
	strcpy(fmiType->flatName,fmiType->name);
	charReplace(fmiType->flatName,len,'.','_');
	charReplace(fmiType->flatName,len,'[','_');
	charReplace(fmiType->flatName,len,']','_');
	charReplace(fmiType->flatName,len,',','_');
	charReplace(fmiType->flatName,len,' ','_');
	#ifdef _DEBUG_
	printf("[%d]: #### fmiType->name = %s\n",__LINE__,fmiType->name);
	printf("[%d]: #### fmiType->flatName = %s\n",__LINE__,fmiType->flatName);
	#endif
	fmiType->desc = getString(type, att_description);
	#ifdef _DEBUG_
	printf("[%d]: #### fmiType->desc = %s\n",__LINE__,fmiType->desc);
	#endif
	
	fmiType->typeSpec = getFmiTypeSpec(type->typeSpec);
	#ifdef _DEBUG_
	if(fmiType->typeSpec->type==elm_EnumerationType){
	printf("[%d]: #### fmiType->typeSpec->tp->min = %d\n",__LINE__,(((fmiEnumerationType *)(fmiType->typeSpec->tp))->min));
	printf("[%d]: #### fmiType->typeSpec->tp->max = %d\n",__LINE__,(((fmiEnumerationType *)(fmiType->typeSpec->tp))->max));
	}
	#endif
	return fmiType;
}

// function that returns the pointer to type definitions
fmiTypeDefinitions *getTypeDefinitions(ModelDescription *md){
	fmiTypeDefinitions *fmiTypeDef = NULL;
	fmiTypeDefinitions *tmp_fmiTypeDef = NULL;
	Type **type = NULL;
	int i;
	
	if(md->typeDefinitions){
		type = md->typeDefinitions;
		fmiTypeDef = (fmiTypeDefinitions *)calloc(1, sizeof(fmiTypeDefinitions));
		tmp_fmiTypeDef = fmiTypeDef;
		tmp_fmiTypeDef->type = getFmiTypeElement(*type);
		tmp_fmiTypeDef->next = NULL;
		for(i = 1; type[i]; i++){
			tmp_fmiTypeDef->next = (fmiTypeDefinitions *)calloc(1, sizeof(fmiTypeDefinitions));
			tmp_fmiTypeDef = tmp_fmiTypeDef->next;
			tmp_fmiTypeDef->type = getFmiTypeElement(type[i]);
			tmp_fmiTypeDef->next = NULL;
		}
	}
...
void instFmuModelDescription(ModelDescription* md, fmuModelDescription* fmuMD, fmiModelVariable* fmuMV){
	...
	assert(fmuMD->guid); // This is a required attribute
	...
	fmuMD->fmiTypeDef = getTypeDefinitions(md);
	...
	if(!pfile){
			...
			for(j = 0; j<fmuMD->modelVariable->nsv; j++){
				...
				if(strncmp(tmpSV[j].name,"der(",4)!=0){ // check if the scalar variable is a derivative						
					switch(tmpSV[j].type){ // sorting by variable types
						...
						case sv_enum:{
							// printf("[%d]: #### encountering sv_enum: %s...\n",__LINE__,tmpSV[j].name);
						}
						default: {}
					}
				}
		...
		if(fmuMD->ncs>0){
			...
			if (noReal>0){
				...
				if(noReal==1) fprintf(pfile,"%u};\n",tmpReal->vr);
				else if(noReal>1){					
					for(j = 0; j<noReal-1; j++){
						fprintf(pfile,"%u, ",tmpReal->vr);
						if(j>0 && j%10==0) fputs("\n",pfile);
			...
			if (noInteger>0){
				...
				if(noInteger==1) fprintf(pfile,"%u};\n",tmpInteger->vr);
				else if(noInteger>1){
					for(j=0; j<noInteger-1; j++){
						fprintf(pfile,"%u, ",tmpInteger->vr);
						if(j>0 && j%10==0) fputs("\n",pfile);
			...
			if (noBoolean>0){
				...
				if(noBoolean==1) fprintf(pfile,"%u};\n",tmpBoolean->vr);
				else if(noBoolean>1){
					for(j=0; j<noBoolean-1; j++){
						fprintf(pfile,"%u, ",tmpBoolean->vr);
						if(j>0 && j%10==0) fputs("\n",pfile);
			...
			if (noString>0){
				...
				if(noString==1) fprintf(pfile,"%u};\n",tmpString->vr);
				else if(noString>1){
					for(j=0; j<noString-1; j++){
						fprintf(pfile,"%u, ",tmpString->vr);
						if(j>0 && j%10==0) fputs("\n",pfile);
...
int main(int argc, char *argv[]){
	...
	LOGPRINT; fprintf(logFile,"FMU decompression path: %s\n",decompPath);
	LOGPRINT; fprintf(logFile,"FMU name: %s\n",fmuname);
	LOGPRINT; fprintf(logFile,"FMU model identifier: %s\n",fmuMD->mid);
	LOGPRINT; fprintf(logFile,"Modelica model name: %s.mo\n",fmuMD->mid);
	#endif
