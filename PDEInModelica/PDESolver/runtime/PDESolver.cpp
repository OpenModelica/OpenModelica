...
int setDt(RUNTIME_DATA* rData){
    ...
    double dt = rData->cfl*mDx/abs(lambda);
...
int updateSpaceDerivatives(MODEL_DATA* mData){
    int M = mData->M;
...
int updateStates(RUNTIME_DATA* rData){
    ...
    for (int iState=0; iState<mData->nStateFields; iState++){
        ...
        if (mData->isBc[iState*2+0] == 0)
            //forward difference
            mData->stateFields[iState*M + 0] = mData->stateFields[iState*M + 0] + rData->dt*mData->stateFieldsDerTime[iState*M + 0];
        ...
        if (mData->isBc[iState*2+1] == 0)
            //forward difference
            mData->stateFields[iState*M + M-1] = mData->stateFields[iState*M + M-1] + rData->dt*mData->stateFieldsDerTime[iState*M + M-1];
    }
...
int doStep(RUNTIME_DATA* rData){
    MODEL_DATA* mData = rData->modelData;
    updateSpaceDerivatives(mData);
    functionPDE(rData->modelData);
    updateStates(rData);
    return 0;
}
