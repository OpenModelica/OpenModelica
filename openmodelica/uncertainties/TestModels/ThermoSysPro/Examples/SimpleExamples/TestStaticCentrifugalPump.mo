...
model TestStaticCentrifugalPump
  annotation(Diagram);
  ThermoSysPro.WaterSteam.Machines.StaticCentrifugalPump StaticCentrifugalPump1 annotation(Placement(transformation(x=-10.0, y=-30.0, scale=0.1, aspectRatio=1.0, flipHorizontal=true, flipVertical=false)));
  ...
  ThermoSysPro.InstrumentationAndControl.Blocks.Sources.Pulse pulse(width=200, period=500, amplitude=1400, offset=0) annotation(Placement(transformation(x=-30.0, y=-60.0, scale=0.1, aspectRatio=1.0, flipHorizontal=false, flipVertical=false)));
equation
  ...
  connect(pulse.y,StaticCentrifugalPump1.rpm_or_mpower) annotation(Line(points={{-19,-60},{-10,-60},{-10,-41}}, color={0,0,255}, smooth=0));
