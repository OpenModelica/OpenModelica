 ...
 function buildParserCode
  ...
  algorithm
    ...
    for i in 2:numRules loop
      ...
      re := "#line";
      (3,_::lineStr::origFile::_) := System.regex(rest,"#line *([0-9]*) *(\"[A-Za-z0-9.]*\")",3,extended=true,sensitive=false);
      ...
      //re := "{[^}]*;}";
      re := "[{] *(.*;) *[}]";
      (numMatches,regexError::resultRegex::_) := System.regex(rest,re,2,extended=true,sensitive=false);
