...
// translateModel and translateFunctions. All other templates are called by
// these root templates.
//
// To future maintainers of this file:
//
// - A variable declared like this:
//     # var = "" /*BUFD*/
//   is a text buffer that you can later append text to. In the new version of
//   Susan it should be declared like this:
//     let &var = buffer ""
//
// - A template parameter in a declaration like this:
//     ..., Text var /*BUFP*/, ...
//   is a reference to a text buffer that can be appended to. In the new
//   version of Susan it should be declared like this:
//     ..., Text &var, ...
//
// - A template parameter in a call like this:
//     ..., var /*BUFC*/, ...
//   is a reference to a text buffer that can be appended to. In the new
//   version of Susan it should be used like this:
//     ..., &var, ...
...
globalDataVarNamesArray(String name, list<SimVar> items) ::=
  if items then
    # itemsStr = (items of SIMVAR: '"<crefSubscript(origName)>"' ", ")
    <<
...
case EXTOBJINFO then
  ...
  # ctorCalls = (
      constructors of (var, fn, exps):
        # expsStr = (exps: daeExp(it, contextOther, preExp /*BUFC*/, varDecls /*BUFC*/) ", ")
        '<cref(var)> = <fn>(<expsStr>);'
      "\n")
  <<
...
functionDaeOutput(list<SimEqSystem> nonStateContEquations,
  ...
  # nonStateContPart = (nonStateContEquations:
    '<equation_(it, contextSimulationNonDescrete, varDecls /*BUFC*/)>' "\n")
  # algAndEqAssertsPart = (algorithmAndEquationAsserts:
    '<algStatement(it, contextSimulationNonDescrete, varDecls /*BUFC*/)>' "\n")
...
functionDaeOutput2(list<SimEqSystem> nonStateDiscEquations,
  ...
  # nonSateDiscPart = (nonStateDiscEquations:
    '<equation_(it, contextSimulationDescrete, varDecls /*BUFC*/)>' "\n")
  # removedPart = (removedEquations:
    '<equation_(it, contextSimulationDescrete, varDecls /*BUFC*/)>' "\n")
  <<
...
functionZeroCrossing(list<ZeroCrossing> zeroCrossings) ::=
  # varDecls = "" /*BUFD*/
  # zeroCrossingCode = zeroCrossingsTpl(zeroCrossings, varDecls /*BUFC*/)
  <<
...
functionUpdateDependents(list<SimEqSystem> allEquations,
  ...
  # hvars = (helpVarInfo of (hindex, exp, _):
      # preExp = "" /*BUFD*/
      # expPart = daeExp(exp, contextSimulationDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
...
functionUpdateDepend(list<SimEqSystem> allEquationsPlusWhen) ::=
  # varDecls = "" /*BUFD*/
  # eqs = (allEquationsPlusWhen: '<equation_(it, contextSimulationDescrete, varDecls /*BUFC*/)>' "\n")
  <<
...
functionStoreDelayed(list<tuple<DAE.Exp, DAE.Exp>> delayedExps) ::=
  ...
  # storePart = (delayedExps of (id, e):
      # preExp = "" /*BUFD*/
      # idRes = daeExp(id, contextSimulationNonDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
      # eRes = daeExp(e, contextSimulationNonDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
      <<
      <preExp>
...
functionWhen(list<SimWhenClause> whenClauses) ::=
  ...
  # cases = (whenClauses of SIM_WHEN_CLAUSE:
    ...
    case <i0>:
      ...
      <reinits of reinit:
        # preExp = "" /*BUFD*/
        # body = functionWhenReinitStatement(reinit, preExp /*BUFC*/, varDecls /*BUFC*/)
        '<preExp><\n><body>'
      "\n">
      break;<\n>
    >>)
...
functionWhenCaseEquation(Option<WhenEquation>, Text varDecls /*BUFP*/) ::=
case SOME(weq as WHEN_EQ) then
  # preExp = "" /*BUFD*/
  # expPart = daeExp(weq.right, contextSimulationDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
  <<
  save(<cref(weq.left)>);
...
functionWhenReinitStatement(ReinitStatement, Text preExp /*BUFP*/,
                            Text varDecls /*BUFP*/) ::=
case REINIT then
  # val = daeExp(value, contextSimulationDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
  <<
  <cref(stateVar)> = <val>;
  >>
...
functionInitial(list<SimEqSystem> initialEquations) ::=
  ...
  # eqPart = (initialEquations of SES_SIMPLE_ASSIGN:
    '<equation_(it, contextOther, varDecls /*BUFC*/)>' "\n")
  <<
  ...
  # body = (
    residualEquations of SES_RESIDUAL:
      if exp is DAE.SCONST then
        ...
        # preExp = "" /*BUFD*/
        # expPart = daeExp(exp, contextOther, preExp /*BUFC*/, varDecls /*BUFC*/)
...
functionExtraResudials(list<SimEqSystem> allEquations) ::=
  (allEquations of eq as SES_NONLINEAR:
     ...
     # body = (eq.eqs of eq2 as SES_RESIDUAL:
         # preExp = "" /*BUFD*/
         # expPart = daeExp(eq2.exp, contextSimulationDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
...
equation_(SimEqSystem eq, Context context, Text varDecls /*BUFP*/) ::=
  case SES_SIMPLE_ASSIGN     then equationSimpleAssign(eq, context, varDecls /*BUFC*/)
  case SES_ARRAY_CALL_ASSIGN then equationArrayCallAssign(eq, context, varDecls /*BUFC*/)
  case SES_ALGORITHM         then equationAlgorithm(eq, context, varDecls /*BUFC*/)
  case SES_LINEAR            then equationLinear(eq, context, varDecls /*BUFC*/)
  case SES_MIXED             then equationMixed(eq, context, varDecls /*BUFC*/)
  case SES_NONLINEAR         then equationNonlinear(eq, context, varDecls /*BUFC*/)
  case SES_WHEN              then equationWhen(eq, context, varDecls /*BUFC*/)
  case _                     then "NOT IMPLEMENTED EQUATION"
...
case SES_ALGORITHM then
  # stmts = (statements : algStatement(it, context, varDecls /*BUFC*/) \n) 
  <<
  <stmts>
  >>
...
case SES_LINEAR then
  ...
  <beqs:
     # preExp = "" /*BUFD*/
     # expPart = daeExp(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
...
functionsMakefile(FunctionCode) ::=
...
subscriptsTpl(list<Subscript> subscripts) ::=
  if subscripts then '[<subscripts of s: subscriptTpl(s) ",">]' else ""
...
functionBody(Function fn) ::=
  case FUNCTION           then functionBodyRegularFunction(fn)
  case EXTERNAL_FUNCTION  then functionBodyExternalFunction(fn)
  case RECORD_CONSTRUCTOR then functionBodyRecordConstructor(fn)
...
case FUNCTION then
  ...
  # _ = (variableDeclarations: varInit(it, "", i1, varDecls /*BUFC*/, varInits /*BUFC*/))
  # bodyPart = (body of stmt : funStatement(stmt, varDecls /*BUFC*/) \n)
  # outVarsStr = (outVars: varOutput(it, retVar, i1, varDecls /*BUFC*/, varInits /*BUFC*/))
  <<
...
case EXTERNAL_FUNCTION then
  ...
  # _ = (outVars: varInit(it, "out", i1, varDecls /*BUFC*/, outputAlloc /*BUFC*/))
  <<
  int in_<fname>(type_description * inArgs, type_description * outVar)
  {
...
varInit(Variable, String outStruct, Integer i, Text varDecls /*BUFP*/,
        Text varInits /*BUFP*/) ::=
case var as VARIABLE then
  ...
  # instDimsInit = (instDims of exp: daeExp(exp, contextOther, varInits /*BUFC*/, varDecls /*BUFC*/) ", ")
  if instDims then
...
varOutput(Variable source, String dest, Integer i, Text varDecls /*BUFP*/,
          Text varInits /*BUFP*/) ::=
case var as VARIABLE then
  # instDimsInit = (instDims of exp: daeExp(exp, contextOther, varInits /*BUFC*/, varDecls /*BUFC*/) ", ")
  if instDims then
...
case EXTERNAL_FUNCTION then
  # fname = underscorePath(name)
  # args = (extArgs: extArg(it, preExp /*BUFC*/, varDecls /*BUFC*/) ", ")
  # returnAssign = if extReturn is SIMEXTARG(cref=c) then '<cref(c)>_ext = ' else ""
  <<
  <extArgs: extFunCallVardecl(it, varDecls /*BUFC*/) "\n">
...
extArg(SimExtArg, Text preExp /*BUFP*/, Text varDecls /*BUFP*/) ::=
...
funStatement(Statement stmt, Text varDecls /*BUFP*/) ::=
  case ALGORITHM then
    (statementLst: algStatement(it, contextOther, varDecls /*BUFC*/) "\n") 
  case _ then
    "NOT IMPLEMENTED FUN STATEMENT"

algStatement(DAE.Statement stmt, Context context, Text varDecls /*BUFP*/) ::=
  case STMT_ASSIGN       then algStmtAssign(stmt, context, varDecls /*BUFC*/)
  case STMT_ASSIGN_ARR   then algStmtAssignArr(stmt, context, varDecls /*BUFC*/)
  case STMT_TUPLE_ASSIGN then algStmtTupleAssign(stmt, context, varDecls /*BUFC*/)
  case STMT_IF           then algStmtIf(stmt, context, varDecls /*BUFC*/)
  case STMT_FOR          then algStmtFor(stmt, context, varDecls /*BUFC*/)
  case STMT_WHILE        then algStmtWhile(stmt, context, varDecls /*BUFC*/)
  case STMT_ASSERT       then algStmtAssert(stmt, context, varDecls /*BUFC*/)
  case STMT_WHEN         then algStmtWhen(stmt, context, varDecls /*BUFC*/)
  case STMT_MATCHCASES   then algStmtMatchcases(stmt, context, varDecls /*BUFC*/)
  case STMT_BREAK        then 'break;<\n>'
  case STMT_TRY          then algStmtTry(stmt, context, varDecls /*BUFC*/)
  case STMT_CATCH        then algStmtCatch(stmt, context, varDecls /*BUFC*/)
  case STMT_THROW        then 'throw 1;<\n>'
  case STMT_RETURN       then 'goto _return;<\n>'
  case STMT_NORETCALL    then algStmtNoretcall(stmt, context, varDecls /*BUFC*/)
  case _                 then "NOT IMPLEMENTED ALG STATEMENT"

algStmtAssign(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
...
algStmtAssignArr(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
case STMT_ASSIGN_ARR(exp=e, componentRef=cref, type_=t) then
  # preExp = "" /*BUFD*/
  # expPart = daeExp(e, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  if indexSpecFromCref(cref, context, preExp /*BUFC*/, varDecls /*BUFC*/) then
    <<
    <preExp>
...
algStmtTupleAssign(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
case STMT_TUPLE_ASSIGN(exp=CALL) then
  # preExp = "" /*BUFD*/
  # retStruct = daeExp(exp, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  <<
  <preExp>
  <expExpLst of CREF: '<scalarLhsCref(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)> = <retStruct>.targ<i1>;' "\n">
  >>

algStmtIf(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
case STMT_IF then
  # preExp = "" /*BUFD*/
  ...
  <preExp>
  if (<condExp>) {
    <statementLst : algStatement(it, context, varDecls /*BUFC*/) \n>
  }
...
algStmtFor(DAE.Statement stmt, Context context, Text varDecls /*BUFP*/) ::=
  case STMT_FOR(exp=rng as RANGE) then
    algStmtForRange(stmt, context, varDecls /*BUFC*/)
  case STMT_FOR then
    algStmtForGeneral(stmt, context, varDecls /*BUFC*/)

algStmtForRange(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
case STMT_FOR(exp=rng as RANGE) then
  # stateVar = tempDecl("state", varDecls /*BUFC*/)
  ...
  # er2 = if rng.expOption is SOME(eo) 
          then daeExp(eo, context, preExp /*BUFC*/, varDecls /*BUFC*/)
          else "(1)"
...
algStmtForGeneral(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
case STMT_FOR then
  # stateVar = tempDecl("state", varDecls /*BUFC*/)
  ...
  # statements = (statementLst: algStatement(it, context, varDecls /*BUFC*/) \n)
  # id = '<ident>'
...
algStmtWhile(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
...
algStmtAssert(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
...
algStmtMatchcases(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
...
algStmtTry(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
case STMT_TRY then
  # body = (tryBody: algStatement(it, context, varDecls /*BUFC*/) "\n")
  <<
  try {
    <body>
  }
...
algStmtCatch(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
case STMT_CATCH then
  # body = (catchBody: algStatement(it, context, varDecls /*BUFC*/) "\n")
  <<
  catch (int i) {
    <body>
  }
...
algStmtNoretcall(DAE.Statement, Context context, Text varDecls /*BUFP*/) ::=
...
algStatementWhenPre(DAE.Statement, Text varDecls /*BUFP*/) ::=
  case STMT_WHEN(exp=ARRAY(array=el)) then
    # restPre = if elseWhen is SOME(ew)
                then algStatementWhenPre(ew, varDecls /*BUFC*/) else ""
    # preExp = "" /*BUFD*/
    # assignments = algStatementWhenPreAssigns(el, helpVarIndices, preExp /*BUFC*/, varDecls /*BUFC*/)
    <<
    <preExp>
    <assignments>
    <restPre>
  ...
  case when as STMT_WHEN then
    ...
    case {i} then
      # restPre = if when.elseWhen is SOME(ew)
                  then algStatementWhenPre(ew, varDecls /*BUFC*/) else ""
      # preExp = "" /*BUFD*/
      # res = daeExp(when.exp, contextSimulationDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
      <<
      <preExp>
...
algStatementWhenElse(Option<DAE.Statement>, Text varDecls /*BUFP*/) ::=
case SOME(when as STMT_WHEN) then
  # statements = (when.statementLst: '<algStatement(it, contextSimulationDescrete, varDecls /*BUFC*/)>' "\n")
...
algStatementWhenPreAssigns(list<Exp> exps, list<Integer> ints,
  ...
  case (firstExp :: restExps) then
    ...
    case (firstInt :: restInts) then
      # rest = algStatementWhenPreAssigns(restExps, restInts, preExp /*BUFC*/, varDecls /*BUFC*/)
      # firstExpPart = daeExp(firstExp, contextSimulationDescrete, preExp /*BUFC*/, varDecls /*BUFC*/)
      <<
      localData-\>helpVars[<firstInt>] = <firstExpPart>;
      <rest>
      >>

elseExpr(DAE.Else, Context context, Text varDecls /*BUFP*/) ::= 
  case NOELSE then ()
  case ELSEIF then
    # preExp = "" /*BUFD*/
    # condExp = daeExp(exp, context, preExp /*BUFC*/, varDecls /*BUFC*/)
...
daeExp(Exp exp, Context context, Text preExp /*BUFP*/,
  ...
  case SCONST         then daeExpSconst(string, preExp /*BUFC*/, varDecls /*BUFC*/)
  case BCONST         then if bool then "(1)" else "(0)"
  case CREF           then daeExpCrefRhs(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case BINARY         then daeExpBinary(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case UNARY          then daeExpUnary(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case LBINARY        then daeExpLbinary(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case LUNARY         then daeExpLunary(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case RELATION       then daeExpRelation(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case IFEXP          then daeExpIf(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case CALL           then daeExpCall(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case ARRAY          then daeExpArray(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case MATRIX         then daeExpMatrix(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case CAST           then daeExpCast(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case ASUB           then daeExpAsub(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case SIZE           then daeExpSize(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case REDUCTION      then daeExpReduction(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case VALUEBLOCK     then daeExpValueblock(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case LIST           then daeExpList(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case CONS           then daeExpCons(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case META_TUPLE     then daeExpMetaTuple(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case META_OPTION    then daeExpMetaOption(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  case METARECORDCALL then daeExpMetarecordcall(it, context, preExp /*BUFC*/, varDecls /*BUFC*/)
...
daeExpCrefRhs(Exp exp, Context context, Text preExp /*BUFP*/,
  ...
  case cref as CREF(componentRef=CREF_IDENT(subscriptLst=subs)) then
    if daeExpCrefRhsArrayBox(exp, context, preExp /*BUFC*/, varDecls /*BUFC*/) then
      it
      ...
      # dimsValuesStr = (subs of INDEX: daeExp(exp, context, preExp /*BUFC*/, varDecls /*BUFC*/) ", ")
      <<
...
case rel as RELATION then
  if daeExpRelationSim(exp, context, preExp /*BUFC*/, varDecls /*BUFC*/) then
    it
  else
...
daeExpMatrix(Exp exp, Context context, Text preExp /*BUFP*/,
  ...
  case m as MATRIX then
    ...
    # catAlloc = (m.scalar of row:
                   # tmp = tempDecl(arrayTypeStr, varDecls /*BUFC*/)
                   # vars = daeExpMatrixRow(row, arrayTypeStr, context, promote /*BUFC*/, varDecls /*BUFC*/)
                   # vars2 += ', &<tmp>'
...
daeExpAsub(Exp exp, Context context, Text preExp /*BUFP*/,
  ...
  case ASUB(exp=cref as CREF, sub=subs) then
    # arrName = daeExpCrefRhs(buildCrefExpFromAsub(cref, subs), context, preExp /*BUFC*/, varDecls /*BUFC*/)
    if context is SIMULATION then
      arrayScalarRhs(cref.ty, subs, arrName, context, preExp /*BUFC*/, varDecls /*BUFC*/)
    else
      '<arrName>'
  case _ then
    'OTHER_ASUB'
...
case VALUEBLOCK then
  ...
  # preExpRes = "" /*BUFD*/
  # varDeclsInner = "" /*BUFD*/
  # _ = (valueblockVars(it): varInit(it, "", 0, varDeclsInner /*BUFC*/, preExpInner /*BUFC*/) "\n")
  # resType = expTypeModelica(ty)
  # res = tempDecl(expTypeModelica(ty), preExp /*BUFC*/)
  # stmts = (body: '<algStatement(it, context, varDeclsInner /*BUFC*/)>' "\n")
  # expPart = daeExp(result, context, preExpRes /*BUFC*/, varDeclsInner /*BUFC*/)
  # preExp +=
  <<
  {
    <varDeclsInner>
...
arrayScalarRhs(ExpType ty, list<Exp> subs, String arrName, Context context,
  ...
  # dimsLenStr = listLength(subs)
  # dimsValuesStr = (subs of exp: daeExp(exp, context, preExp /*BUFC*/, varDecls /*BUFC*/) ", ")
  <<
  (*<arrayType>_element_addr(&<arrName>, <dimsLenStr>, <dimsValuesStr>))
  >>
...
case META_TUPLE then
  ...
  # args = (listExp of e: daeExpMetaHelperConstant(e, context, preExp /*BUFC*/, varDecls /*BUFC*/) ", ")
  # tmp = tempDecl("modelica_metatype", varDecls /*BUFC*/)
...
case METARECORDCALL then
  ...
  # argsStr = if args then
      ', <args of exp: daeExpMetaHelperConstant(exp, context, preExp /*BUFC*/, varDecls /*BUFC*/) ", ">'
      else ""
...
daeExpMetaHelperConstant(Exp e, Context context, Text preExp /*BUFP*/,
                         Text varDecls /*BUFP*/) ::=
  # expPart = daeExp(e, context, preExp /*BUFC*/, varDecls /*BUFC*/)
  daeExpMetaHelperConstantNameType(expPart, Exp.typeof(e), context, preExp /*BUFC*/, varDecls /*BUFC*/)
  ...
  case ET_COMPLEX(name=cname) then
    ...
    # args = if varLst then
      ', <varLst of v as COMPLEX_VAR(name=cvname):
           ...
           daeExpMetaHelperConstantNameType(nameText, tp, context, preExp /*BUFC*/, varDecls /*BUFC*/)
         ", ">'
      else ""
