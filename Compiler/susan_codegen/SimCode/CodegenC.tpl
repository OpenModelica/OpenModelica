...
case simCode as SIMCODE(__) then
  ...
  <%functionInitializeDataStruc_X_(modelInfo, fileNamePrefix, guid)%>
...
case "jacobianVars" then
  ...
  case SIMVAR(aliasvar=NOALIAS()) then
    <<
    #define <%cref(name)%> localData-><%arrayName%>[<%intAdd(offset,index)%>]
    >> 
  end match
case _ then
  match simVar
  case SIMVAR(arrayCref=SOME(c),aliasvar=NOALIAS()) then
    <<
    #define <%cref(c)%> localData-><%arrayName%>[<%intAdd(offset,index)%>]
    #define $P$PRE<%cref(c)%> localData-><%arrayName%>Pre[<%intAdd(offset,index)%>]
    #define <%cref(name)%> localData-><%arrayName%>[<%intAdd(offset,index)%>]
    #define $P$old<%cref(name)%> oldlocalData-><%arrayName%>[<%intAdd(offset,index)%>]
    #define $P$old2<%cref(name)%> old2localData-><%arrayName%>[<%intAdd(offset,index)%>]
    #define $P$PRE<%cref(name)%> localData-><%arrayName%>Pre[<%intAdd(offset,index)%>]
    >>
  case SIMVAR(aliasvar=NOALIAS()) then
    <<
    #define <%cref(name)%> localData-><%arrayName%>[<%intAdd(offset,index)%>]
...
template functionInitializeDataStruc_X_(ModelInfo modelInfo, String fileNamePrefix, String guid)
 "Generates function in simulation file."
::=
  <<
  ...
  >>
end functionInitializeDataStruc_X_;
