...
case MODELINFO(varInfo=VARINFO(__)) then
  ...
  data->modelData.nJacobianVars = <%varInfo.numJacobianVars%>;
...
case MODELINFO(varInfo=VARINFO(numStateVars=numStateVars), vars=SIMVARS(__)) then
  ...
  /* States */
  <%vars.stateVars |> var =>
    globalDataVarDefine_X_(var, "realVars",0)
  ;separator="\n"%>
  /* StatesDerivatives */
  <%vars.derivativeVars |> var =>
    globalDataVarDefine_X_(var, "realVars",numStateVars)
  ;separator="\n"%>
  /* Algebraic Vars */
  <%vars.algVars |> var =>
    globalDataVarDefine_X_(var, "realVars",intMul(2,numStateVars))
  ;separator="\n"%>
  /* Algebraic Parameter */
  <%vars.paramVars |> var =>
  ...
  /* External Objects */
  <%vars.extObjVars |> var =>
    globalDataParDefine_X_(var, "extObjs")
  ;separator="\n"%>
  /* Algebraic Integer Vars */
  <%vars.intAlgVars |> var =>
    globalDataVarDefine_X_(var, "integerVars",0)
  ;separator="\n"%>
...
template globalDataVarInfoArray_X_(String _name, list<SimVar> items, Integer offset)
...
template globalDataParDefine_X_(SimVar simVar, String arrayName)
...
template globalDataVarDefine_X_(SimVar simVar, String arrayName, Integer offset)
...
case "jacobianVars" then
  ...
  let tmp = System.tmpTick()
    <<
    #define <%cref(name)%> data->simulationInfo.<%arrayName%>[<%intAdd(offset,index)%>]
...
case _ then
 ...
 int functionJac<%matrixname%>(_X_DATA* data, double* jac){
    ...
    for(i=0,k=0;  i < <%index_%>;i++){
      ...
      functionJac<%matrixname%>_0(data, seed, localtmp);
      seed[i] = 0;
...
case SIMVAR(__) then
  ...
  <ScalarVariable 
    ...
    <%ScalarVariableType(type_,unit,displayUnit,initialValue,isFixed,nominalValue)%>
  </ScalarVariable>  
  >>
end ScalarVariable;
...
template ScalarVariableType(DAE.Type type_, String unit, String displayUnit, Option<DAE.Exp> initialValue, Boolean isFixed, Option<DAE.Exp> nominalValue)
 "Generates code for ScalarVariable Type file for FMU target."
::=
  match type_
    case T_INTEGER(__) then '<Integer <%ScalarVariableTypeCommonAttribute(initialValue,isFixed)%> <%ScalarVariableTypeNominalAttribute(nominalValue)%> <%ScalarVariableTypeRealAttribute(unit,displayUnit)%> />' 
    case T_REAL(__) then '<Real <%ScalarVariableTypeCommonAttribute(initialValue,isFixed)%> <%ScalarVariableTypeNominalAttribute(nominalValue)%> <%ScalarVariableTypeRealAttribute(unit,displayUnit)%> />' 
    case T_BOOL(__) then '<Boolean <%ScalarVariableTypeCommonAttribute(initialValue,isFixed)%> <%ScalarVariableTypeNominalAttribute(nominalValue)%> <%ScalarVariableTypeRealAttribute(unit,displayUnit)%> />' 
    case T_STRING(__) then '<String <%ScalarVariableTypeCommonAttribute(initialValue,isFixed)%> <%ScalarVariableTypeNominalAttribute(nominalValue)%> <%ScalarVariableTypeRealAttribute(unit,displayUnit)%> />' 
    case T_ENUMERATION(__) then '<Integer <%ScalarVariableTypeCommonAttribute(initialValue,isFixed)%> <%ScalarVariableTypeNominalAttribute(nominalValue)%> <%ScalarVariableTypeRealAttribute(unit,displayUnit)%> />' 
    else 'UNKOWN_TYPE'
