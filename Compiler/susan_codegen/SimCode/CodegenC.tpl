...
case SIMCODE(modelInfo=MODELINFO(__), extObjInfo=EXTOBJINFO(__)) then
  ...
  // Generated by soon redesined CodegenC 
...
case SIMCODE(modelInfo=MODELINFO(__), makefileParams=MAKEFILE_PARAMS(__), simulationSettingsOpt = sopt) then
  ...
  LDFLAGS=-L"<%makefileParams.omhome%>/lib/omc2" -lSimulationCRuntime <%makefileParams.ldflags%>
...
case _ then 
  <<
  /* CodegenC.tpl template: writeLhsCref: UNHANDLED LHS 
...
::=
  ...
  case ecr then
    ...
    <<
    /* CodegenC.tpl template: daeExpCrefLhs2: UNHANDLED EXPRESSION: 
