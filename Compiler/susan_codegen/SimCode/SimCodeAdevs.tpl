...
case SIMCODE(modelInfo = MODELINFO(varInfo = vi as VARINFO(__))) then
  ...
  { 
      public:
		  ...
		  */
         <%lastIdentOfPath(modelInfo.name)%>(int extra_state_events = 0); 
		 /// Destructor
         ~<%lastIdentOfPath(modelInfo.name)%>();
         /// Index of the first extra state event
		 int numStateEvents() const { return <%vi.numZeroCrossings%>; }
		 /**
...
// Below here is from the C template and is unchanged
...
::=
  ...
  case CALL(path=IDENT(name="sample"), expLst={start, interval}) then
    ...
    <<
    <%preExp%>
    ADEVS_ZEROCROSSING(<%index1%>,Sample(*t,<%e1%>,<%e2%>));
    >>
  else
    <<
    ZERO CROSSING ERROR
