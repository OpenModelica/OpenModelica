...
public function generateExpression_new "function: generateExpression
...
protected function relOpStr_new
  input DAE.Operator op;
  output String opStr;
algorithm
	isReal := matchcontinue (op)
	  case (DAE.LESS(ty = _)) then "RELESS";
	  case (DAE.LESSEQ(ty = _)) then "RELESSEQ";
	  case (DAE.GREATER(ty = _)) then "REGREATER";
	  case (DAE.GREATEREQ(ty = _)) then "REGREATEREQ";
	  case (_) then fail();
  end matchcontinue;
end relOpStr_new;
