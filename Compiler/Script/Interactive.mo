...
algorithm
  ...
  matchcontinue (inValue,inExp,inAbsynAlgorithmItemLst,inSymbolTable,info)
    ...
    case (Values.BOOL(boolean = true),exp,algitemlst,st,_)
      ...
      then
        st_3;
    case (Values.BOOL(_), _,_,st,_) // An error occured when evaluating the algorithm items
      then
        st;
    case (value,exp,_,st,_) // The condition value was not a boolean
      equation
        (estr,_) = stringRepresOfExpr(exp, st);
...
algorithm
  ...
  matchcontinue (inValue,inExp,inAbsynAlgorithmItemLst,inTplAbsynExpAbsynAlgorithmItemLstLst,inSymbolTable,info)
    ...
    case (Values.BOOL(boolean = false),_,algitemlst,algrest,st,_)
      ...
      then
        st_1;
    case (value,exp,_,_,st,_) /* Report type error */
      equation
        (estr,_) = stringRepresOfExpr(exp, st);
        vtype = Types.typeOfValue(value);
        tstr = Types.unparseType(vtype);
    ...
    case ((IVAR(varIdent = id,value = v,type_ = tp) :: rest),env)
      equation
        ...
        env_1 = Env.updateFrameV(env,
          DAE.TYPES_VAR(id,DAE.dummyAttrVar,tp,DAE.VALBOUND(v,DAE.BINDING_FROM_DEFAULT_VALUE()),NONE()), Env.VAR_TYPED(), {});
    ...
    case ((IVAR(varIdent = id,value = v,type_ = tp) :: rest),env)
      equation
        ...
        env_1 = Env.extendFrameV(env,
          DAE.TYPES_VAR(id,DAE.dummyAttrVar,tp,DAE.VALBOUND(v,DAE.BINDING_FROM_DEFAULT_VALUE()),NONE()),NONE(), Env.VAR_UNTYPED(), {});
        env_2 = addVarsToEnv(rest, env_1);
      then
        env_2;
    case ({},env) then env;
...
algorithm
  found := matchcontinue(cr,lst)
    ...
    case (_,Absyn.MODIFICATION(componentRef = cr2)::_)
      equation
...
algorithm
  outString := matchcontinue (inAbsynElementArgLst,id,f)
    ...
    case (((ann as Absyn.MODIFICATION(componentRef = Absyn.CREF_IDENT(name = id1),modification = mod)) :: _),id2,_)
      equation
        true = stringEq(id1, id2);
        str = f(mod);
      then
...
algorithm
  outStringLst := matchcontinue (inElArgLst,info,inClass,inFullProgram,inModelPath)
    ...
    case (Absyn.MODIFICATION(componentRef = Absyn.CREF_IDENT(annName,_), modification = SOME(Absyn.CLASSMOD(mod,_))) :: rest,_,_, _, _)
      ...
      then
        (gexpstr :: res);
    case (Absyn.MODIFICATION(componentRef = Absyn.CREF_IDENT(annName,_), modification = SOME(Absyn.CLASSMOD(mod,Absyn.NOMOD()))) :: rest,_,_,_,_)
      equation
