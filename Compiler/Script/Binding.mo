  ...
  (result) := matchcontinue(elems)
   local
    ...
    case SCode.EXTENDS(p, _, _, _, _)::_
      equation
        true = (Absyn.pathString(p) == typeName);
        then (true);
    case _::rest
   ...
   case {}
    ...
    case (Absyn.ELEMENTITEM(Absyn.ELEMENT(finalPrefix,redeclareKeywords,innerOuter,
      ...
      algorithm
          ...
          if ((Absyn.typeSpecPathString(rootType) == Absyn.typeSpecPathString(tSpec)) and not(listEmpty(exp2))) then
...
algorithm
  ...
  local
    ...
    case name::rest then Absyn.MODIFICATION(false, Absyn.NON_EACH(),
          Absyn.stringPath(name), SOME(Absyn.CLASSMOD({}, Absyn.EQMOD(exp, Absyn.dummyInfo))), NONE(), Absyn.dummyInfo)::buildComponentModifiers2(rest, exp);
        end match;
      ...
      algorithm
         ...
         // if Interactive.isPrimitive(Absyn.pathToCref(path), env) then fail(); end if;
          ...
          if (Absyn.typeSpecPathString(typeSpec) == className) then
           ...
           //print("... NOT found provider " + Absyn.typeSpecPathString(typeSpec) + "\n");
          cnew := in_components;
          end if;

          re_items := Absyn.getElementItemsInClass(def); // if the provider is an element from the class
  ...
  val := match (predecessors)
   local
    ...
    case CLIENT_E(_, _, ots, _, _, _, _):: rest
        then if Absyn.typeSpecEqual(ts, ots) then true else isAlreadyInList(ts, rest);
   end match;
end isAlreadyInList;
...
algorithm
    ...
    local
      ...
      case {} then {};
      case ci::r then Absyn.componentName(ci)::getComponentNames(r);
    end match;
    ...
    case (Absyn.ELEMENTITEM(Absyn.ELEMENT(_,_,_, Absyn.COMPONENTS(_,typeSpec, components), _, _))::rest)
      algorithm
          ...
          // TODO: check if it was not already added to the list
           ...
           //print("---------------> L1" + Absyn.pathString(path) + "\n");
