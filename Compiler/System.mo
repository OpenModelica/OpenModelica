...
public function stringFind "locates substring searchStr in str. If succeeds return position, otherwise return -1"
  ...
  external "C" outInteger=System_stringFind(str,searchStr);
end stringFind;
...
public function stringReplace
  ...
  external "C" res=System_stringReplace(str,source,target);
end stringReplace;
...
"Does not fail. Returns strings describing the error instead."
  ...
  external "C" outString = System_readFile(inString);
end readFile;
...
public function regularFileExists
  ...
  external "C" outBool = System_regularFileExists(inString);
