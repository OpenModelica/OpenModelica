...
algorithm
  outEquations := matchcontinue(inConnections, inFlowVariables)
    ...
    case (Connect2.CONNECTIONS(connections, expconnl, _, _), _)
      equation
        // Create set structure. TODO: Better set size?
        set_size = ConnectUtil2.connectionCount(inConnections) + listLength(inFlowVariables);
        disjoint_sets = ConnectionSets.emptySets(set_size);

        // Add flow variables to the set structure.
        flows = List.mapFlatReverse(inFlowVariables, ConnectUtil2.expandConnector);
        ...
        expconnl = ExpandableConnectors.elaborate(expconnl);
        /*-------------------------------------------------------------------*/
...
algorithm
  outSets := matchcontinue(inConnection, inSets)
    ...
    case (Connect2.CONNECTION(lhs = Connect2.CONNECTOR(attr =
      ...
      then ConnectionSets.expandAddConnection(inConnection, inSets);
...
algorithm
  outDae := matchcontinue(inElements)
    ...
    case {_} then DAEUtil.emptyDae;
      ...
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        str = stringDelimitList(List.map(inElements, ConnectUtil2.connectorStr), ", ");
...
algorithm
  (outEquations, outIsOnlyConst) := matchcontinue(inLhsConnector, inRhsConnector,
    ...
    case (Connect2.CONNECTOR(ty = lhs_ty), Connect2.CONNECTOR(ty = rhs_ty), _, _)
      equation
        false = ConnectUtil2.isConstOrComplexConnector(inLhsConnector);
        false = ConnectUtil2.isConstOrComplexConnector(inRhsConnector);
      then
        (inEquations, false);
...
algorithm
  lhs_conn := ConnectUtil2.varToConnector(inLhsVar, inLhsCref, Connect2.INSIDE());
  rhs_conn := ConnectUtil2.varToConnector(inRhsVar, inRhsCref, Connect2.INSIDE());
