...
algorithm
  (outCache,outEnv,outIH,outDae) := match (inCache,inEnv,inIH,inProgram)
    ...
    case (cache, env, ih, {}) then (cache, env, ih, DAE.emptyDae);
...
algorithm
  ...
  matchcontinue (inCache,inEnv,inIH,inStore,inMod,inPrefix,inClass,inInstDims,inBoolean,inCallingScope,inGraph,inSets)
    ...
    case (cache,env,ih,store,mod,pre,
          ...
          inst_dims,impl,callscope,graph,_)
      equation
        recursionDepthReached = listLength(env) < Global.recursionDepthLimit;
  ...
  matchcontinue (inCache,inEnv,inIH,inStore,inMod2,inPrefix3,inState5,className,inClassDef6,inRestriction7,inVisibility,inPartialPrefix,inEncapsulatedPrefix,inInstDims9,inBoolean10,inCallingScope,inGraph,inSets,instSingleCref,info,stopInst)
    ...
    case (cache,env,ih,store,mods,pre,ci_state,_,
      ...
      equation
        ...
        (cdefelts,extendsclasselts,extendselts as _::_,compelts as {}) = splitElts(els);        
        ...
        (cache,_,_,_,extcomps,eqs2 as {},initeqs2 as {},alg2 as {},initalg2 as {}) =
        ...
        (_, _, _, {}) = splitElts(compelts_2_elem);
    ...
    case (cache,env,ih,store,mods,pre,ci_state,_,
      ...
      equation
        ...
        (compelts_1, eqs_1, initeqs_1, alg_1, initalg_1) = 
          extractConstantPlusDepsTpl(compelts_1, instSingleCref, {}, className, eqs_1, initeqs_1, alg_1, initalg_1);
...
protected function extractConstantPlusDepsTpl "
    ...
    case (cache,env,ih, (tSpec as Absyn.TCOMPLEX(p,_,_)) :: restTypeSpecs,pre,dims,impl,localAccTypes,_)
      equation
        ...
        c = SCode.CLASS(id,SCode.defaultPrefixes,
                        ...
                        SCode.noComment,
                        Absyn.dummyInfo);
        (cache,cenv,ih,_,_,csets,ty,_,oDA,_)=instClass(cache,env,ih,UnitAbsyn.noStore,DAE.NOMOD(),pre,c,dims,impl,InstTypes.INNER_CALL(), ConnectionGraph.EMPTY, inSets);
        localAccTypes = ty::localAccTypes;
        (cache,env,ih,localAccTypes,csets,_) =
        instClassDefHelper(cache,env,ih,restTypeSpecs,pre,dims,impl,localAccTypes, csets);
      then (cache,env,ih,localAccTypes,csets,oDA);
  ...
  matchcontinue (inCache,inEnv,inIH,inStore,inSCodeElementLst2,inSCodeElementLst3,inMod4,inInstDims5,className,info,stopInst)
    ...
    case (cache,env,ih,store,{SCode.EXTENDS(baseClassPath = path,modifications = mod)},{},mods,inst_dims,_,_,_)
      equation
        ...
        cenv = Env.addModification(cenv, Env.M(Prefix.NOPRE(), className, {}, m_2, env, inst_dims));
...
algorithm 
  ...
  local
    ...
    case (cache,env,ih,_,{SCode.EXTENDS(baseClassPath = path,modifications = mod)},(_ :: _),mods,inst_dims,_,_,_) /* Inherits baseclass -and- has components */
      equation
        (cache,m_1) = Mod.elabModForBasicType(cache, env, ih, Prefix.NOPRE(), mod, true, info);
        (cache,cdef,cenv) = Lookup.lookupClass(cache,env, path, true);
  ...
  matchcontinue (inCache, inEnv, inIH, inUnitStore, inMod, inPrefix, inState,
    ...
    case (cache, env, ih, store, mods, pre, ci_state,
      ...
      equation
        ...
        cenv = Env.mergeEnv(cenv, env2, name, cls, Env.M(pre, name, ad, mod_1, env2, inst_dims));
        cenv = Env.addModification(cenv, Env.M(pre, name, ad, mod_1, env2, inst_dims));
        ...
        (comp_env, env2) = Env.splitEnv(comp_env, env2, name);
  ...
  matchcontinue (inCache,inEnv,inIH,inStore,inState,inMod,inPrefix,inName,inClass,inAttributes,inPrefixes,inDimensions,inSubscripts,inInstDims,inImpl,inComment,inInfo,inGraph,inSets)
    ...
    case (cache,env,ih,store,ci_state,mod,pre,n,cl,attr,pf,dims,idxs,inst_dims,impl,comment,info,graph,csets)
      equation
        ...
        (cache,env_1,ih,ci_state,vars) = partialInstClassIn(cache, env, ih, mod, pre, ci_state, cl, SCode.PUBLIC(), inst_dims, 0);
        dae = DAE.emptyDae;
        (cache, path) = makeFullyQualified(cache, env, Absyn.IDENT(n));
        ty = DAE.T_COMPLEX(ci_state, vars, NONE(), {path});
        ty = makeArrayType(dims, ty);
      then
     ...
     case (cache,env,ih,store,ci_state,mod as DAE.MOD(subModLst = subMods, eqModOption = NONE()),pre,n,cl as SCode.CLASS(restriction = SCode.R_RECORD()),attr,pf,dims,idxs,inst_dims,impl,comment,info,graph,csets)
      equation
        ...
        (cache,env_1,ih,store,dae1,csets,ty,st,_,graph) =
          instClass(cache, env, ih, store, /* mod */ DAE.NOMOD(), pre, cl, inst_dims, impl, InstTypes.INNER_CALL(), graph, csets);
    ...
    case (cache,env,ih,store,ci_state,mod,pre,n,(cl as SCode.CLASS(name=n2)),attr,pf,dims,idxs,inst_dims,impl,comment,info,graph,csets)
       equation
        ...
        (cache,env_1,ih,store,dae1,csets,ty,st,_,_) =
          instClass(cache, env, ih, store, mod, pre, cl, inst_dims, impl, InstTypes.INNER_CALL(), ConnectionGraph.EMPTY, csets);
  ...
  matchcontinue (inCache,inEnv,inCenv,inIH,pre,path,name,ad,cl,attr,inPrefixes,dattr,info,m,
    ...
    case (cache,env,cenv,ih,_,_,_,_,_,_,_,_,_,_,_,mod,_,_,_,updatedComps)
      equation
        ...
        // Instantiate the component */
        cenv = Env.mergeEnv(cenv, env, name, cls, Env.M(pre, name, ad, mod_3, env, {}));
        ...
        (compenv, env) = Env.splitEnv(compenv, env, name);
    ...
    case (cache,env,cenv,ih,_,_,_,_,_,_,_,_,_,_,_,mod,_,_,_,updatedComps)
      equation
        ...
        // Instantiate the component */
        cenv = Env.mergeEnv(cenv, env, name, cls, Env.M(pre, name, ad, m_1, env, {}));
        (cache,compenv,ih,_,_,_,ty,_) =
    ...
    case (cache,env,ih,store,(ci_state as ClassInf.FUNCTION(path = _)),mod,pre,n,(cl,attr),pf,i,dim,dims,idxs,inst_dims,impl,comment,_,graph, csets)
      equation
        ...
        (cache,env_1,ih,store,dae1,_,ty,st,_,graph) =
          instClass(cache,env,ih,store, mod, pre, cl, inst_dims, true, InstTypes.INNER_CALL(), graph, csets) "Which has an expression binding";
...
algorithm
  (outCache,outPath) := matchcontinue (inCache,inEnv,inPath)
    ...
    /*/ do not qualify functions or records!
    case (cache,env,path)
      equation
        (cache,cl as SCode.CLASS(name = name),env_1) = Lookup.lookupClass(cache, env, path, false);
        true = SCode.isFunction(cl) or SCode.isRecord(cl);
      then
        (cache,path);*/
  ...
  matchcontinue (inCache,inEnv,inIH,inElement,outerMod,inImplicit)
    ...
    case (cache,env,ih,
      ...
      equation
        ...
        (cache,compenv,ih,_,_,_,tp_1,_) = instVar(cache, cenv, ih, UnitAbsyn.noStore, ClassInf.FUNCTION(Absyn.IDENT(""), false), mod_1, Prefix.NOPRE(),
...
protected type CachedPartialInstItemOutputs = tuple<Env.Env, ClassInf.State, list<DAE.Var>>;
