...
encapsulated package EnvExtends
" file:        EnvExtends.mo
  package:     EnvExtends
  ...
  SCodeEnv.extendEnvWithExtends, which is used to index the array.
  ...
  details. When e.g. SCodeLookup.lookupBaseClassName encounters such a path it
  ...
  SCodeFlattenRedeclare. See the comment in SCodeFlattenRedeclare for more
  information about this.

  UPDATE CLASS EXTENDS:
  Class extends are handled by initially adding them to the environment with
  SCodeEnv.extendEnvWithClassExtends. This function add the given class as a
  ...
  BASE_CLASS_SUFFIX defined in SCodeEnv. The alias can then be safely redeclared
...
protected type AvlTree = SCodeEnv.AvlTree;
protected type AvlTreeValue = SCodeEnv.AvlTreeValue;
protected type ClassType = SCodeEnv.ClassType;
protected type Extends = SCodeEnv.Extends;
protected type Frame = SCodeEnv.Frame;
...
algorithm
  outEnv := matchcontinue(inEnv)
    ...
    case (_)
      equation
        ext_count = System.tmpTickIndex(SCodeEnv.extendsTickIndex);
        ext_table = createExtendsTable(ext_count);
      then
        qualify2(inEnv, SCodeEnv.USERDEFINED(), ext_table);

    else
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        Debug.traceln("- EnvExtends.qualify failed.");
      then
        fail();
...
algorithm
  ...
  outEnv := SCodeEnv.setEnvClsAndVars(tree, env);
end qualify2;
...
algorithm
  outTree := match(inTree, inEnv, inExtendsTable)
    ...
    case (SOME(SCodeEnv.AVLTREENODE(SOME(SCodeEnv.AVLTREEVALUE(
        name, SCodeEnv.CLASS(cls, {cls_env}, cls_ty))), h, left, right)), _, _)
      equation
        ...
        item = SCodeEnv.CLASS(cls, {cls_env}, cls_ty);
        value = SOME(SCodeEnv.AVLTREEVALUE(name, item));
      then
        SOME(SCodeEnv.AVLTREENODE(value, h, left, right));

     case (SOME(SCodeEnv.AVLTREENODE(value, h, left, right)), _, _)
       equation
...
protected
  ...
  Option<SCode.Element> cei;
algorithm
  SCodeEnv.EXTENDS_TABLE(exts, re, cei) := SCodeEnv.getEnvExtendsTable(inEnv);
  exts := qualifyExtendsList(exts, inClassType, inEnv, inExtendsTable);
  outEnv := SCodeEnv.setEnvExtendsTable(SCodeEnv.EXTENDS_TABLE(exts, re, cei), inEnv);
...
algorithm
  outExtends := match(inExtends, inClassType, inEnv, inExtendsTable)
    ...
    case (ext :: extl, SCodeEnv.CLASS_EXTENDS(), _, _)
      equation
...
algorithm
  outExtends := matchcontinue(inExtends, inEnv, inExtendsTable)
    ...
    // need to do anything.
    case (SCodeEnv.EXTENDS(baseClass = Absyn.IDENT(name = id)), _, _)
      equation
    ...
    case (SCodeEnv.EXTENDS(baseClass = bc), _, _)
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        Debug.traceln("- EnvExtends.qualifyExtends failed on " +&
          Absyn.pathString(bc) +& "\n");
      then
        fail();
...
algorithm
  outExtends := matchcontinue(inExtends, inEnv, inExtendsTable)
    local
      Absyn.Path bc;
      list<SCodeEnv.Redeclaration> rl;
      Integer index;
      Absyn.Info info;
      Extends ext;
      Env env;

    case (SCodeEnv.EXTENDS(index = index), _, _)
      then lookupQualifiedExtends(index, inExtendsTable);

    case (SCodeEnv.EXTENDS(bc, rl, index, info), _, _)
      equation
        ...
        ext = SCodeEnv.EXTENDS(bc, rl, index, info); 
...
algorithm
  outErrorPath := matchcontinue(inIsFirst, inFromExtends, inPartName, inItem,
    ...
    case (_, _, _, SCodeEnv.CLASS(cls = SCode.CLASS(prefixes = SCode.PREFIXES(
    ...
    case (_, _, _, SCodeEnv.CLASS(cls = _), _,
        SCodeEnv.FRAME(clsAndVars = SCodeEnv.AVLTREENODE(value = NONE(),
      ...
      equation
        ...
        // the look up process.
        {} = SCodeEnv.getEnvExtendsFromTable(inOriginEnv);
      then
        NONE();
    ...
    case (_, _, _, SCodeEnv.CLASS(cls = _), _, _, _)
      equation
    ...
    case (_, _, _, SCodeEnv.CLASS(cls = SCode.CLASS(prefixes =
        SCode.PREFIXES(replaceablePrefix = SCode.REPLACEABLE(cc = _)))), _, _, _)
      equation
        part = SCodeEnv.mergePathWithEnvPath(inPartName, inFoundEnv);
      then 
    ...
    // allowed either.
    case (_, _, _, SCodeEnv.VAR(var = _), _, _, _)
      equation
        part = SCodeEnv.mergePathWithEnvPath(inPartName, inFoundEnv);
      then
  ...
  _ := matchcontinue(inErrorPath, inEnv, inInfo)
    ...
    case (Absyn.QUALIFIED(name = "$E",
      ...
      equation
        ...
        env = SCodeEnv.removeExtendFromLocalScope(inErrorPath, inEnv);
      ...
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        Debug.traceln("- EnvExtends.printExtendsError failed to print error " +&
  ...
  _ := matchcontinue(inError, inBaseClass, inPartPath, inEnv, inInfo)
    ...
    case (_, _, _, _, _)
      equation
        ...
        (SCodeEnv.CLASS(cls = SCode.CLASS(name = part, info = info)), _, _) =
          SCodeLookup.lookupFullyQualified(inPartPath, inEnv);
        bc_str = Absyn.pathString(inBaseClass);
    ...
    case (_, _, _, _, _)
      equation
        ...
        (SCodeEnv.VAR(var = SCode.COMPONENT(name = part, info = info)), _, _) =
          SCodeLookup.lookupFullyQualified(inPartPath, inEnv);
        bc_str = Absyn.pathString(inBaseClass);
...
algorithm
  _ := matchcontinue(inName, inExtends, inEnv)
    ...
    case (_, (ext as SCodeEnv.EXTENDS(baseClass = bc, info = info2)) :: rest_ext, _)
      equation
        (SOME(item), _, _) = SCodeLookup.lookupInBaseClasses3(inName, ext,
          inEnv, inEnv, SCodeLookup.IGNORE_REDECLARES(), {});
        info1 = SCodeEnv.getItemInfo(item);
        SCodeEnv.EXTENDS(baseClass = bc, info = info2) = ext;
...
algorithm
  ...
  matchcontinue(inName, inEnv, inExtendsTable)
    ...
    case (_, SCodeEnv.FRAME(frameType = frame_type) :: env, _)
      equation
        SCodeLookup.frameNotEncapsulated(frame_type);
...
algorithm
  ...
  matchcontinue(inName, inEnv, inExtendsTable)
    ...
    case (_, _, _)
      equation
        (item, env) = SCodeLookup.lookupInClass(inName, inEnv);
      then
        (SOME(item), SOME(Absyn.IDENT(inName)), SOME(env), false);

    case (_, SCodeEnv.FRAME(extendsTable = SCodeEnv.EXTENDS_TABLE(
  ...
  matchcontinue(inName, inExtends, inEnv, inExtendsTable)
    ...
    case (_, ext :: _, _, _)
      equation
        ...
        // classes.
        env = SCodeEnv.setImportTableHidden(inEnv, false);
...
algorithm
  (outItem, outEnv) := match(inName, inExtends, inEnv, inExtendsTable)
    ...
    case (_, SOME(SCodeEnv.EXTENDS(baseClass = Absyn.FULLYQUALIFIED(bc))), _, _)
      equation
        ...
        env = SCodeEnv.setImportTableHidden(env, true);
  ...
  (outItem, outPath, outEnv) := matchcontinue(inName, inImports, inEnv, inExtendsTable)
    ...
    case (_, Absyn.NAMED_IMPORT(name = name, path = path) :: _, _, _)
      equation
        ...
        path = SCodeEnv.prefixIdentWithEnv(inName, env);
        path = Absyn.makeFullyQualified(path);
      then
...
algorithm
  outExtends := match(inExtends, inExtendsTable)
    ...
    case (UNQUALIFIED_EXTENDS(ext = SCodeEnv.EXTENDS(baseClass = bc)), _)
      then NONE();
...
algorithm
  SCodeEnv.FRAME(name, ty, tree,
    SCodeEnv.EXTENDS_TABLE(bcl, re, _), imps, iu) :: rest_env := inEnv;
  SOME(tree) := update3(SOME(tree), inEnv);
  env := SCodeEnv.FRAME(name, ty, tree,
    SCodeEnv.EXTENDS_TABLE(bcl, {}, NONE()), imps, iu) :: rest_env;
...
algorithm
  outTree := match(inTree, inEnv)
    local
      ...
      SCode.Element cls;
      Frame cls_env;
      Option<SCodeEnv.AvlTreeValue> value;
      Item item;
      ClassType cls_ty;
    ...
    case (SOME(SCodeEnv.AVLTREENODE(SOME(SCodeEnv.AVLTREEVALUE(
        name, SCodeEnv.CLASS(cls, {cls_env}, cls_ty))), h, left, right)), _)
      equation
    ...
    case (SOME(SCodeEnv.AVLTREENODE(value, h, left, right)), _)
      equation
        // Recurse into left and right branch of the tree.
  ...
  (outPath, outItem) := matchcontinue(inName, inEnv, inInfo)
    ...
    case (_, _, _)
      equation
        ...
        (item, _) = SCodeLookup.lookupInheritedName(basename, inEnv);
    ...
    case (_, _, _)
      equation
        (item, _) = SCodeLookup.lookupInheritedName(inName, inEnv);
