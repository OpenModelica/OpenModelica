...
algorithm
  ...
  matchcontinue(inTypePath, inEntry, inMod, inPrefixes, inEnv, inPrefix, inInstPolicy, inGlobals)
    local
      //Absyn.ArrayDim dims;
      Absyn.Info info;
      //Absyn.Path path;
    ...
    case (_, NFEnv.ENTRY(element = SCode.CLASS(name = name, restriction = SCode.R_TYPE(),
        classDef = SCode.PARTS(elementLst = _))), _, _, _, _, ip, globals)
      equation
    ...
    case (_, NFEnv.ENTRY(element = SCode.CLASS(name = name, restriction = res,
      ...
      equation
        ...
        mel = NFMod.applyModifications(inMod, el, inPrefix, env);
        //exts = NFSCodeEnv.getEnvExtendsFromTable(env);

        (elems, cse, globals) = instElementList(mel, inPrefixes, env,
        ...
        cls = NFInstFlatten.flattenClass(cls, false);
...
protected function instElementList
  ...
  output Globals outGlobals;
algorithm
  (outElements, outContainsSpecialExtends, outGlobals) := instElementList2(
    inElements, inPrefixes, inEnv, inPrefix, inInstPolicy, {}, false, inGlobals);
end instElementList;
...
algorithm
  ...
  match(inElements, inPrefixes, inEnv, inPrefix, inInstPolicy, inAccumEl,
      inContainsSpecialExtends, inGlobals)
    local
    ...
    case (elem :: rest_el, _, env, _, _, accum_el, cse, globals)
      equation
        (accum_el, cse, globals) = instElement(elem, inPrefixes, env, inPrefix,
          inInstPolicy, accum_el, cse, globals);
        (accum_el, cse, globals) = instElementList2(rest_el, inPrefixes,
...
//          {"NFInst.instElementList2 has extends left!."});
//      then
//        fail();
//
//  end match;
...
algorithm
  (outElements, outContainsSpecialExtends, outGlobals) :=
  match(inElement, inPrefixes, inEnv, inPrefix, inInstPolicy, inAccumEl,
      inContainsSpecialExtends, inGlobals)
    local
      SCode.Element elem;
      Modifier mod;
    ...
    case ((elem as SCode.COMPONENT(name = _), mod), _, _, _, INST_ALL(), _, cse, globals)
      equation
        (res, globals) = instComponent(elem, mod, inPrefixes, inEnv, inPrefix,
            inInstPolicy, globals);
      then
        (res :: inAccumEl, cse, globals);

    else (inAccumEl, inContainsSpecialExtends, inGlobals);
...
//      equation
//        Error.addMessage(Error.INTERNAL_ERROR,
//          {"NFInst.instElement_dispatch ran out of extends!."});
//      then
//        fail();
//
...
protected function instComponent
  ...
  output Globals outGlobals;
algorithm
  (outElement, outGlobals) := match(inElement, inClassMod, inPrefixes, inEnv,
    ...
    case (SCode.COMPONENT(name = name, attributes = SCode.ATTR(arrayDims = ad),
      ...
      equation
        ...
        cmod = NFMod.propagateMod(inClassMod, dim_count);
        ...
        dim_arr = NFInstUtil.makeDimensionArray({});
...
//      equation
//        true = Flags.isSet(Flags.FAILTRACE);
//        Debug.traceln("NFInst.instElement failed on unknown element.\n");
//      then
//        fail();
//
//  end match;
...
//    else
//      equation
//        print("NFInst.instFunction failed: " +& Absyn.printComponentRefStr(inName) +&
