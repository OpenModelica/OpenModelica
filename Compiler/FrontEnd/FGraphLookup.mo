...
encapsulated package FGraphLookup
" file:        FGraphLookup.mo
  package:     FGraphLookup
  description: SCode flattening

  RCS: $Id: FGraphLookup.mo 14085 2012-11-27 12:12:40Z adrpo $
...
"Looks up a simple identifier in the graph and returns nodeId"
...
algorithm
  (SOME(outItem), SOME(outPath), SOME(outEnv)) :=
    lookupSimpleName2(inName, inEnv, inSearchInBase, {});
...
protected function lookupSimpleName2
  ...
  input Absyn.Ident inName;
  input Env inEnv;
  input Boolean inSearchInBase "if we should search inside base classes";
  input list<String> inVisitedScopes;
  output Option<Item> outItem;
  output Option<Absyn.Path> outPath;
  output Option<Env> outEnv;
algorithm
  (outItem, outPath, outEnv) := matchcontinue(inName, inEnv, inSearchInBase, inVisitedScopes)
    local
    ...
    case (_, _, _, _)
      equation
        (opt_item, opt_path, opt_env) =
          lookupInLocalScope(inName, inEnv, inSearchInBase, inVisitedScopes);
      then
        (opt_item, opt_path, opt_env);
    ...
    case (_, Env.FRAME(name = SOME(scope_name), frameType = frame_type) ::
      ...
      equation
        ...
        (opt_item, opt_path, opt_env) =
          lookupSimpleName2(inName, rest_env, inSearchInBase, scope_name :: inVisitedScopes);
      then
        (opt_item, opt_path, opt_env);
    ...
    case (_, Env.FRAME(frameType = Env.ENCAPSULATED_SCOPE()) ::
      ...
      equation
        ...
        (opt_item, opt_path, opt_env) = lookupSimpleName2(inName, rest_env, inSearchInBase, {});
...
algorithm
  _ := match(inItem)
    ...
    case (SOME(Env.CLASS(classType = Env.BASIC_TYPE()))) then ();
    case (NONE()) then ();
  end match;
...
algorithm
  (outItem, outPath, outEnv) := matchcontinue(inName, inEnv, inSearchInBase, inVisitedScopes)
    ...
    case (_, _, _, _)
      equation
        true = boolEq(inSearchInBase, bSearchInBase);
        (opt_item, opt_path, opt_env) =
    ...
    case (_, Env.FRAME(importTable =
        Env.IMPORT_TABLE(hidden = false, qualifiedImports = imps)) :: _, _, _)
      equation
        (opt_item, opt_path, opt_env) =
          lookupInQualifiedImports(inName, imps, inEnv, inSearchInBase);
      then
        (opt_item, opt_path, opt_env);

    // Look among the unqualified imports.
    case (_, Env.FRAME(importTable =
        Env.IMPORT_TABLE(hidden = false, unqualifiedImports = imps)) :: _, _, _)
      equation
        (item, path, env) =
          lookupInUnqualifiedImports(inName, imps, inEnv, inSearchInBase);
      then
    ...
    case (_, Env.FRAME(frameType = Env.IMPLICIT_SCOPE(iterIndex=_)) :: rest_env, _, _)
      equation
        (opt_item, opt_path, opt_env) =
          lookupInLocalScope(inName, rest_env, inSearchInBase, inVisitedScopes);
      then
        (opt_item, opt_path, opt_env);
...
protected
  AvlTree tree;
algorithm
  (outItem, outEnv) := matchcontinue(inName, inEnv)
  
    // return the top env as enclosing scope when searching in it! 
    case (_, {Env.FRAME(name = NONE())})
      equation 
    ...
    // otherwise return the same as before 
    case (_, _)
      equation 
        Env.FRAME(clsAndVars = tree) :: _ = inEnv;
        outItem = Env.avlTreeGet(tree, inName);
...
algorithm
  (outItem, outPath, outEnv) := match(inName, inBaseClass, inEnv,
    ...
    case (_, Env.EXTENDS(baseClass = bc, redeclareModifiers = redecls, info = info),
      ...
      equation
        // Find the base class.
        (item, path, env) = lookupBaseClassName(bc, inEnv, bSearchInBase, info);
...
algorithm
  (outItem, outPath, outEnv) := match(inName, inItem, inEnv)
    ...
    case (_, SOME(item), SOME(env))
      equation
        (item, path, env) = lookupNameInItem(inName, item, env, bSearchInBase);
      then
        (SOME(item), SOME(path), SOME(env));
...
public function lookupInQualifiedImports
  ...
  output Option<Env> outEnv;
algorithm
  (outItem, outPath, outEnv) := matchcontinue(inName, inImports, inEnv, inSearchInBase)
    local
      Absyn.Ident name;
      Absyn.Path path;
      Item item;
    ...
    case (_, Absyn.NAMED_IMPORT(name = name) :: rest_imps, _, _)
      equation
        ...
        (opt_item, opt_path, opt_env) =
          lookupInQualifiedImports(inName, rest_imps, inEnv, inSearchInBase);
      then
        (opt_item, opt_path, opt_env);
    ...
    case (_, Absyn.NAMED_IMPORT(name = name, path = path) :: _, _, _)
      equation
        ...
        (item, path, env) = lookupFullyQualified(path, inEnv, inSearchInBase);
      then
        (SOME(item), SOME(path), SOME(env));
  ...
  (outItem, outPath, outEnv) := matchcontinue(inName, inImports, inEnv, inSearchInBase)
    ...
    case (_, Absyn.UNQUAL_IMPORT(path = path) :: _, _, _)
      equation
        ...
        (item, path2, env) = lookupNameInItem(Absyn.IDENT(inName), item, env, inSearchInBase);
    ...
    case (_, _ :: rest_imps, _, _)
      equation
        (item, path, env) =
          lookupInUnqualifiedImports(inName, rest_imps, inEnv, inSearchInBase);
      then
        (item, path, env);
  end matchcontinue;
...
algorithm
  env := FEnv.getEnvTopScope(inEnv);
  (outItem, outPath, outEnv, _) := lookupNameInPackage(inName, env, inSearchInBase);
...
algorithm
  (outItem, outPath, outEnv, outOrigin) := match(inName, inEnv, inSearchInBase)
    local
      Absyn.Ident name;
    ...
    case (Absyn.IDENT(name = name), _, _)
      equation
        (SOME(item), SOME(path), SOME(env)) = lookupInLocalScope(name, inEnv, inSearchInBase, {});
        env = FEnv.setImportTableHidden(env, false);
    ...
    // Qualified name.
    case (Absyn.QUALIFIED(name = name, path = path), top_scope :: _, _)
      equation
        // Look up the name in the local scope.
        (SOME(item), SOME(new_path), SOME(env)) =
          lookupInLocalScope(name, inEnv, inSearchInBase, {});
        origin = itemOrigin(item);
        env = FEnv.setImportTableHidden(env, false);
        // Look for the rest of the path in the found item.
        (item, path, env) = lookupNameInItem(path, item, env, inSearchInBase);
...
algorithm
  (outItem, outCref) := match(inCref, inEnv)
    ...
    case (Absyn.CREF_IDENT(name = name, subscripts = subs), _)
      equation
        (SOME(item), SOME(new_path), _) = lookupInLocalScope(name, inEnv, bSearchInBase, {});
    ...
    case (Absyn.CREF_QUAL(name = name, subscripts = subs,
      ...
      equation
        ...
        (SOME(item), SOME(new_path), SOME(env)) =
          lookupInLocalScope(name, inEnv, bSearchInBase, {});
...
public function lookupNameInItem
  ...
  output Env outEnv;
algorithm
  (outItem, outPath, outEnv) := match(inName, inItem, inEnv, inSearchInBase)
    local
      Item item;
      Absyn.Path path;
      Frame class_env;
    ...
    case (_, Env.VAR(var = SCode.COMPONENT(typeSpec = type_spec,
        modifications = mods, info = info)), env, _)
      equation
        //env = FEnv.setImportTableHidden(env, false);
        // Look up the variable type.
        (item, _, type_env) = lookupTypeSpec(type_spec, env, inSearchInBase, info);
        ...
        (item, path, env) = lookupNameInItem(inName, item, type_env, inSearchInBase);
      then
        (item, path, env);

    // A class.
    case (_, Env.CLASS(env = {class_env}), _, _)
      equation
        // Look in the class's environment.
        env = FEnv.enterFrame(class_env, inEnv);
        (item, path, env, _) = lookupNameInPackage(inName, env, inSearchInBase);
      then
        (item, path, env);

    case (_, Env.REDECLARED_ITEM(item = item, declaredEnv = env), _, _)
      equation
        (item, path, env) = lookupNameInItem(inName, item, env, inSearchInBase);
      then
        (item, path, env);
  ...
  (outItem, outCref) := match(inCref, inItem, inEnv)
    ...
    case (_, Env.VAR(var = SCode.COMPONENT(typeSpec = type_spec,
      ...
      equation
        ...
        (item, _, type_env) = lookupTypeSpec(type_spec, inEnv, bSearchInBase, info);
        // Apply redeclares to the type and look for the name inside the type.
        redeclares = FFlattenRedeclare.extractRedeclaresFromModifier(mods);
        (item, type_env, _) = FFlattenRedeclare.replaceRedeclaredElementsInEnv(
          redeclares, item, type_env, inEnv, NFInstTypes.emptyPrefix);
  ...
  outResult := matchcontinue(inBaseClass, inName, inEnv, inAccum)
    ...
    case (Env.EXTENDS(baseClass = bc, redeclareModifiers = redecls,
      ...
      equation
        ...
        (item, _, env) = lookupBaseClassName(bc, inEnv, bSearchInBase, info);
        ...
        (item, _, _) = lookupNameInItem(Absyn.IDENT(inName), item, env, bSearchInBase);
        (items, bcl) = inAccum;
      then
        ((item :: items, bc :: bcl));
  ...
  (outItem, outEnv) := matchcontinue(inItem, inEnv, inInfo)
    ...
    // outputs its own errors.
      ...
      equation
        ...
        Debug.traceln("- FGraphLookup.lookupRedeclaredClassByItem failed on " +&
...
algorithm
  (outItem, outEnv) :=
    ...
    case (Env.VAR(var = SCode.COMPONENT(name = name, info = info)), _, _, _, _)
      ...
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        Debug.traceln("- FGraphLookup.lookupRedeclaredClass2 failed on " +&
            FEnv.getItemName(inItem) +& " in " +&
            FEnv.getEnvName(inEnv));
      then
        fail();
...
protected function lookupBuiltinName
  input String inName;
  input Env inEnv;
  output Env.Item outItem;
  output Env outEnv;
algorithm
  (outItem, outEnv) := match(inName, inEnv)
    local
      Env.Item item;
      Env env;
    ...
    case (_, _)
      equation
        true = listMember(inName, {"Real", "Integer", "Boolean", "String"});
        env = FEnv.getEnvTopScope(inEnv);
        (item, env) = lookupInClass(inName, env);
      then
        (item, env);
...
algorithm
  ...
  matchcontinue(inName, inEnv, inLookupStrategy, inSearchInBase, inInfo, inErrorType)
    local
      Absyn.Ident id;
      Item item;
      Absyn.Path path, new_path;
      Env env;
      String name_str, env_str, n1, n2;
      Error.Message error_id;
      Origin origin;

    // Builtin types.
    case (_, _, LOOKUP_ANY(), _, _, _)
      equation
        id = Absyn.pathFirstIdent(inName);
        (item, env) = lookupBuiltinName(id, inEnv);
      then
    ...
    case (_, _, _, _, _, _)
      equation
        ...
        print("lc: " +&  Absyn.pathString(inName) +& "/" +&  Env.printEnvPathStr(inEnv) +& "\n");
      then
        fail();

    // Simple name.
    case (Absyn.IDENT(name = id), _, _, _, _, _)
      equation
        (item, new_path, env) = lookupSimpleName(id, inEnv, inSearchInBase);
        origin = itemOrigin(item);
      then
        (item, new_path, env, origin);

    // Qualified name.
    case (Absyn.QUALIFIED(name = id, path = path), _, _, _, _, _)
      equation
    ...
    case (Absyn.FULLYQUALIFIED(path = path), _, _, _, _, _)
      equation
        (item, path, env) = lookupFullyQualified(path, inEnv, inSearchInBase);
      then
        (item, path, env, CLASS_ORIGIN());

    case (_, _, _, _, _, SOME(error_id))
      equation
...
algorithm
  (outItem, outName, outEnv, outOrigin) := lookupName(inName, inEnv,
    LOOKUP_ANY(), inSearchInBase, inInfo, NONE());
...
algorithm
  (outItem, outName, outEnv, _) := lookupName(inName, inEnv, LOOKUP_ANY(),
    inSearchInBase, inInfo, SOME(Error.LOOKUP_ERROR));
end lookupClassName;
...
public function lookupBaseClassName
  ...
  output Env outEnv;
algorithm
  (outItem, outName, outEnv) := matchcontinue(inName, inEnv, inSearchInBase, inInfo)
    local
      Absyn.Ident id;
      Env env;
      Item item;
    ...
    case (_, _, _, _)
      equation
        true = Flags.isSet(Flags.LOOKUP);
        print("lbc: " +&  Absyn.pathString(inName) +& "/" +&  Env.printEnvPathStr(inEnv) +& "\n");
      then
        fail();

    // Special case for the baseclass of a class extends. Should be looked up
    // among the inherited elements of the enclosing class.
    case (Absyn.QUALIFIED(name = "$ce", path = path as Absyn.IDENT(name = id)), _ :: env, _, _)
      equation
    ...
    case (Absyn.QUALIFIED(name = "$E"), _, _, _)
      equation
    ...
    // Normal baseclass.
      ...
      equation
        (item, path, env, _) = lookupName(inName, inEnv, LOOKUP_ANY(), inSearchInBase, inInfo,
...
public function lookupVariableName
  ...
  output Env outEnv;
algorithm
  (outItem, outName, outEnv, _) := lookupName(inName, inEnv, NO_BUILTIN_TYPES(), inSearchInBase,
...
algorithm
  (outItem, outName, outEnv, outOrigin) := lookupName(inName, inEnv,
    NO_BUILTIN_TYPES(), inSearchInBase, inInfo, SOME(Error.LOOKUP_FUNCTION_ERROR));
...
algorithm
  (outCref, outEnv) := match(inCref, inEnv)
    ...
    case (Absyn.CREF_IDENT(name, subs), _)
      equation
        (_, path, env) = lookupSimpleName(name, inEnv, bSearchInBase);
    ...
    case (Absyn.CREF_QUAL(name, subs, rest_cref), _)
      equation
        ...
        (item, new_path, env) = lookupSimpleName(name, inEnv, bSearchInBase);
        cref = Absyn.pathToCrefWithSubs(new_path, subs);
...
algorithm
  (outItem, outTypeSpec, outTypeEnv) := match(inTypeSpec, inEnv, inSearchInBase, inInfo)
    local
    ...
    // A normal type.
    case (Absyn.TPATH(path, ad), _, _, _)
      equation
        (item, newpath, env) = lookupClassName(path, inEnv, inSearchInBase, inInfo);
      then
        (item, Absyn.TPATH(newpath, ad), env);

    // A MetaModelica type such as list or tuple.
    case (Absyn.TCOMPLEX(path = Absyn.IDENT(name = name)), _, _, _)
      equation
...
public function qualifyPath
  ...
  output Absyn.Path outPath;
algorithm
  outPath := matchcontinue(inPath, inEnv, inSearchInBase, inInfo, inErrorType)
    local
      Absyn.Ident id;
      Absyn.Path path;
      Env env;
    ...
    case (_, _, _, _, _)
      equation
        (_, path, env, _) = lookupName(inPath, inEnv, NO_BUILTIN_TYPES(),
          inSearchInBase, inInfo, inErrorType);
      ...
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        Debug.traceln("- FGraphLookup.qualifyPath failed on " +&
...
public function itemOrigin
  ...
  output Origin outOrigin;
algorithm
  outOrigin := matchcontinue(inItem)
    local Item item;
...
algorithm
  (outIsGlobal, outEnv) := match(inCref, inEnv)
    ...
    // We shouldn't get any other types of crefs here.
      ...
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        Debug.traceln("- FGraphLookup.lookupCrefUnique failed on unknown cref!\n");
      then
        fail();
...
algorithm
  (outIsGlobal, outEnv) := matchcontinue(inIdentifier, inEnv)
    ...
    case (_, _)
      equation
        (SOME(item), _, _) = lookupInLocalScope(inIdentifier, inEnv, bSearchInBase, {});
    ...
    case (_, _)
      ...
      equation
        true = Flags.isSet(Flags.FAILTRACE);
        Debug.traceln("- FGraphLookup.lookupCrefUnique2 failed on " +&
          inIdentifier +& "\n");
      then
        fail();
...
algorithm
  outEnv := matchcontinue(inIdentifier, inEnv)
    ...
    case (_, _ :: env)
      equation
        (SOME(_), _, _) = lookupInLocalScope(inIdentifier, env, bSearchInBase, {});
      then
        SOME(env);
