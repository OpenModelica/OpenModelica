...
algorithm
  (outMod,outTplSCodeElementModLst) := matchcontinue (inMod,inName,inClassExtendsElt,inTplSCodeElementModLst)
    ...
    case (emod,name1,classExtendsElt,(SCode.CLASSDEF(name2,finalPrefix2,replaceablePrefix2,redecl2,cl,cc2),mod1,b)::rest)
      equation
...
algorithm
  (outCache,outElts) := matchcontinue (cache,env,elt,ht)
    local
      ...
      Boolean finalPrefix,replaceablePrefix,protectedPrefix,partialPrefix,rd;
    ...
    case (cache,env,SCode.COMPONENT(component, innerOuter, finalPrefix, replaceablePrefix, protectedPrefix, rd, attributes, typeSpec, modifications, comment, condition, info, cc),ht)
      equation
      ...
      then (cache,SCode.COMPONENT(component, innerOuter, finalPrefix, replaceablePrefix, protectedPrefix, rd, attributes, typeSpec, modifications, comment, condition, info, cc));
    case (cache,env,SCode.CLASSDEF(id,finalPrefix,replaceablePrefix,rd,SCode.CLASS(name,partialPrefix,true,restriction,classDef,info),cc),ht)
      equation
        //Debug.fprintln("debug","fixClassdef " +& id);
        (cache,env) = Builtin.initialEnv(cache);
        (cache,classDef) = fixClassdef(cache,env,classDef,ht);
      then (cache,SCode.CLASSDEF(id,finalPrefix,replaceablePrefix,rd,SCode.CLASS(name,partialPrefix,true,restriction,classDef,info),cc));
    case (cache,env,SCode.CLASSDEF(id,finalPrefix,replaceablePrefix,rd,SCode.CLASS(name,partialPrefix,false,restriction,classDef,info),cc),ht)
      equation
        //Debug.fprintln("debug","fixClassdef " +& id);
        (cache,classDef) = fixClassdef(cache,env,classDef,ht);
      then (cache,SCode.CLASSDEF(id,finalPrefix,replaceablePrefix,rd,SCode.CLASS(name,partialPrefix,false,restriction,classDef,info),cc));
