...
algorithm 
  oattr := matchcontinue (bindExp,attr)
    ...
    case (bindExp,SOME(DAE.VAR_ATTR_REAL(e1,e2,e3,min,e4,e5,e6,sSelectOption,unc,distOption,_,ip,fn)))
    then (SOME(DAE.VAR_ATTR_REAL(e1,e2,e3,min,e4,e5,e6,sSelectOption,unc,distOption,SOME(bindExp),ip,fn)));
    
    case (bindExp,SOME(DAE.VAR_ATTR_INT(e1,min,e2,e3,unc,distOption,_,ip,fn)))
    then SOME(DAE.VAR_ATTR_INT(e1,min,e2,e3,unc,distOption,SOME(bindExp),ip,fn));
...
algorithm
  start := matchcontinue (inVariableAttributesOption)
    local
      DAE.Exp u;
    case (SOME(DAE.VAR_ATTR_REAL(_,SOME(u),_,_,_,_,_,_,_,_,_,_,_))) then u;
...
algorithm
  ...
  match (attr,minMax)
    local
      ...
      Option<DAE.Exp> eb;
      Option<Boolean> ip,fn;
    case (SOME(DAE.VAR_ATTR_REAL(q,u,du,_,i,f,n,ss,unc,distOpt,eb,ip,fn)),minMax)
    then SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,i,f,n,ss,unc,distOpt,eb,ip,fn));
    case (SOME(DAE.VAR_ATTR_INT(q,_,i,f,unc,distOpt,eb,ip,fn)),minMax)
    then SOME(DAE.VAR_ATTR_INT(q,minMax,i,f,unc,distOpt,eb,ip,fn));
    case (SOME(DAE.VAR_ATTR_ENUMERATION(q,_,u,du,eb,ip,fn)),minMax)
    then SOME(DAE.VAR_ATTR_ENUMERATION(q,minMax,u,du,eb,ip,fn));
    case (NONE(),minMax)
      then SOME(DAE.VAR_ATTR_REAL(NONE(),NONE(),NONE(),minMax,NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE()));
  end match;
end setMinMax;
...
algorithm
  ...
  match (attr,start)
    local
      ...
      Option<DAE.Exp> eb;
      Option<Boolean> ip,fn;
    case (SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,_,f,n,ss,unc,distOpt,eb,ip,fn)),start)
    then SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,SOME(start),f,n,ss,unc,distOpt,eb,ip,fn));
    case (SOME(DAE.VAR_ATTR_INT(q,minMax,_,f,unc,distOpt,eb,ip,fn)),start)
    then SOME(DAE.VAR_ATTR_INT(q,minMax,SOME(start),f,unc,distOpt,eb,ip,fn));
    ...
    case (NONE(),start)
      then SOME(DAE.VAR_ATTR_REAL(NONE(),NONE(),NONE(),(NONE(),NONE()),SOME(start),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE()));
  end match;
end setStartAttr;
...
algorithm
  ...
  match (attr,nominal)
    local
      ...
      Option<DAE.Exp> eb;
      Option<Boolean> ip,fn;
    case (SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,s,f,_,ss,unc,distOpt,eb,ip,fn)),nominal)
    then SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,s,f,SOME(nominal),ss,unc,distOpt,eb,ip,fn));
    case (NONE(),nominal)
      then SOME(DAE.VAR_ATTR_REAL(NONE(),NONE(),NONE(),(NONE(),NONE()),NONE(),NONE(),SOME(nominal),NONE(),NONE(),NONE(),NONE(),NONE(),NONE()));
  end match;
end setNominalAttr;
...
algorithm
  ...
  match (attr,unit)
    local
      ...
      Option<DAE.Exp> eb;
      Option<Boolean> ip,fn;
    case (SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,s,f,n,ss,unc,distOpt,eb,ip,fn)),unit)
    then SOME(DAE.VAR_ATTR_REAL(q,SOME(unit),du,minMax,s,f,n,ss,unc,distOpt,eb,ip,fn));
    case (NONE(),unit)
      then SOME(DAE.VAR_ATTR_REAL(NONE(),SOME(unit),NONE(),(NONE(),NONE()),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE()));
  end match;
end setUnitAttr;
...
algorithm
  ...
  matchcontinue (attr,isProtected)
    ...
    then SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,i,f,n,ss,unc,distOpt,eb,SOME(isProtected),fn));
    case (SOME(DAE.VAR_ATTR_INT(q,minMax,i,f,unc,distOpt,eb,_,fn)),isProtected)
    then SOME(DAE.VAR_ATTR_INT(q,minMax,i,f,unc,distOpt,eb,SOME(isProtected),fn));
...
algorithm
  ...
  matchcontinue (attr,start)
    ...
    then SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,ini,start,n,ss,unc,distOpt,eb,ip,fn));
    case (SOME(DAE.VAR_ATTR_INT(q,minMax,ini,_,unc,distOpt,eb,ip,fn)),start)
    then SOME(DAE.VAR_ATTR_INT(q,minMax,ini,start,unc,distOpt,eb,ip,fn));
...
algorithm
  outAttr := match (attr,finalPrefix)
    ...
    case (SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,i,f,n,ss,unc,distOpt,eb,ip,_)),finalPrefix)
    then SOME(DAE.VAR_ATTR_REAL(q,u,du,minMax,i,f,n,ss,unc,distOpt,eb,ip,SOME(finalPrefix)));
    case (SOME(DAE.VAR_ATTR_INT(q,minMax,i,f,unc,distOpt,eb,ip,_)),finalPrefix)
    then SOME(DAE.VAR_ATTR_INT(q,minMax,i,f,unc,distOpt,eb,ip,SOME(finalPrefix)));
    ...
    case (NONE(),finalPrefix)
      then SOME(DAE.VAR_ATTR_REAL(NONE(),NONE(),NONE(),(NONE(),NONE()),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),NONE(),SOME(finalPrefix)));
  end match;
end setFinalAttr;
...
algorithm
  (outElt,oextraArg) := match (ielt,func,iextraArg)
    ...
    case(DAE.CONSTRAINT(DAE.CONSTRAINT_EXPS(exps),source),func,extraArg)
      equation
...
algorithm
  (traversedDaeList,oextraArg) := match(attr,func,iextraArg)
    local
      ...
      Option<Boolean> ip,fn;
      Type_a extraArg;
    case(SOME(DAE.VAR_ATTR_REAL(quantity,unit,displayUnit,(min,max),initial_,fixed,nominal,stateSelect,uncertainty,distribution,eb,ip,fn)),func,extraArg)
      equation
      ...
      then (SOME(DAE.VAR_ATTR_REAL(quantity,unit,displayUnit,(min,max),initial_,fixed,nominal,stateSelect,uncertainty,distribution,eb,ip,fn)),extraArg);

    case(SOME(DAE.VAR_ATTR_INT(quantity,(min,max),initial_,fixed,uncertainty,distribution,eb,ip,fn)),func,extraArg)
      equation
        (quantity,extraArg) = traverseDAEOptExp(quantity,func,extraArg);
        (min,extraArg) = traverseDAEOptExp(min,func,extraArg);
        (max,extraArg) = traverseDAEOptExp(max,func,extraArg);
        (initial_,extraArg) = traverseDAEOptExp(initial_,func,extraArg);
        (fixed,extraArg) = traverseDAEOptExp(fixed,func,extraArg);
      then (SOME(DAE.VAR_ATTR_INT(quantity,(min,max),initial_,fixed,uncertainty,distribution,eb,ip,fn)),extraArg);
...
algorithm
  (v,ie,ia,e,a,co,o) := splitElements_dispatch(inElements,{},{},{},{},{},{},{});
end splitElements;
...
"@author: adrpo
  ...
  output list<DAE.Element> o;
algorithm
  (v,ie,ia,e,a,co,o) := match(inElements,in_v_acc,in_ie_acc,in_ia_acc,in_e_acc,in_a_acc,in_co_acc,in_o_acc)
    local
      DAE.Element el;
      list<DAE.Element> rest, ell, v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc;
      
    // handle empty case
    case ({}, v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) 
    then (listReverse(v_acc),listReverse(ie_acc),listReverse(ia_acc),listReverse(e_acc),listReverse(a_acc),listReverse(co_acc),listReverse(o_acc));
    
    // variables
    case ((el as DAE.VAR(kind=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, el::v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
  
    // initial equations
    case ((el as DAE.INITIALEQUATION(exp1=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,el::ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.INITIAL_ARRAY_EQUATION(exp=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,el::ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.INITIAL_COMPLEX_EQUATION(lhs=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,el::ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.INITIALDEFINE(exp=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,el::ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.INITIAL_IF_EQUATION(condition1=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,el::ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    ...
    // equations
    case ((el as DAE.EQUATION(exp=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.EQUEQUATION(cr1=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.ARRAY_EQUATION(exp=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.COMPLEX_EQUATION(lhs=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.DEFINE(exp=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.ASSERT(condition=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.IF_EQUATION(condition1=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.WHEN_EQUATION(condition=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.REINIT(exp=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    case ((el as DAE.NORETCALL(functionName=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,el::e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
    ...
    // initial algorithms
    case ((el as DAE.INITIALALGORITHM(algorithm_=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,el::ia_acc,e_acc,a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);

    // algorithms
    case ((el as DAE.ALGORITHM(algorithm_=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,e_acc,el::a_acc,co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
        
    // constraints
    case ((el as DAE.CONSTRAINT(constraints=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,e_acc,a_acc,el::co_acc,o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);
        
    // external objects
    case ((el as DAE.EXTOBJECTCLASS(path=_))::rest,v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc) = splitElements_dispatch(rest, v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,el::o_acc);
      then
        (v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc);

    case ((el as DAE.COMP(dAElist = ell)) :: rest, v_acc,ie_acc,ia_acc,e_acc,a_acc,co_acc,o_acc)
      equation
        v_acc = listAppend(ell, v_acc);
        (v_acc, ie_acc, ia_acc, e_acc, a_acc,co_acc, o_acc) =
          splitElements_dispatch(rest, v_acc, ie_acc, ia_acc, e_acc, a_acc,co_acc, o_acc);
      then
        (v_acc, ie_acc, ia_acc, e_acc, a_acc,co_acc, o_acc);
...
algorithm
  isComplete := matchcontinue(functions)
    local
      ...
      list<DAE.Element> v, ie, ia, e, a, o, co;
    ...
    case (DAE.FUNCTION_DEF(els)::rest)
      equation
        // algs are not empty
        (v,ie,ia,e,a,co,o) = splitElements(els);
