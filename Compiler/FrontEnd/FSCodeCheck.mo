...
encapsulated package FSCodeCheck
" file:  FSCodeCheck.mo
  package:     FSCodeCheck
  description: SCode checking

  RCS: $Id: FSCodeCheck.mo 14085 2012-11-27 12:12:40Z adrpo $
...
algorithm
  ...
  ty_path = FEnv.getEnvPath(inTypeEnv);
...
algorithm
  _ := match(inBaseClass, inOriginInfo)
    ...
    case (Env.CLASS(cls = SCode.CLASS(prefixes = SCode.PREFIXES(
  replaceablePrefix = SCode.REPLACEABLE(cc = _)))), _)
      then ();

    case (Env.CLASS(cls = SCode.CLASS(name = name, prefixes = SCode.PREFIXES(
...
public function checkRedeclareModifier
  input Env.Redeclaration inModifier;
...
algorithm
  _ := match(inModifier, inBaseClass, inEnv)
    ...
    case (Env.RAW_MODIFIER(e as SCode.CLASS(classDef =
...
algorithm
  ...
  fail();
    ...
    case (Env.CLASS(cls = SCode.CLASS(restriction = res, info = info)),
...
public function checkDuplicateRedeclarations
  ...
  input list<Env.Redeclaration> inRedeclarations;
protected
  SCode.Element el;
  String el_name;
  Absyn.Info el_info;
algorithm
  (el_name, el_info) := FEnv.getRedeclarationNameInfo(inRedeclare);
...
algorithm
  outIsDuplicate := matchcontinue(inRedeclareName, inRedeclareInfo,
    ...
    case (_, _, redecl :: rest_redecls)
      equation
  (el_name, el_info) = FEnv.getRedeclarationNameInfo(redecl);
...
algorithm
  _ := matchcontinue(inComponentName, inComponentInfo, inTypeEnv, inTypeItem,
    ...
    case (_, _, _, _, _)
      equation
  false = FEnv.envPrefixOf(inTypeEnv, inComponentEnv);
      then
  ();

    // Make an exception for components in functions.
    case (_, _, _, _, Env.FRAME(name = SOME(cls_name)) ::
  Env.FRAME(clsAndVars = tree) :: _)
      equation
  Env.CLASS(cls = el) = Env.avlTreeGet(tree, cls_name);
  ...
  ();
      ...
      equation
  ty_name = FEnv.getItemName(inTypeItem);
...
algorithm
  _ := matchcontinue(inItem, inPrefix, inInfo)
    ...
    case (Env.CLASS(cls = SCode.CLASS(restriction = res)), _, _)
      equation
  ...
  Debug.traceln("- FSCodeCheck.checkInstanceRestriction failed on unknown item.");
      then
  fail();
...
algorithm
  _ := match(inItem, inInfo)
    ...
    case (Env.CLASS(cls = SCode.CLASS(name = name, partialPrefix = SCode.PARTIAL())), _)
      equation
