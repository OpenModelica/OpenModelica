...
algorithm 
  (outProgram,outCompiledFunctions) := matchcontinue (inPath,inString,inCompiledFunctions)
    local
    ...
    case (Absyn.IDENT(name = classname),mp,cf) 
      equation 
    ...
    case (Absyn.QUALIFIED(name = pack,path = rest),mp, cf) 
      equation 
        gd = System.groupDelimiter();
        mps = System.strtok(mp, gd);
    ...
    case ((path as Absyn.QUALIFIED(name = pack,path = rest)),mp,cf)
      equation 
        ts = Absyn.getNewTimeStamp();
        gd = System.groupDelimiter();
        mps = System.strtok(mp, gd);
        (p, newCF) = loadCompletePackageFromMps(pack, mps, Absyn.TOP(), Absyn.PROGRAM({},Absyn.TOP(),ts), cf);
...
"function: loadClassFromMps 
  ...
  output list<Interactive.CompiledCFunction> outCompiledFunctions;
algorithm 
  (outProgram,outCompiledFunctions) := matchcontinue (inIdent,inStringLst,inCompiledFunctions)
    local
      Absyn.Program p;
      String class_,mp;
      list<String> mps;
...
"function: loadClassFromMp  
  ...
  output list<Interactive.CompiledCFunction> outCompiledFunctions;
algorithm 
  (outProgram,outCompiledFunctions) := matchcontinue (inIdent,inString,inCompiledFunctions)
    local
    ...
    case (class_,mp_1,cf)
      equation 
        ...
        (p, newCF) = loadCompletePackageFromMp(class_, mp, Absyn.TOP(), Absyn.PROGRAM({},Absyn.TOP(), ts), cf);
      then
        (p, newCF);
    case (_,_,_)
      equation 
...
"function: loadCompletePackageFromMps 
  ...
  output list<Interactive.CompiledCFunction> outCompiledFunctions;
algorithm 
  (outProgram,outCompiledFunctions) := matchcontinue (inIdent,inStringLst,inWithin,inProgram,inCompiledFunctions)
    local
...
"function: loadCompletePackageFromMp  
  ...
  output list<Interactive.CompiledCFunction> outCompiledFunctions;
algorithm 
  (outProgram,outCompiledFunctions) := matchcontinue (inIdent,inString,inWithin,inProgram,inCompiledFunctions)
    local
    ...
    case (pack,mp,(within_ as Absyn.TOP()),Absyn.PROGRAM(classes = oldc),cf)
      equation 
        ...
        Print.printBuf("\n");
        (p1_1, newCF) = Interactive.updateProgram(Absyn.PROGRAM(p1,w1,ts), Absyn.PROGRAM(oldc,Absyn.TOP(),ts), cf);
    ...
    case (pack,mp,(within_ as Absyn.WITHIN(path = wpath)),Absyn.PROGRAM(classes = oldc),cf)
      equation 
        ...
        Print.printBuf(packagefile);
        Print.printBuf("\n");
        (p1_1,newCF) = Interactive.updateProgram(Absyn.PROGRAM(p1,Absyn.WITHIN(wpath),ts),Absyn.PROGRAM(oldc,Absyn.TOP(),ts),cf);
        subdirs = System.subDirectories(mp_1);
        subdirstr = Util.stringDelimitList(subdirs, ", ");
        print("subdirs =");
        print(subdirstr);
...
"function: loadCompleteSubdirs  
  ...
  output list<Interactive.CompiledCFunction> outCompiledFunctions;
algorithm 
  (outProgram,outCompiledFunctions) := matchcontinue (inStringLst,inIdent,inString,inWithin,inProgram,inCompiledFunctions)
    local
    ...
    case ({},_,_,w,Absyn.PROGRAM(classes = oldcls,within_ = w2, globalBuildTimes=ts),cf) 
      then (Absyn.PROGRAM(oldcls,w2,ts),cf);      
...
algorithm 
  (outProgram,outCompiledFunction) := matchcontinue (inStringLst,inString,inWithin,inProgram,inCompiledFunction)
    local
      String mp,pd,f_1,f;
    ...
    case ({},mp,within_,Absyn.PROGRAM(classes = cls,within_ = w, globalBuildTimes=ts),cf) 
      then (Absyn.PROGRAM(cls,w,ts),cf);
              
    case ((f :: fs),mp,within_,Absyn.PROGRAM(classes = oldc,globalBuildTimes = ts),cf) 
      equation 
        pd = System.pathDelimiter();
        ...
        Print.printBuf(f_1);
        Print.printBuf("\n");
        (p_1,newCF) = Interactive.updateProgram(Absyn.PROGRAM(cls,within_,ts), Absyn.PROGRAM(oldc,Absyn.TOP(),ts),cf);        
...
"function loadFile
  ...
  output list<Interactive.CompiledCFunction> outCompiledFunctions;
algorithm 
  (outProgram, outCompiledFunctions) := matchcontinue (inString,inCompiledFunctions)
    local
...
"function loadModelFromEachClass
  ...
  output list<Interactive.CompiledCFunction> outCompiledFunctions;
algorithm 
  (outProgram,outCompiledFunctions) := matchcontinue (inProgram,inCompiledFunctions,inString)
    local
      Absyn.Within a;
      Absyn.Path path;
    ...
    case (Absyn.PROGRAM(classes = {},within_ = a,globalBuildTimes=ts),cf,_) 
      then (Absyn.PROGRAM({},a,ts),cf);
              
    case (Absyn.PROGRAM(classes = (Absyn.CLASS(name = id) :: res),within_ = a,globalBuildTimes=ts),cf,dir) 
      equation 
        path = Absyn.IDENT(id);
        (pnew, newCF) = loadClass(path, dir, cf);
        (p_res, newCF_1) = loadModelFromEachClass(Absyn.PROGRAM(res,a,ts), newCF, dir);        
