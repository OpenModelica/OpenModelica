...
case SIMCODE(modelInfo=modelInfo as MODELINFO(__)) then
  ...
  let()= textFile(simulationHeaderFile(simCode,contextFMI, extraFuncs ,extraFuncsDecl, "","","",true,false), 'OMCpp<%name%>.h')
  let()= textFile(simulationCppFile(simCode, contextFMI,extraFuncs ,extraFuncsDecl, "",false), 'OMCpp<%name%>.cpp')
...
case SIMCODE(modelInfo=MODELINFO(__), makefileParams=MAKEFILE_PARAMS(__), simulationSettingsOpt = sopt) then
  let extraCflags = match sopt case SOME(s as SIMULATION_SETTINGS(__)) then ""
  let modelName = '<%lastIdentOfPath(modelInfo.name)%>'
  let platformstr = match makefileParams.platform case "linux-i686" then 'linux32' case "x86_64-linux" then 'linux64' else '<%makefileParams.platform%>'
