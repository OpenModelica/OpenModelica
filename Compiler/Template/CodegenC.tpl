...
::=
  match simCode
    ...
    extern "C" {
      ...
      int <%symbolName(modelNamePrefixStr,"pickUpBoundsForInputsInOptimization")%>(DATA* data, modelica_real* min, modelica_real* max, modelica_real*nominal, modelica_boolean *useNominal, char ** name);
    #if defined(_MSC_VER)
    }
    #endif
    >>
...
::=
  match simCode
    ...
    extern int <%symbolName(modelNamePrefixStr,"pickUpBoundsForInputsInOptimization")%>(DATA* data, modelica_real* min, modelica_real* max, modelica_real*nominal, modelica_boolean *useNominal, char ** name); 
...
case SIMCODE(modelInfo=MODELINFO(__), makefileParams=MAKEFILE_PARAMS(__), simulationSettingsOpt = sopt) then
  ...
  let ParModelicaLibs = if acceptParModelicaGrammar() then 'OMOCLRuntime.lib OpenCL.lib' // else ""
...
case var as VARIABLE(parallelism = PARGLOBAL(__)) then
  ...
  //mahge: Removed here because of leak. All return statments should be fixed this way. even serials.
    //let &varInits += 'alloc_<%expTypeShort(var.ty)%>_array(&<%dest%>.c<%ix%>, <%listLength(instDims)%>, <%instDimsInit%>);<%\n%>'
    //let &varAssign += 'copy_<%expTypeShort(var.ty)%>_array_data(&<%contextCref(var.name,contextFunction)%>, &<%dest%>.c<%ix%>);<%\n%>'
    let &varAssign += '<%dest%>.c<%ix%> = <%contextCref(var.name,contextFunction)%>;<%\n%>'
    ""
...
::=
  ...
  case s as STMT_ASSIGN(exp1=lhs as PATTERN(__)) then
    ...
    <%patternMatch(lhs.pattern,expPart,"MMC_THROW_INTERNAL()",&varDecls,&assignments)%><%assignments%>
    >>
...
::=
    ...
    case{} then
        ...
        int <%symbolName(modelNamePrefixStr,"pickUpBoundsForInputsInOptimization")%>(DATA* data, modelica_real* min, modelica_real* max, modelica_real*nominal, modelica_boolean *useNominal, char ** name){return -1;}
        >>  
      else
