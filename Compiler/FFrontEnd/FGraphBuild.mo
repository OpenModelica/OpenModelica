...
"Recursively analyses an expression."
  ...
  output Graph outGraph; 
algorithm
  (_, (_, _, (_, _, outGraph))) := Absyn.traverseExpBidir(inExp, (analyseExpTraverserEnter,
    analyseExpTraverserExit, (inRef, inKind, inGraph)));
end analyseExp;
...
protected function analyseExpTraverserEnter
  ...
  input tuple<Absyn.Exp, tuple<Ref, Kind, Graph>> inTuple;
  output tuple<Absyn.Exp, tuple<Ref, Kind, Graph>> outTuple;
protected
  Absyn.Exp exp;
  Ref ref;
  Kind k;
...
algorithm
  (exp, (ref, k, g)) := inTuple;
  g := analyseExp2(exp, ref, k, g);
  outTuple := (exp, (ref, k, g));
  ...
  outTuple := match(inTuple)
    ...
    case ((equf as SCode.EQ_FOR(index = iter_name, info = info), (ref, k, g)))
      equation
        ...
        (equ, (_, (_, _, g))) = SCode.traverseEEquationExps(equf, (traverseExp, (ref, k, g)));
      then
        ((equ, (ref, k, g)));
    ...
    case ((equr as SCode.EQ_REINIT(cref = cref1, info = info), (ref, k, g)))
      equation
        ...
        (equ, (_, (_, _, g))) = SCode.traverseEEquationExps(equr, (traverseExp, (ref, k, g)));
      then
        ((equ, (ref, k, g)));
    ...
    case ((equ, (ref, k, g)))
      equation
        ...
        (equ, (_, (_, _, g))) = SCode.traverseEEquationExps(equ, (traverseExp, (ref, k, g)));
      then
        ((equ, (ref, k, g)));
...
algorithm
  (exp, (ref, k, g)) := inTuple;
  (exp, (_, _, (ref, k, g))) := Absyn.traverseExpBidir(exp,
    (analyseExpTraverserEnter, analyseExpTraverserExit, (ref, k, g)));
  outTuple := (exp, (ref, k, g));
end traverseExp;
...
algorithm
  outTuple := match(inTuple)
    ...
    case ((stmt as SCode.ALG_FOR(index = iter_name, info = info), (ref, k, g)))
      equation
        g = addIterators({Absyn.ITERATOR(iter_name, NONE(), NONE())}, ref, k, g);
        (_, (_, (_, _, g))) = SCode.traverseStatementExps(stmt, (traverseExp, (ref, k, g)));
      then
        ((stmt, (ref, k, g)));
