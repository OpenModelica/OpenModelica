...
//        true = replace + replace1 > 0;
        // Add Residual eqn
        eqns_1 = DAELow.equationSetnth(eqns,residualeqn-1,BackendDAE.EQUATION(DAE.BINARY(eqn,DAE.SUB(DAE.ET_REAL()),scalar),DAE.CREF(crt,DAE.ET_REAL()),source));
        eqns1_1 = DAELow.equationSetnth(eqns1,residualeqn-1,BackendDAE.EQUATION(DAE.BINARY(eqn,DAE.SUB(DAE.ET_REAL()),scalar),DAE.RCONST(0.0),source));
        // add equation to calc org var
        eqns_2 = DAELow.equationAdd(eqns_1,BackendDAE.EQUATION(DAE.CALL(Absyn.IDENT("tearing"),
        ...
        assign1 = DAELow.assignmentsCreate(nvars, memsize, 0);
        assign2 = DAELow.assignmentsCreate(nvars, memsize, 0);
        // try matching
        DAELow.checkMatching(dlow_1, (BackendDAE.NO_INDEX_REDUCTION(), BackendDAE.EXACT(), BackendDAE.KEEP_SIMPLE_EQN()));
        Debug.fcall("tearingdump", BackendDump.dumpIncidenceMatrix, m_1);
        Debug.fcall("tearingdump", BackendDump.dumpIncidenceMatrixT, mT_1);
        Debug.fcall("tearingdump", BackendDump.dump, dlow_1);
        (ass1,ass2,dlow_2,m_2,mT_2,_,_) = DAELow.matchingAlgorithm2(dlow_1, m_1, mT_1, nvars, neqns, 1, assign1, assign2, (BackendDAE.NO_INDEX_REDUCTION(), BackendDAE.EXACT(), BackendDAE.KEEP_SIMPLE_EQN()),DAEUtil.avlTreeNew(),{},{});
        v1_1 = DAELow.assignmentsVector(ass1);
        v2_1 = DAELow.assignmentsVector(ass2);
        (comps) = DAELow.strongComponents(m_2, mT_2, v1_1, v2_1);
        Debug.fcall("tearingdump", BackendDump.dumpMatching, v1_1);
...
/*        print("\ntfixedexplst DivExpLst:\n");
        ...
        Util.listMap0(s,print);
        print("\n===============================\n");
*/        eqns_1 = DAELow.equationSetnth(eqns,e_1,BackendDAE.EQUATION(expr,varexp,source));
