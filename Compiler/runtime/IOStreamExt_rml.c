...
/*
 *
 *  RCS: $Id: IOStreamExt_rml.c 5482 2010-05-10 05:56:23Z adrpo $
 *
...
RML_BEGIN_LABEL(IOStreamExt__printReversedList)
{
  ...
  {
    fprintf(stderr, "\nIOStreamExt.printReversedList failed! Error: Could not allocate string of length %d! Not enough memory.", (int) len+1);
    fflush(stderr);
    RML_TAILCALLK(rmlFC);
  }
