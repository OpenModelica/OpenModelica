...
partial function FuncTypeKeyToStr
 input Key key;
 output String outString;
end FuncTypeKeyToStr;

partial function FuncTypeValToStr
 input Val val;
 output String outString;
...
uniontype Tree "a tree is a node and two optional printing functions"
...
constant Node<Key,Val> emptyNode = NODE(NO_ITEM(), 0, NO_NODE(), NO_NODE());
...
protected function getKeyOfValNode
  ...
  output Key outKey;
algorithm
  outKey := matchcontinue(inTree, inNode, inVal)
    local
      Node<Key,Val> left, right;
      Item<Key,Val> item;
      Val v;
    ...
    case (_, NO_NODE(), _) then fail();

    case (_, NODE(item = NO_ITEM()), _) then fail();
