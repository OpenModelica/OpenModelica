...
algorithm
  (outString,outInteractiveSymbolTable) := matchcontinue (inAlgorithmItem,inInteractiveSymbolTable)
    ...
    case (Absyn.ALGORITHMITEM(
      ...
      equation
        env = buildEnvFromSymboltable(st);
        (cache,econd,prop,SOME(st_1),_) = Static.elabExp(Env.emptyCache(),env, cond, true, SOME(st),true,Prefix.NOPRE());
    ...
    case (Absyn.ALGORITHMITEM(algorithm_ = Absyn.ALG_NORETCALL(functionCall = Absyn.CREF_IDENT(name = "assert"),
      ...
      equation
        env = buildEnvFromSymboltable(st);
        (cache,msg_1,prop,SOME(st_1),_) = Static.elabExp(Env.emptyCache(),env, msg, true, SOME(st),true,Prefix.NOPRE());
    ...
    case
      ...
      equation
        env = buildEnvFromSymboltable(st);
        (cache,sexp,DAE.PROP(t,_),SOME(st_1),_) = Static.elabExp(Env.emptyCache(),env, exp, true, SOME(st),true,Prefix.NOPRE());
    ...
    case
      ...
      equation
        env = buildEnvFromSymboltable(st);
        (cache,srexp,rprop,SOME(st_1),_) = Static.elabExp(Env.emptyCache(),env, rexp, true, SOME(st),true,Prefix.NOPRE());
...
algorithm
  ...
  matchcontinue (inExp,inInteractiveSymbolTable)
    ...
    case (exp,(st as SYMBOLTABLE(ast = p)))
      equation
        env = buildEnvFromSymboltable(st);
        (cache,sexp,prop,SOME(st_1),_) = Static.elabExp(Env.emptyCache(), env, exp, true, SOME(st),true,Prefix.NOPRE());
        (_,value,SOME(st_2)) = Ceval.ceval(cache,env, sexp, true, SOME(st_1), NONE, Ceval.MSG());
      then
        (value,st_2);
  end matchcontinue;
...
algorithm
  env := buildEnvFromSymboltable(st);
  (_,sexp,prop,SOME(st_1),_) := Static.elabExp(Env.emptyCache(),env, exp, true, SOME(st),true,Prefix.NOPRE());
...
"function: getVariableBindingInComponentitem
  ...
  output Absyn.Exp outExp;
algorithm
  outExp := matchcontinue (inComponentItem)
    local Absyn.Exp e;
  ...
  outString := matchcontinue (inModelPath,inProgram,inInteger)
    ...
    case (modelpath,p,n)
      equation
        ...
        resstr = System.stringAppendList({str, ",", tpstr});
      then
        resstr;
    case (_,_,_) then "Error";
  end matchcontinue;
  ...
  outString := matchcontinue (inString,inWithin)
    ...
    case (id,Absyn.WITHIN(path = path))
      equation
        ...
        result = System.stringAppendList({str_path, ".", id});
      then
        result;
  end matchcontinue;
end getQualified;
...
algorithm
  annotationStr := matchcontinue(inClass, annotationType, inProgram, inModelPath)
    local
...
algorithm
  docStr := matchcontinue(mod)
    ...
    case(SOME(Absyn.CLASSMOD(expOption = SOME(e)))) 
      equation
        docStr = Dump.printExpStr(e);
      then 
        docStr;
...
"function: getConnectionAnnotationStr
  ...
  output String outString;
algorithm
  outString := matchcontinue (inEquationItem, inClass, inFullProgram, inModelPath)
    local
...
algorithm
  outEquationItem := matchcontinue (inClass,inInteger)
    local
...
"function: getComponentitemsAnnotation
  ...
  output String outString;
algorithm
  outString := matchcontinue (inAbsynComponentItemLst,inClass,inFullProgram,inModelPath)
    local
    ...
    case ((Absyn.COMPONENTITEM(
      ...
      equation
        ...
        res = System.stringAppendList({"{", s1, "},", str});
      then
        res;
    case ({Absyn.COMPONENTITEM(
      comment = SOME(Absyn.COMMENT(SOME(Absyn.ANNOTATION((mod as (Absyn.MODIFICATION(_,_,Absyn.CREF_IDENT("Placement",_),_,_) :: _)))),_)))},
...
algorithm
  (outCache, outEnv) := matchcontinue(inFullProgram, inModelPath, inAnnotationMod, inAnnotationClass)
    local
      ...
      SCode.Class classContainingAnnotation;
      Boolean b1, b2;
  ...
  outString := matchcontinue (inAnnotation,inClass,inFullProgram,inModelPath)
    ...
    case (Absyn.ANNOTATION(elementArgs = {Absyn.MODIFICATION(componentRef = Absyn.CREF_IDENT(name = "Icon"),modification = SOME(Absyn.CLASSMOD(mod,_)))}),
      ...
      equation
        ...
        totstr = System.stringAppendList({str, ",", gexpstr});
      then
        totstr;
