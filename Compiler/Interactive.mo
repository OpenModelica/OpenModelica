...
algorithm
  ...
  matchcontinue (inInteractiveVariableLst,inEnv)
    ...
    case ((IVAR(varIdent = id,value = v,type_ = tp) :: rest),env)
      equation
        (_,_,_,_,_,_,_,_) = Lookup.lookupVar(Env.emptyCache(),env, DAE.CREF_IDENT(id,DAE.ET_OTHER(),{}));
    ...
    case ((IVAR(varIdent = id,value = v,type_ = tp) :: rest),env)
      equation
        failure((_,_,_,_,_,_,_,_) = Lookup.lookupVar(Env.emptyCache(),env, DAE.CREF_IDENT(id,DAE.ET_OTHER(),{})));
  ...
  matchcontinue (inEquationItem)
    ...
    case (Absyn.EQUATIONITEM(equation_ = Absyn.EQ_CONNECT(connector1 = _),
      ...
      equation
        ...
        (cache,env) = Inst.makeEnvFromProgram(Env.emptyCache(),p_1, Absyn.IDENT(""));
        (_,newexp,prop) = Static.elabGraphicsExp(cache,env, Absyn.CALL(Absyn.CREF_IDENT("Line",{}),fargs), false) "impl" ;
