...
algorithm
  (equation_, ouniqueEqIndex, otempvars) := matchcontinue (eqNum, varNum, syst, shared, helpVarInfo, linearSystem, skipDiscInAlgorithm, iuniqueEqIndex, itempvars)
    ...
    // solved equation
    case (_, _, BackendDAE.EQSYSTEM(orderedVars=vars, orderedEqs=eqns), _, _, _, _, _, _)
      equation
...
algorithm
  ...
  matchcontinue (ass1,ass2,inIntegerLst4,inIntegerLst5,inIntegerLst6,inBackendDAE,inTplIntegerIntegerBackendDAEEquationLstOption,inJacobianType,helpVarInfo,iuniqueEqIndex,itempvars)
    ...
    case (_,_,block_,r,t,BackendDAE.DAE(eqs=BackendDAE.EQSYSTEM(orderedVars=v,orderedEqs=eqn)::{},
      ...
      then
        (SimCode.SES_NONLINEAR(uniqueEqIndex, simeqnsystem1, tcrs, 0, NONE()),uniqueEqIndex+1,tempvars);
    else
      equation
...
algorithm
  ...
  matchcontinue (inSES)
    ...
    case SimCode.SES_WHEN(index = index, left = cr, right = e, conditions = conditions, elseWhen= NONE(), source = source)
      equation
        e = addDivExpErrorMsgtoExp(e,source);
      then
        SimCode.SES_WHEN(index,cr,e,conditions,NONE(),source);
    case SimCode.SES_WHEN(index = index, left = cr, right = e, conditions = conditions, elseWhen= SOME(elseWhen), source = source)
      equation
        e = addDivExpErrorMsgtoExp(e,source);
        elseWhenEq = addDivExpErrorMsgtoSimEqSystem(elseWhen);
      then
        SimCode.SES_WHEN(index,cr,e,conditions,SOME(elseWhenEq),source);
    else then inSES;
  end matchcontinue;
