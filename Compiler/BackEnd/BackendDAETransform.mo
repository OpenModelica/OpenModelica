  ...
  matchcontinue (isyst,ishared,inMatchingOptions,daeHandler,inFunctions)
    ...
    case (syst as BackendDAE.EQSYSTEM(vars,eqs,SOME(m),SOME(mt),_),_,_,_,_)
      equation
        ...
        (_,syst,shared,assign1, assign2,_,_,arg1) = daeHandler({},BackendDAE.STARTSTEP(),syst,ishared,inFunctions, assign1, assign2,{},{},arg);
        ...
        (_,syst as BackendDAE.EQSYSTEM(vars,eqs,SOME(m),SOME(mt),_),shared,ass1,ass2,_,_,_) = daeHandler({},BackendDAE.ENDSTEP(),syst,shared,inFunctions,ass1,ass2,{},{},arg2);
  ...
  matchcontinue (isyst,ishared,nv,nf,i,ass1,ass2,inMatchingOptions9,daeHandler,inArg,inFunctions,inDerivedAlgs,inDerivedMultiEqn)
    ...
    case (_,_,_,_,_,_,_,match_opts as (BackendDAE.INDEX_REDUCTION(),eq_cons),_,_,_,_,_)
      equation
        ...
        (_,syst,shared,ass1_1,ass2_1,derivedAlgs,derivedMultiEqn,arg) = daeHandler(meqns,BackendDAE.REDUCE_INDEX(),isyst,ishared,inFunctions,ass1,ass2,inDerivedAlgs,inDerivedMultiEqn,inArg) 
        ...
        (ass1_3,ass2_3,syst,shared,derivedAlgs1,derivedMultiEqn1,arg1) = matchingAlgorithm2(syst,shared,nv_1,nf_1,i,ass1_2,ass2_2,match_opts,daeHandler,arg,inFunctions,derivedAlgs,derivedMultiEqn);
      then
...
"function: assign
  ...
  output BackendDAE.Assignments outAssignments2;
algorithm
  outAssignments1 := assignmentsSetnth(ass1, v - 1, e);
  outAssignments2 := assignmentsSetnth(ass2, e - 1, v);
end assign;
...
algorithm
  outComps:=
  match (inComps,syst,shared,inIncidenceMatrix,inIncidenceMatrixT,inAss1,inAss2)
    local
    ...
    case (comp::comps,syst,shared,m,mt,ass1,ass2)
      equation
        acomps = analyseStrongComponents(comps,syst,shared,m,mt,ass1,ass2);
        acomp = analyseStrongComponent(comp,syst,shared,m,mt,ass1,ass2);
      then
        (acomp::acomps);
    else
      equation
...
algorithm
  outComp:=
  matchcontinue (inComp,inEqnLst,inVarVarindxLst,isyst,ishared,inIncidenceMatrix,inIncidenceMatrixT,inAss1,inAss2,inLoop)
    local
      Integer i;
    ...
    case (comp,eqn_lst,var_varindx_lst,_,_,m,mt,ass1,ass2,false)
      equation
        SOME(i) = singleComplexEquation(eqn_lst,NONE());
        varindxs = List.map(var_varindx_lst,Util.tuple22);        
      then
        BackendDAE.SINGLECOMPLEXEQUATION(i,comp,varindxs);
    ...
    case (comp,eqn_lst,var_varindx_lst,syst as BackendDAE.EQSYSTEM(orderedVars=vars,orderedEqs=eqns),shared as BackendDAE.SHARED(arrayEqs=ae,algorithms=al,complEqs=complEqs),m,mt,ass1,ass2,_)
      equation
        ...
        jac = BackendDAEUtil.calculateJacobian(vars_1, eqns_1, ae1, complEqs, m, mt,true);
...
protected function singleComplexEquation
  input list<BackendDAE.Equation> inEqnLst;
  input Option<Integer> inIndx;
  output Option<Integer> outIndx;
algorithm
  ...
  matchcontinue (inOrgEqns,inVars)
    ...
    case ((_,orgeqntpl)::rest,v)
      equation
        ...
        orgeqn = List.map(orgeqntpl,Util.tuple21);
...
algorithm
  ...
  matchcontinue (inIntegerLst,inBackendDAE,inStateOrd)
    ...
    case ((e :: es),(dae as BackendDAE.DAE(BackendDAE.EQSYSTEM(v,eqns,om,omT,matching)::{},BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,BackendDAE.EVENT_INFO(wclst,zc),eoc,btp))),inStateOrd)
      equation
        e_1 = e - 1;
        eqn = BackendDAEUtil.equationNth(eqns, e_1);
        ...
        dae = replaceStateOrder(es,BackendDAE.DAE(BackendDAE.EQSYSTEM(v,eqns_1,om,omT,matching)::{},BackendDAE.SHARED(kv,ev,av,ie,seqns,ae1,al1,complEqs1,BackendDAE.EVENT_INFO(wclst1,zc),eoc,btp)),inStateOrd);
      then
        (dae);
    else
      equation
...
"function: reduceIndexDynamicStateSelection
  ...
  output BackendDAE.DAEHandlerArg outArg;
algorithm
  (changedEqns,osyst,oshared,outAssignments1,outAssignments2,outDerivedAlgs,outDerivedMultiEqn,outArg):=
  matchcontinue (eqns,inJop,isyst,ishared,inFunctions,inAssignments1,inAssignments2,inDerivedAlgs,inDerivedMultiEqn,inArg)
    local
    ...
    case (eqns,BackendDAE.REDUCE_INDEX(),syst as BackendDAE.EQSYSTEM(m=SOME(m),mT=SOME(mt)),shared as BackendDAE.SHARED(arrayEqs=ae),inFunctions,ass1,ass2,derivedAlgs,derivedMultiEqn,(so,orgEqnsLst))
      equation
      ...
      then
        fail();      
    case (eqns,BackendDAE.REDUCE_INDEX(),syst,shared,inFunctions,ass1,ass2,derivedAlgs,derivedMultiEqn,(so,orgEqnsLst))
      equation
        diff_eqns = BackendDAEEXT.getDifferentiatedEqns();
        eqns_1 = List.setDifferenceOnTrue(eqns, diff_eqns, intEq);
        Debug.fcall(Flags.BLT_DUMP, print, "Reduce Index\nmarked equations: ");
        ...
        (syst,shared,derivedAlgs1,derivedMultiEqn1,so1,orgEqnsLst2,changedeqns) = differentiateEqnsDynamicState(syst,shared,eqns_1,inFunctions,derivedAlgs,derivedMultiEqn,so,orgEqnsLst,{});
      then
        (changedeqns,syst,shared,ass1,ass2,derivedAlgs1,derivedMultiEqn1,(so1,orgEqnsLst2));

    case (eqns,BackendDAE.STARTSTEP(),syst,shared,inFunctions,ass1,ass2,derivedAlgs,derivedMultiEqn,(so,orgEqnsLst))
      equation
    ...
    case (eqns,BackendDAE.ENDSTEP(),syst as BackendDAE.EQSYSTEM(m=SOME(m),mT=SOME(mt)),shared as BackendDAE.SHARED(arrayEqs=ae),inFunctions,ass1,ass2,derivedAlgs,derivedMultiEqn,(so,orgEqnsLst))
      equation
         vec1 = assignmentsVector(ass1);
         vec2 = assignmentsVector(ass2);
    ...
    case (eqns,BackendDAE.REDUCE_INDEX(),syst,shared,_,_,_,_,_,_)
      equation
        diff_eqns = BackendDAEEXT.getDifferentiatedEqns();
        eqns_1 = List.setDifferenceOnTrue(eqns, diff_eqns, intEq);
...
algorithm
  outArrayListeqns:=
  matchcontinue (inOrgEqns,syst,ae,inArrayListeqns)
    local
      list<tuple<Integer,Integer,Boolean>> eqns;
      list<tuple<Integer,list<tuple<Integer,Integer,Boolean>>>> rest;
      BackendDAE.BackendDAE dae;
      array<list<BackendDAE.Equation>> arrayListeqns;
    case ({},syst,ae,arrayListeqns)
       then arrayListeqns;
    case ((_,eqns)::rest,syst,ae,arrayListeqns)
      equation
        arrayListeqns = getScalarArrayEqns1(eqns,syst,ae,arrayListeqns);
        arrayListeqns =  getScalarArrayEqns(rest,syst,ae,arrayListeqns);
      then
        arrayListeqns;
  end matchcontinue;
  ...
  matchcontinue (inOrgEqns,syst,ae,inArrayListeqns)
    ...
    case ((e,_,_)::rest,syst as BackendDAE.EQSYSTEM(orderedEqs=eqnsarr),ae,arrayListeqns)
      equation
        ...
        arrayListeqns = getScalarArrayEqns1(rest,syst,ae,arrayListeqns);
      then
        arrayListeqns;
    case ((_,_,_)::rest,syst,ae,arrayListeqns)
      equation
        arrayListeqns = getScalarArrayEqns1(rest,syst,ae,arrayListeqns);
      then
        arrayListeqns;       
...
algorithm
  (osyst,oshared,outDerivedAlgs,outDerivedMultiEqn,outStateOrd,outOrgEqnsLst,outchangedEqns):=
  matchcontinue (isyst,ishared,inIntegerLst6,inFunctions,inDerivedAlgs,inDerivedMultiEqn,inStateOrd,inOrgEqnsLst,inchangedEqns)
    local
      ...
      BackendDAE.BackendDAEType btp;
      BackendDAE.Matching matching;
    case (_,_,{},_,_,_,_,_,_) then (isyst,ishared,inDerivedAlgs,inDerivedMultiEqn,inStateOrd,inOrgEqnsLst,inchangedEqns);
    case (syst as BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),shared as BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,BackendDAE.EVENT_INFO(wclst,zc),eoc,btp),(e :: es),_,_,_,_,_,_)
      equation
        e_1 = e - 1;
        eqn = BackendDAEUtil.equationNth(eqns, e_1);
        ...
        (syst,shared,derivedAlgs1,derivedMultiEqn1,so1,orgEqnsLst,changedEqns) = differentiateEqnsDynamicState(syst,shared, es, inFunctions,inDerivedAlgs,inDerivedMultiEqn,inStateOrd,inOrgEqnsLst,inchangedEqns);
      then
        (syst,shared,derivedAlgs1,derivedMultiEqn1,so1,orgEqnsLst,changedEqns);
    case (syst as BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),shared as BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,BackendDAE.EVENT_INFO(wclst,zc),eoc,btp),(e :: es),_,_,_,_,_,_)
      equation
        e_1 = e - 1;
        eqn = BackendDAEUtil.equationNth(eqns, e_1);
        ...
        shared = BackendDAE.SHARED(kv,ev,av,ie,seqns,ae1,al1,complEqs,BackendDAE.EVENT_INFO(wclst1,zc),eoc,btp);
        syst = BackendDAEUtil.updateIncidenceMatrix(syst, shared, eqnslst1);
        orgEqnsLst = addOrgEqn(inOrgEqnsLst,e,eqn,false);
        changedEqns = listAppend(inchangedEqns,eqnslst1);
        (syst,shared,derivedAlgs1,derivedMultiEqn1,so1,orgEqnsLst,changedEqns) = differentiateEqnsDynamicState(syst,shared, es, inFunctions,derivedAlgs,derivedMultiEqn,so,orgEqnsLst,changedEqns);
      then
        (syst,shared,derivedAlgs1,derivedMultiEqn1,so1,orgEqnsLst,changedEqns);
    else
      equation
...
algorithm
  ...
  matchcontinue (inOrgEqns,isyst,ishared,inStateOrd,inFunctions,e)
    ...
    case ({},syst,shared,_,_,_) 
      then (syst,shared,{});
    case ((orgeqn,false)::rest,syst as BackendDAE.EQSYSTEM(v,eqnsarr,SOME(m),SOME(mt),matching),shared as BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,BackendDAE.EVENT_INFO(wclst,zc),eoc,btp),so,inFunctions,e)
      equation
        ...
        shared = BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,BackendDAE.EVENT_INFO(wclst,zc),eoc,btp);
...
algorithm
  outStates := match (syst,inStateOrd,orgEqnsLst)
    local
      list<tuple<DAE.ComponentRef,Integer>> cr_lst;
      BackendDAE.Variables v,v1;
    case (syst as BackendDAE.EQSYSTEM(orderedVars = v),inStateOrd,orgEqnsLst)
      equation
        v1 = getVarsOfOrgEqn1(orgEqnsLst,syst);
...
algorithm
  outStates:=
  matchcontinue (inStates,syst,shared)
    local
    ...
    case (inStates,BackendDAE.EQSYSTEM(orderedVars=vars,orderedEqs = eqns,m=SOME(m),mT=SOME(mt)),BackendDAE.SHARED(knownVars=knvars))
      equation
...
algorithm
  (osyst,oshared,outAssignments1,outAssignments2,outStateOrd,outStates,outOrgEqns,outOrgEqnsLst,outReplaced):=
  matchcontinue (inOrgEqns,isyst,ishared,inAssignments1,inAssignments2,inStateOrd,inStates,inOrgEqns1,inOrgEqnsLst)
    local
    ...
    case ({},syst,shared,ass1,ass2,so,cstates,orgEqnslst,inOrgEqnsLst) 
      then (syst,shared,ass1,ass2,so,cstates,orgEqnslst,inOrgEqnsLst,0);
    case ((e,ep,l)::rest,syst as BackendDAE.EQSYSTEM(orderedVars=v,orderedEqs=eqns,m=SOME(m),mT=SOME(mt)),shared,ass1,ass2,so,cstates,orgEqnslst,inOrgEqnsLst)
      equation
        e_1 = ep - 1;
        ...
        vlst = List.map1r(slst,BackendVariable.getVarAt,v);
        crlst = List.map(vlst,BackendVariable.varCref);         
        (dcr,stateno) = selectDummyState(crlst, slst, syst, shared);
        ...
        (syst,shared,rest) = replaceDummyStateInOrgEqns2(rest,repl,syst,shared);
        (syst,shared,orgEqnslst) = replaceDummyStateInOrgEqns2(orgEqnslst,repl,syst,shared);
        (syst,shared,orgeqns) = replaceDummyStateInOrgEqns(inOrgEqnsLst,repl,syst,shared);
        slst = mt[stateno];
    ...
    case ((e,ep,l)::rest,syst,shared,ass1,ass2,so,cstates,orgEqnslst,inOrgEqnsLst)
      equation
        // next 
        (syst,shared,ass1,ass2,so1,cstates,orgEqnslst,orgeqns,r) = processAlliasStates(rest,syst,shared,ass1,ass2,so,cstates,(e,ep,l)::orgEqnslst,inOrgEqnsLst);
      then
        (syst,shared,ass1,ass2,so1,cstates,orgEqnslst,orgeqns,r); 
    else
      equation
...
algorithm
  (osyst,oshared,outOrgEqns):=
  matchcontinue (inOrgEqns,repl,isyst,ishared)
    local
    ...
    case ({},_,syst,shared) then (syst,shared,{});
    case ((e,eqns)::rest,repl,syst,shared)
      equation
        (syst,shared,eqns_1) = replaceDummyStateInOrgEqns1(eqns,repl,syst,shared);
        (syst,shared,orgeqns) =  replaceDummyStateInOrgEqns(rest,repl,syst,shared);
      then
        (syst,shared,(e,eqns_1)::orgeqns);
  end matchcontinue;
...
algorithm
  ...
  matchcontinue (inOrgEqns,repl,isyst,ishared)
    ...
    case ((ep,l,b)::rest,repl,syst as BackendDAE.EQSYSTEM(orderedEqs=eqnsarr),shared)
      equation
        e_1 = ep - 1;
        orgeqn = BackendDAEUtil.equationNth(eqnsarr, e_1);        
        {orgeqn} = BackendVarTransform.replaceEquations({orgeqn},repl);
        syst = BackendEquation.equationSetnthDAE(e_1,orgeqn,syst);
        // next 
        (syst,shared,orgEqnslst) = replaceDummyStateInOrgEqns1(rest,repl,syst,shared);
      then
        (syst,shared,(ep,l,b)::orgEqnslst);
...
algorithm
  ...
  matchcontinue (inOrgEqns,repl,isyst,ishared)
    ...
    case ((e,ep,l)::rest,repl,syst as BackendDAE.EQSYSTEM(orderedEqs=eqnsarr),shared)
      equation
        e_1 = ep - 1;
        orgeqn = BackendDAEUtil.equationNth(eqnsarr, e_1);        
        {orgeqn} = BackendVarTransform.replaceEquations({orgeqn},repl);
...
algorithm
  (outJac,outOrgEqns):=
  matchcontinue (inOrgEqns,syst,shared,inStates)
    local
      Integer e,ep,e_1,l;
      list<tuple<Integer,Integer,Integer>> rest,orgeqns;
      BackendDAE.BackendDAE dae,dae1;
    ...
    case ({},syst,shared,inStates) then ({},{});
    case ((e,ep,_)::rest,syst as BackendDAE.EQSYSTEM(orderedEqs=eqns),shared as BackendDAE.SHARED(knownVars=knvars,arrayEqs=ae,algorithms=al),inStates)
      equation
        e_1 = ep - 1;
        orgeqn = BackendDAEUtil.equationNth(eqns, e_1); 
   ...
   matchcontinue (inDynDummyDer,syst,shared,inFunctionTree,inStates,inStateOrd,inOrgEqns,inDummyStates,ep)
    ...
    case ((dyndummyder as (_::_::_))::rest,syst,shared,inFunctionTree,states,inStateOrd,inOrgEqns,inDummyStates,ep)
      equation
        ...
        (states,dummystate,stateno) = selectDummyDerivative(dummystates,stateindx,syst,shared,states,inStateOrd);
        ((_,_,dse1)) = List.getMemberOnTrue(stateno,dyndummyder,getDummyExp);
        dummystates = List.removeOnTrue(dummystate,ComponentReference.crefEqualNoStringCompare,dummystates);
        stateindx = List.removeOnTrue(stateno,intEq,stateindx);
...
algorithm
  ...
  matchcontinue(inDummyDerLst,inDummyDerLst1)
      ...
      case({},ddlst)
        equation
          dummydercomps = List.map(ddlst,listMake); 
        then dummydercomps;
      case (tpl::rest,ddlst)
        equation
         dummydercomps = List.map1r(rest,List.create2,tpl);
...
algorithm
  (osyst,oshared,outAssignments1,outAssignments2,outStates) := 
   matchcontinue (inDummyDers,syst,shared,inAssignments1,inAssignments2,inStates)
    local
    ...
    case ({},syst,shared,ass1,ass2,inStates) 
      then 
       (syst,shared,ass1,ass2,inStates);
    case (((dummystate,dse1,stateno,ep)::{})::rest,syst,shared,ass1,ass2,inStates)
      equation
      ...
      then
        (syst,shared,ass1,ass2,states1);
    case (((dummystate,dse1,stateno,ep)::((dummystate1,dse2,stateno1,_)::{}))::rest,syst,shared,ass1,ass2,inStates)
      equation
...
algorithm
  (outStates,outDummyState,outStateNo) := 
   matchcontinue (dummystates,stateindx,syst,shared,inStates,inStateOrd)
    local
      list<DAE.ComponentRef> dummystates;
      list<tuple<DAE.ComponentRef,Integer>> states;
...
algorithm
  _:=
  matchcontinue (orgEqns,syst,shared)
    local
      BackendDAE.Equation orgeqn;
      list<tuple<Integer,Integer,Boolean>> rest;
      BackendDAE.EquationArray eqns;
...
algorithm
  ...
  matchcontinue (eqns,inJop,isyst,ishared,inFunctions,inAssignments1,inAssignments2,inDerivedAlgs,inDerivedMultiEqn,inArg)
    ...
    case (eqns,BackendDAE.REDUCE_INDEX(),syst as BackendDAE.EQSYSTEM(mT=SOME(mt)),shared,inFunctions,ass1,ass2,derivedAlgs,derivedMultiEqn,inArg)
      equation
        ...
        // BackendDump.dumpStateVariables(BackendVariable.daeVars(syst));
      then
        (changedeqns,syst,shared,ass1,ass2,derivedAlgs1,derivedMultiEqn1,inArg);

    case (eqns,BackendDAE.STARTSTEP(),syst,shared,inFunctions,ass1,ass2,derivedAlgs,derivedMultiEqn,inArg)
      then ({},syst,shared,ass1,ass2,derivedAlgs,derivedMultiEqn,inArg);

    case (eqns,BackendDAE.ENDSTEP(),syst,shared,inFunctions,ass1,ass2,derivedAlgs,derivedMultiEqn,inArg)
      then ({},syst,shared,ass1,ass2,derivedAlgs,derivedMultiEqn,inArg);
...
algorithm
  ...
  matchcontinue (inExp1,inExp2,isyst,ishared,inIntegerLst6)
    ...
    case (stateexpcall,dummyderexp,BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),BackendDAE.SHARED(kv,ev,av as BackendDAE.ALIASVARS(aliasVars = aliasVars),ie,seqns,ae,al,complEqs,BackendDAE.EVENT_INFO(wclst,zeroCrossingLst),eoc,btp),{})
      equation
       ...
       then (BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),BackendDAE.SHARED(kv,ev,av,ie1,seqns1,ae2,al2,complEqs2,BackendDAE.EVENT_INFO(wclst2,zeroCrossingLst),eoc,btp));

    case (stateexpcall,dummyderexp,BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,BackendDAE.EVENT_INFO(wclst,zeroCrossingLst),eoc,btp),(e :: rest))
      equation
        e_1 = e - 1;
        eqn = BackendDAEUtil.equationNth(eqns, e_1);
        (eqn_1,al1,ae1,complEqs1,wclst1,_) = traverseBackendDAEExpsEqn(eqn, al, ae, complEqs, wclst, replaceDummyDer2Exp,(stateexpcall,dummyderexp));
        (eqn_1,al2,ae2,complEqs2,wclst2,(v_1,_)) = traverseBackendDAEExpsEqn(eqn_1,al1,ae1,complEqs1,wclst1,replaceDummyDerOthersExp,(v,0));
        eqns_1 = BackendEquation.equationSetnth(eqns, e_1, eqn_1)
         "incidence_row(v\'\',eqn\') => row\' &
          Util.list_replaceat(row\',e\',m) => m\' &
          transpose_matrix(m\') => mt\' &" ;
        (syst,shared) = replaceDummyDer(stateexpcall, dummyderexp, BackendDAE.EQSYSTEM(v_1,eqns_1,SOME(m),SOME(mt),matching),BackendDAE.SHARED(kv,ev,av,ie,seqns,ae2,al2,complEqs2,BackendDAE.EVENT_INFO(wclst2,zeroCrossingLst),eoc,btp), rest);
      then
        (syst,shared);
...
"function traversereplaceDummyDer
  ...
  replaceable type Type_a subtypeof Any;
  input tuple<BackendDAE.Equation,tuple<array<DAE.Algorithm>,array<BackendDAE.MultiDimEquation>,array<BackendDAE.ComplexEquation>,list<BackendDAE.WhenClause>,FuncExpType,Type_a>> inTpl;
  output tuple<BackendDAE.Equation,tuple<array<DAE.Algorithm>,array<BackendDAE.MultiDimEquation>,array<BackendDAE.ComplexEquation>,list<BackendDAE.WhenClause>,FuncExpType,Type_a>> outTpl;
...
algorithm
  ...
  matchcontinue inTpl
    local 
      ...
      FuncExpType func;
    case ((e,(algs,ae,complEqs,wclst,func,ext_arg)))
      equation
         (e1,algs1,ae1,complEqs1,wclst1,ext_arg_1) = traverseBackendDAEExpsEqn(e,algs,ae,complEqs,wclst,func,ext_arg);
      then
        ((e1,(algs1,ae1,complEqs1,wclst1,func,ext_arg_1)));
    case inTpl then inTpl;
  end matchcontinue;
...
algorithm
  (outEquation,outAlgs,outMultiDimEquationArray,outComplEqs,outWhenClauseLst,(_,outTypeA)) := traverseBackendDAEExpsEqnWithSymbolicOperation(inEquation,inAlgs,inMultiDimEquationArray,inComplEqs,inWhenClauseLst,traverseBackendDAEExpsEqnWithoutSymbolicOperationHelper,(func,inTypeA));
...
algorithm
  (outEquation,outAlgs,outMultiDimEquationArray,outComplEqs,outWhenClauseLst,outTypeA) := matchcontinue (inEquation,inAlgs,inMultiDimEquationArray,inComplEqs,inWhenClauseLst,func,inTypeA)
    local
    ...
    case (BackendDAE.EQUATION(exp = e1,scalar = e2,source = source),inAlgs,ae,ce,wclst,func,inTypeA)
      equation
    ...
    case (BackendDAE.SOLVED_EQUATION(componentRef = cr,exp = e2,source=source),inAlgs,ae,ce,wclst,func,inTypeA)
      equation
        ...
        source = List.foldr(ops, DAEUtil.addSymbolicTransformation, source);
      then
        (BackendDAE.SOLVED_EQUATION(cr1,e2_1,source),inAlgs,ae,ce,wclst,ext_arg_1);
    case (BackendDAE.RESIDUAL_EQUATION(exp = e1,source=source),inAlgs,ae,ce,wclst,func,inTypeA)
      equation
        ((e1_1,(ops,ext_arg_1))) = func((e1,({},inTypeA)));
        source = List.foldr(ops, DAEUtil.addSymbolicTransformation, source);
      then
    ...
    case (BackendDAE.WHEN_EQUATION(whenEquation =
      ...
      then
        (res,inAlgs,ae,ce,wclst1,ext_arg_3);

    case (BackendDAE.WHEN_EQUATION(whenEquation =
          BackendDAE.WHEN_EQ(index = i,left = cr,right = e1,elsewhenPart=SOME(elsepart)),source = source),inAlgs,ae,ce,wclst,func,inTypeA)
      equation
        ((e1_1,(ops,ext_arg_1))) = func((e1,({},inTypeA)));
        source = List.foldr(ops, DAEUtil.addSymbolicTransformation, source);
        (BackendDAE.WHEN_EQUATION(elsepartRes,source),algs,ae1,ce1,wclst1,ext_arg_2) = traverseBackendDAEExpsEqnWithSymbolicOperation(BackendDAE.WHEN_EQUATION(elsepart,source),inAlgs,ae,ce,wclst,func,ext_arg_1);
...
algorithm
  (outEquations,outAlgs,outMultiDimEquationArray,outComplEqs,outWhenClauseLst,outTypeA):=
  matchcontinue (inEquations,inAlgs,inMultiDimEquationArray,inComplEqs,inWhenClauseLst,func,inTypeA)
    local
      ...
      array<BackendDAE.ComplexEquation> complEqs,complEqs1,complEqs2;
      Type_a ext_arg_1,ext_arg_2;
    case ({},inAlgs,ae,complEqs,wclst,func,inTypeA) then ({},inAlgs,ae,complEqs,wclst,inTypeA);
    case (e::eqns,inAlgs,ae,complEqs,wclst,func,inTypeA)
      equation
         (e1,algs,ae1,complEqs1,wclst1,ext_arg_1) = traverseBackendDAEExpsEqn(e,inAlgs,ae,complEqs,wclst,func,inTypeA);
         (eqns1,algs1,ae2,complEqs2,wclst2,ext_arg_2) = traverseBackendDAEExpsEqnList(eqns,algs,ae1,complEqs1,wclst1,func,ext_arg_1);
      then
        (e1::eqns1,algs1,ae2,complEqs2,wclst2,ext_arg_2);
  end matchcontinue;
...
algorithm
  ...
  matchcontinue (varCrefs,varIndices,syst,shared)
    local
      DAE.ComponentRef s;
      BackendDAE.Value sn;
      BackendDAE.Variables vars,knvars;
...
algorithm
  tuples := match(inVarCrefs,inVarIndices,vars,knvars,eqns,m,mt)
    local 
...
"function varStateSelectHeuristicPrio1
  ...
  output Real prio;
algorithm
  prio := matchcontinue(cr,inEqnLst,vars,knvars,eqns)
    local 
...
algorithm
  ...
  matchcontinue (inIntegerLst,syst)
    local
    ...
    case ({},_) then ({},{});
    case ((e :: rest),syst as BackendDAE.EQSYSTEM(orderedVars = vars,orderedEqs = eqns,m=SOME(m)))
      equation
        (res1,res2) = statesInEqns(rest, syst);
        vars2 = List.select(m[e], Util.intNegative);
        vars2 = List.map(vars2,intAbs);
        (res11,res22) = statesInVars(vars, vars2);
        res1_1 = listAppend(res11, res1);
        res2_1 = listAppend(res22, res2);
      then
        (res1_1,res2_1);
    case ((e :: rest),BackendDAE.EQSYSTEM(orderedEqs = eqns))
      equation
        se = intString(e);
...
"function: differentiateEqns
  ...
  output list<tuple<Integer,Integer,Integer>> outDerivedMultiEqn;
algorithm
  (osyst,oshared,outIntegerLst6,outDerivedAlgs,outDerivedMultiEqn):=
  matchcontinue (isyst,ishared,inIntegerLst6,inFunctions,inDerivedAlgs,inDerivedMultiEqn)
    local
      BackendDAE.BackendDAE dae;
      array<list<BackendDAE.Value>> m,mt;
    ...
    case (BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),shared as BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,wc,eoc,btp),(e :: es),inFunctions,inDerivedAlgs,inDerivedMultiEqn)
      equation
        e_1 = e - 1;
        eqn = BackendDAEUtil.equationNth(eqns, e_1);
        ...
        BackendDAEEXT.markDifferentiated(e) "length gives index of new equation Mark equation as differentiated so it won\'t be differentiated again" ;
        (syst,shared,reqns,derivedAlgs1,derivedMultiEqn1) = differentiateEqns(BackendDAE.EQSYSTEM(v,eqns_1,SOME(m),SOME(mt),matching),BackendDAE.SHARED(kv,ev,av,ie,seqns,ae1,al1,complEqs,wc,eoc,btp), es, inFunctions,derivedAlgs,derivedMultiEqn);
      then
    ...
    case (BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),shared as BackendDAE.SHARED(kv,ev,av,ie,seqns,ae,al,complEqs,wc,eoc,btp),(e :: es),inFunctions,inDerivedAlgs,inDerivedMultiEqn)
      equation
        ...
        Debug.fcall(Flags.BLT_DUMP, debugdifferentiateEqns,(eqn,eqn_1));
        BackendDAEEXT.markDifferentiated(e) "length gives index of new equation Mark equation as differentiated so it won\'t be differentiated again" ;
        (syst,shared,reqns,derivedAlgs1,derivedMultiEqn1) = differentiateEqns(BackendDAE.EQSYSTEM(v,eqns,SOME(m),SOME(mt),matching),BackendDAE.SHARED(kv,ev,av,ie,seqns,ae1,al1,complEqs,wc,eoc,btp), es, inFunctions,derivedAlgs,derivedMultiEqn);
      then
        (syst,shared,(e :: reqns),derivedAlgs1,derivedMultiEqn1);
    else
      equation
