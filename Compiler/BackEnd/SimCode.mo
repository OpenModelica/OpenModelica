...
algorithm
  (oodeEquations,oalgebraicEquations,oallEquations) := matchcontinue (ifcpp,systs,shared,helpVarInfo,odeEquations,algebraicEquations,allEquations)
    local
...
algorithm
  outBool := match(inVar1,inVar2)
    local
      DAE.ComponentRef cref1,cref2;
      Boolean res;
    case(SIMVAR(name=cref1),SIMVAR(name=cref2))
...
protected function appendAllVar
  input list<JacobianColumn> inJacobianMatrix;
  output list<SimVar> outEqn;
algorithm 
  outEqn := 
  ...
  matchcontinue (inFunctions,inBackendDAE,inDiffVars,inStateVars,inInputVars,inParameterVars,inDifferentiatedVars,inVars,inOrigTuple,inName)
    ...
    case (inFunctions,inBackendDAE,inDiffVars,inStateVars,inInputVars,inParameterVars,inDifferentiatedVars,inVars,(inOrigVars,inOrigKnVars),inName)
      equation
        ...
        knvars = List.fold(inOrigVars, BackendVariable.deleteVar,knvars);
        knvars = List.fold(inOrigKnVars, BackendVariable.deleteVar,knvars);        
  ...
  parameterEquations := matchcontinue (shared,acc)
    ...
    case (BackendDAE.SHARED(knownVars=knvars,externalObjects=extobj,initialEqs=ie,algorithms=algs,arrayEqs=arrayEqs,extObjClasses=extObjClasses),acc)
      equation
        ...
        parameterEquations = createEquations(false, false, true, false, false, syst, shared, comps, helpVarInfo);
