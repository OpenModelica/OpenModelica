...
encapsulated package HpcOmMemory
  ...
  // -------------------------------------------
	  ...
	  oMemoryMap := matchcontinue(iModelInfo, iTaskGraph, iTaskGraphMeta, iEqSystems, iFileNamePrefix, iSchedulerInfo, iSchedule, iSccSimEqMapping, iCriticalPaths, iCriticalPathsWoC, iCriticalPathInfo, iAllComponents)
	    case(_,_,_,_,_,_,_,_,_,_,_,_)
	      equation
	        ...
	        tmpMemoryMap = convertCacheMapToMemoryMap(cacheMap,hashTable);
	        //print cache map
    ...
    "author: marcusw
     Creates a CacheMap optimized for the selected scheduler."
  ...
  algorithm
    oMemoryMap := match(iCacheMap, iScVarNameIdxMapping)
      case(CACHEMAP(cacheLineSize=cacheLineSize, cacheVariables=cacheVariables, cacheLinesFloat=cacheLinesFloat), _)
        equation
