...
protected function getLastCalcTask "get the last calculcated task in the task list. starting list traversing at the last element.
author: Waurich TUD 2013-11"
  input list<Task> taskLst;
  output Integer calcTaskIdx;
protected
  Integer size;
algorithm
  size := listLength(taskLst);
  calcTaskIdx := getLastCalcTask1(taskLst,size,0);
end getLastCalcTask;


protected function getLastCalcTask1
  input list<Task> taskLst;
  input Integer idx;
  input Integer currTaskIdx;
  output Integer calcTaskIdx;
algorithm
  calcTaskidx := matchcontinue(taskLst,idx,currTaskIdx)
    local
      Integer taskIdx;
      Task task;
    case(_,_,0)
      equation
        task = listGet(taskLst,idx);
        taskIdx = getTaskThreadIdx(task);
        taskIdx = getLastCalcTask1(taskLst,idx-1,taskIdx);
      then
        taskIdx;
    case({},_,_)
      equation
...
end getLastCalcTask1;


protected function getTaskThreadIdx "if its a CalcTask, output the threadidx.otherwise -1.
author:Waurich TUD 2013-11"
  input Task taskIn;
  output Integer threadIdx;
algorithm
  threadIdx := match(taskIn)
    local
      Integer idx;
    case(CALCTASK(threadIdx=idx))
      then
        idx;
    case(ASSIGNLOCKTASK(lockId=_))
      then
        -1;
    case(RELEASELOCKTASK(lockId=_))
      then
        -1;
  end match;
end getTaskThreadIdx;


protected function getTaskFinishingTime "if its a CalcTask, output the finishing time. otherwise -1.
author:Waurich TUD 2013-11"
  input Task taskIn;
...
algorithm
  finishingTime := match(taskIn)
    local
      Real fTime;
    case(CALCTASK(timeFinished=fTime))
      then
        fTime;
    case(ASSIGNLOCKTASK(lockId=_))
      then
