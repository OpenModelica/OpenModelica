...
algorithm
  (oSeqns,oIndex,oMT) := match(vars1,ilst1,varskn1,time1,vars2,ilst2,varskn2,time2,negate,source,iSeqns,iIndex,iMT)
    local
      ...
      array<list<Integer>> mT;
      DAE.ComponentRef cr1,cr2;
    case ({BackendDAE.VAR(varName=cr1)},_,false,true,{BackendDAE.VAR(varName=cr2)},{i2},false,false,_,_,_,_,_)
      equation
        colum = iMT[i2];
        _ = arrayUpdate(iMT,i2,iIndex::colum);
      then 
        (TIMEALIAS(cr2,i2,source,negate,-1)::iSeqns,iIndex+1,iMT);
    case ({BackendDAE.VAR(varName=cr1)},{i1},false,false,{BackendDAE.VAR(varName=cr2)},_,false,true,_,_,_,_,_)
      equation
        colum = iMT[i1];
    ...
    case(v1::vlst1,i1::irest1,_,false,v2::vlst2,i2::irest2,_,false,_,_,_,_,_)
      equation
        (seqns,index,mT) = generateSimpleContainter(v1,i1,varskn1,time1,v2,i2,varskn2,time2,negate,source,iSeqns,iIndex,iMT); 
...
"function generateSimpleContainter
  ...
  output array<list<Integer>> oMT;
algorithm
  (oSeqns,oIndex,oMT) := match(v1,i1,varskn1,time1,v2,i2,varskn2,time2,negate,source,iSeqns,iIndex,iMT)
    local
      DAE.ComponentRef cr1,cr2;
      list<Integer> colum;
    case (BackendDAE.VAR(varName=cr1),_,false,false,BackendDAE.VAR(varName=cr2),_,false,false,_,_,_,_,_)
      equation
        colum = iMT[i1];
        _ = arrayUpdate(iMT,i1,iIndex::colum);
        colum = iMT[i2];
    ...
    case (BackendDAE.VAR(varName=cr1),_,true,false,BackendDAE.VAR(varName=cr2),_,false,false,_,_,_,_,_)
      equation
        colum = iMT[i2];
        _ = arrayUpdate(iMT,i2,iIndex::colum);
      then 
        (PARAMETERALIAS(cr2,i2,cr1,i1,source,negate,-1)::iSeqns,iIndex+1,iMT);
    case (BackendDAE.VAR(varName=cr1),_,false,false,BackendDAE.VAR(varName=cr2),_,true,false,_,_,_,_,_)
      equation
        colum = iMT[i1];
        _ = arrayUpdate(iMT,i1,iIndex::colum);
      then 
...
algorithm
  ...
  matchcontinue (index,mark,simpleeqnsarr,iMT,iVars,iEqnslst,ishared,iRepl)
   ...
   case (_,_,_,_,_,_,_,_)
      equation
        // collect set
        (rmax,smax,unremovable,const) = getAlias({index},{},mark,simpleeqnsarr,iMT,iVars,NONE(),NONE(),NONE(),NONE());
...
algorithm
  (oRmax,oSmax,oUnremovable,oConst) := match(rows,nextRows,mark,simpleeqnsarr,iMT,vars,iRmax,iSmax,iUnremovable,iConst)
    local
      Integer r;
      ...
      Option<Integer> unremovable,const;
      Boolean b;
    case ({},{},_,_,_,_,_,_,_,_) then (iRmax,iSmax,iUnremovable,iConst);
    case ({},_::_,_,_,_,_,_,_,_,_)
      equation
        (rmax,smax,unremovable,const) = getAlias(nextRows,{},mark,simpleeqnsarr,iMT,vars,iRmax,iSmax,iUnremovable,iConst);
      then
        (rmax,smax,unremovable,const);
    case (r::rest,_,_,_,_,_,_,_,_,_)
      equation
        s = simpleeqnsarr[r];
        b = isVisited(mark,s);
        (rmax,smax,unremovable,const) = getAlias1(b,s,r,rest,nextRows,mark,simpleeqnsarr,iMT,vars,iRmax,iSmax,iUnremovable,iConst);
      then
        (rmax,smax,unremovable,const);
  end match;
end getAlias;
...
protected function getAlias1
  ...
  output Option<Integer> oConst;
algorithm
  (oRmax,oSmax,oUnremovable,oConst) := match(visited,s,r,rows,nextRows,mark,simpleeqnsarr,iMT,vars,iRmax,iSmax,iUnremovable,iConst)
    local
    ...
    case (false,_,_,_,_,_,_,_,_,_,_,_,_)
      equation
        ...
        (rest,next,rmax,smax,unremovable,const) = getAlias2(s,r,rows,nextRows,mark,simpleeqnsarr,iMT,vars,iRmax,iSmax,iUnremovable,iConst);
        // update candidates
        (rmax,smax,unremovable,const) = getAlias(rest,next,mark,simpleeqnsarr,iMT,vars,rmax,smax,unremovable,const);
      then
        (rmax,smax,unremovable,const);
  end match;
end getAlias1;
...
protected function getAlias2
  ...
  input Option<Integer> iUnremovable;
  input Option<Integer> iConst;
  output list<Integer> oRows;
  output list<Integer> oNextRows;
  output Option<tuple<Integer,Integer>> oRmax;
  output Option<tuple<Integer,Integer>> oSmax;
  output Option<Integer> oUnremovable;
  output Option<Integer> oConst;
algorithm
  (oRows,oNextRows,oRmax,oSmax,oUnremovable,oConst) := match(s,r,rows,nextRows,mark,simpleeqnsarr,iMT,vars,iRmax,iSmax,iUnremovable,iConst)
    local
      list<Integer> rest,next;
      Option<tuple<Integer,Integer>> rmax,smax;
      Option<Integer> unremovable,const;
      BackendDAE.Var v;
      Integer i1,i2;
      Boolean state,replacable;
    case (ALIAS(i1=i1,i2=i2),_,_,_,_,_,_,_,_,_,_,_)
      equation
        // collect next rows
        next = List.fold2(iMT[i1],appendNextRow,mark,simpleeqnsarr,nextRows);
        next = List.fold2(iMT[i2],appendNextRow,mark,simpleeqnsarr,next);
       ...
       then 
         (rows,next,rmax,smax,unremovable,iConst);  
...
algorithm
  ...
  matchcontinue (iRmax,iSmax,iUnremovable,iConst,simpleeqnsarr,iMT,iVars,iEqnslst,ishared,iRepl)
   ...
   case (_,_,_,SOME(r),_,_,_,_,_,_)
     equation
       PARAMETERALIAS(i1=i,paramcr=pcr,source=source,negate=negate) =  simpleeqnsarr[r];
       ...
       (vars,eqnslst,shared,repl) = traverseAliasTree(List.removeOnTrue(r,intEq,iMT[i]),i,exp,SOME(exp),true,simpleeqnsarr,iMT,vars,eqnslst,shared,repl);
     then
       (vars,eqnslst,shared,repl);
   // time set
   case (_,_,_,SOME(r),_,_,_,_,_,_)
     equation
       TIMEALIAS(i=i,source=source,negate=negate) =  simpleeqnsarr[r];
       // generate exp from cref an negate if necessary
       ...
       replacable = replaceableAlias(v);
       (vars,eqnslst,shared,repl) = handleSetVar(replacable,v,i,source,exp,iMT,iVars,iEqnslst,ishared,iRepl);
       (vars,eqnslst,shared,repl) = traverseAliasTree(List.removeOnTrue(r,intEq,iMT[i]),i,exp,SOME(exp),false,simpleeqnsarr,iMT,vars,eqnslst,shared,repl);
     then
       (vars,eqnslst,shared,repl);
   // constant set
   case (_,_,_,SOME(r),_,_,_,_,_,_)
     equation
       TIMEINDEPENTVAR(i=i,exp=exp,source=source) =  simpleeqnsarr[r];
       ...
       (vars,eqnslst,shared,repl) = traverseAliasTree(List.removeOnTrue(r,intEq,iMT[i]),i,exp,SOME(exp),true,simpleeqnsarr,iMT,vars,iEqnslst,shared,iRepl);
     then
       (vars,eqnslst,shared,repl);
   // variable set
   case (NONE(),NONE(),SOME(i),NONE(),_,_,_,_,_,_)
     ...
     then
       (vars,eqnslst,shared,repl);      
   // variable set
   case (NONE(),SOME((i,_)),_,NONE(),_,_,_,_,_,_)
     equation
       (v as BackendDAE.VAR(varName=cr)) = BackendVariable.getVarAt(iVars,i);
       exp = Expression.crefExp(cr);      
       (vars,eqnslst,shared,repl) = traverseAliasTree(iMT[i],i,exp,NONE(),false,simpleeqnsarr,iMT,iVars,iEqnslst,ishared,iRepl);
 ...
 (oVars,oEqnslst,oshared,oRepl):=
  match (rows,ilast,exp,optExp,replaceState,simpleeqnsarr,iMT,iVars,iEqnslst,ishared,iRepl)
    ...
    case (r::rest,_,_,_,_,_,_,_,_,_,_)
      equation
        (vars,eqnslst,shared,repl) = traverseAliasTree1(simpleeqnsarr[r],r,ilast,exp,optExp,replaceState,simpleeqnsarr,iMT,iVars,iEqnslst,ishared,iRepl);
  ...
  match (sc,r,ilast,exp,optExp,replaceState,simpleeqnsarr,iMT,iVars,iEqnslst,ishared,iRepl)
    ...
    case (ALIAS(cr1,i1,cr2,i2,source,negate,_),_,_,_,_,_,_,_,_,_,_,_)
      equation
        ...
        source = addSubstitutionOption(optExp,crexp,source);
        (vars,eqnslst,shared,repl) = handleSetVar(replacable,v,i,source,exp,iMT,iVars,iEqnslst,ishared,iRepl);
