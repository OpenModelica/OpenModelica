...
protected function matchFunctionCallResultVars
"function matchFunctionCallResultVars, match all vars a=f()
 autor: Frenkel TUD 2012-04"
  input BackendDAE.EqSystem isyst;
  input BackendDAE.Shared ishared;
  input array<Integer> ass1 "ass[eqnindx]=varindx";
...
algorithm
  _:=
  matchcontinue (isyst,ishared,ass1,ass2)
    local 
      BackendDAE.Variables v;
      BackendDAE.EquationArray eqns;
      array<BackendDAE.MultiDimEquation> ae;
      array<list<Integer>> matcharr;
    case (BackendDAE.EQSYSTEM(orderedVars=v,orderedEqs=eqns),BackendDAE.SHARED(arrayEqs=ae),_,_)
      equation
        matcharr = arrayCreate(arrayLength(ae),{});
        _ = Util.arrayFold(ae,matchFunctionCallResultVars1,(1,matcharr,v));
        _ = BackendEquation.traverseBackendDAEEqns(eqns,matchFunctionCallResultVars2,(1,matcharr,ass1,ass2));
      then
        ();
    else
      then
...
end matchFunctionCallResultVars;

protected function matchFunctionCallResultVars2
"function matchFunctionCallResultVars2, assign all array equations like a=f() with vars from a
 autor: Frenkel TUD 2012-04"
  input tuple<BackendDAE.Equation,tuple<Integer,array<list<Integer>>,array<Integer>,array<Integer>>> tpl;
  output tuple<BackendDAE.Equation,tuple<Integer,array<list<Integer>>,array<Integer>,array<Integer>>> outTpl;
algorithm
  outTpl := 
  matchcontinue (tpl)
    local
    ...
    case ((eqn as BackendDAE.ARRAY_EQUATION(index=i),(e,arr,ass1,ass2)))
      equation
        i_1 = i+1;
        v::rest = arr[i_1];
        _ = arrayUpdate(arr,i_1,rest);
        false = intGt(ass1[e],0);
        false = intGt(ass2[v],0);
        _ = arrayUpdate(ass1,e,v);
        _ = arrayUpdate(ass2,v,e);
        print("Assign Eqn " +& intString(e) +& " Var " +& intString(v) +& "\n");
      then
        ((eqn,(e+1,arr,ass1,ass2)));
    case ((eqn,(e,arr,ass1,ass2)))
      then
        ((eqn,(e+1,arr,ass1,ass2)));
  end matchcontinue;
end matchFunctionCallResultVars2;

protected function matchFunctionCallResultVars1
"function matchFunctionCallResultVars1, collect array eqns like a=f(..)
 autor: Frenkel TUD 2012-04"
  input BackendDAE.MultiDimEquation inEqn;
  input tuple<Integer,array<list<Integer>>,BackendDAE.Variables> inTpl;
  output tuple<Integer,array<list<Integer>>,BackendDAE.Variables> outTpl;
algorithm
  outTpl := matchcontinue(inEqn,inTpl)
    local
      Integer indx,i;
      array<list<Integer>> arr;
      BackendDAE.Variables v;
      ...
      list<Integer> vallst;
    case (BackendDAE.MULTIDIM_EQUATION(dimSize,e1,e2 as DAE.CALL(attr=attr),_),(indx,arr,v))
      equation
      ((_,(_,vallst))) = Expression.traverseExpTopDown(e1, BackendDAEUtil.traversingincidenceRowExpSolvableFinder, (v,{}));
      vallst = List.select1(vallst,intGt,0);
      i = List.fold(dimSize, intAdd, 0);
      true = intEq(listLength(vallst),i);
      vallst = List.sort(vallst,intGt);
      _ = arrayUpdate(arr,indx,vallst);
      then
        ((indx+1,arr,v));
    case (BackendDAE.MULTIDIM_EQUATION(dimSize,e1 as DAE.CALL(attr=attr),e2,_),(indx,arr,v))
      equation
      ((_,(_,vallst))) = Expression.traverseExpTopDown(e2, BackendDAEUtil.traversingincidenceRowExpSolvableFinder, (v,{}));
      vallst = List.select1(vallst,intGt,0);
      i = List.fold(dimSize, intAdd, 0);
      true = intEq(listLength(vallst),i);
      vallst = List.sort(vallst,intGt);
      ...
      then
        ((indx+1,arr,v));  
    case (BackendDAE.MULTIDIM_EQUATION(dimSize,e1,e2 as DAE.UNARY(exp = DAE.CALL(attr=attr)),_),(indx,arr,v))
      equation
      ((_,(_,vallst))) = Expression.traverseExpTopDown(e1, BackendDAEUtil.traversingincidenceRowExpSolvableFinder, (v,{}));
      vallst = List.select1(vallst,intGt,0);
      i = List.fold(dimSize, intAdd, 0);
      ...
      then
        ((indx+1,arr,v));   
    case (BackendDAE.MULTIDIM_EQUATION(dimSize,e1 as DAE.UNARY(exp=DAE.CALL(attr=attr)),e2,_),(indx,arr,v))
      equation
      ((_,(_,vallst))) = Expression.traverseExpTopDown(e2, BackendDAEUtil.traversingincidenceRowExpSolvableFinder, (v,{}));
      vallst = List.select1(vallst,intGt,0);
      i = List.fold(dimSize, intAdd, 0);
      ...
      then
        ((indx+1,arr,v));  
    case (BackendDAE.MULTIDIM_EQUATION(dimSize,e1 as DAE.ARRAY(array=_),e2,_),(indx,arr,v))
      equation
      ((_,(_,vallst))) = Expression.traverseExpTopDown(e1, BackendDAEUtil.traversingincidenceRowExpSolvableFinder, (v,{}));
      vallst = List.select1(vallst,intGt,0);
      i = List.fold(dimSize, intAdd, 0);
      ...
      then
        ((indx+1,arr,v));                          
    case (_,(indx,arr,v)) then ((indx+1,arr,v));
   end matchcontinue;  
end matchFunctionCallResultVars1;
