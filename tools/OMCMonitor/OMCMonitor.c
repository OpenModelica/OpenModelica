...
BOOL am_ConnectComputer(LPSTR szComputerName)
{
    ...
    if (RegConnectRegistry(szComputerName, HKEY_LOCAL_MACHINE, &hKeyRemote)
        ...
        g_stComputers[i].szComputerName = _strdup(szComputerName);
...
BOOL OMCManageService(LPCSTR szServiceName, LPCSTR szImagePath,
                      ...
                      LPDWORD lpdwPid)
{
    ...
    if (g_dwOSVersion == OS_VERSION_WIN9X)
    {
        hWnd = FindWindow("OMCWin95ServiceMonitor", szServiceName);
                          ...
                          WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        {
            OMCManageService(g_stServices[LOWORD(wParam)
                   ...
                   LPSTR lpCmdLine, int nCmdShow)
{
    ...
    for (i = IDS_MSG_FIRST; i <= IDS_MSG_LAST; ++i) {
        ...
        g_lpMsg[i - IDS_MSG_FIRST] = _strdup(szTmp);
    }
    ...
    _strupr(szCmp);
    g_szLocalHost = _strdup(szCmp);

    memset(g_stComputers, 0, sizeof(ST_MONITORED_COMP) * MAX_APACHE_COMPUTERS);
    g_stComputers[0].szComputerName = _strdup(szCmp);
    g_stComputers[0].hRegistry = HKEY_LOCAL_MACHINE;
    g_szTitle = _strdup(szTmp);
    LoadString(hInstance, IDS_APMONITORCLASS, szTmp, MAX_LOADSTRING);
    g_szWindowClass = _strdup(szTmp);
