/* Copyright (c)2005 Linköping University Sweden, All Rights Reserved
...
void usageModelicaXML(void)
{
	...
	cout << "-dtd dtdURL\n\t sets the dtdURL as the dtd for the xml file:" 
		 << "c:\\path\\to\\modelica.dtd OR http://host.to/modelica.dtd" 
	...
	cout << "-d directory -o outputFile\n\t transforms ALL file.mo files from the directory" 
		 << "\n\t AND subdirectories one big xml file called directory\\file.xml" << endl;
}
...
int ModelicaXML::serializeEachMoFileInDirectory(char *directoryName)
{
	...
	for(it=dirList.begin(); it!=dirList.end(); ++it)
	{
		...
		{
			filename = mstring(*it) + "\\" + mstring(*itFileList);
...
int ModelicaXML::serializeAllMoFilesInDirectoryInXMLFile(char *directoryName, char *xmlFileName)
{
	...
	xmlFile = mstring(directoryName) + mstring("\\") + xmlFile;
	xmlFile += ".xml";
	serializeXMLDocumentToFile(xmlFile);
...
{	
	...
	if (bPackageExists)
	{
		fileList.push_front("package.mo");
	}
	if (bExampleExists)
	{
		fileList.push_back("Examples.mo");
	}
	return fileList;
}
