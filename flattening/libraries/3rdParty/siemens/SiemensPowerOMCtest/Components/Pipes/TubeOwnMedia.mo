...
model TubeOwnMedia
  ...
  parameter Medium.MassFlowRate m_flowLaminar=0.001
...
parameter Medium.Temperature T_wall_start[numberOfNodes]=T_start
 ...
 parameter SiemensPowerOMCtest.Utilities.Structures.PropertiesMetal metal
  ...
  Medium.Density d[numberOfNodes](start=rho0);
  Medium.Density dAverage(start=sum(rho0)/numberOfNodes);
  SI.SpecificVolume vol[numberOfNodes];
  SI.SpecificVolume volAverage(start=1/Medium.density_phX(0.5*(pIn_start+pOut_start),0.5*(hIn_start+hOut_start),XIn_start));
  Medium.SpecificEnthalpy h[numberOfNodes](start=linspace(hIn_start, hOut_start, numberOfNodes));
  Medium.AbsolutePressure dpfric(start=0.015*geoPipe.L/(geoPipe.d_out-2*geoPipe.s)^5*m_flow_start/geoPipe.Nt*0.5*(1/rho0[1] + 1/rho0[numberOfNodes]));
  Medium.AbsolutePressure dphyd(start=g*geoPipe.H*sum(rho0)/numberOfNodes);
  SI.HeatFlux qMetalFluidDelayed[numberOfNodes];
  SI.Length perimeter;
  Medium.MassFlowRate m_flow(start=m_flow_start/geoPipe.Nt);
  Medium.AbsolutePressure p(start=0.5*(pIn_start+pOut_start)) "pressure";
  Medium.AbsolutePressure pUndelayed(start=0.5*(pIn_start+pOut_start));
  ...
  SiemensPowerOMCtest.Components.SolidComponents.Wall wall(
...
protected
  final parameter Medium.Temperature T_start[numberOfNodes]=Medium.temperature_phX(linspace(pIn_start,pOut_start,numberOfNodes),linspace(hIn_start,hOut_start,numberOfNodes), XIn_start)
    "start values for fluid temperatures";
  final parameter Medium.Density rho0[numberOfNodes]=linspace(Medium.density_phX(pIn_start,hIn_start, XIn_start), Medium.density_phX(pOut_start,hOut_start, XOut_start), numberOfNodes);

  SI.HeatFlux qMetalFluid[numberOfNodes];
  Real drdp[numberOfNodes];
  Real drdh[numberOfNodes];
  inner Medium.Temperature TWall[numberOfNodes];
  Integer TTSEid(start=0);

algorithm
//    when (initial()) then
//      if not useINTH2O then
//       TTSEid:=SiemensPowerOMCtest.Media.TTSE.init_ttse();
//      else
       TTSEid:=0;
//      end if;
//    end when;
...
for j in 2:(numberOfNodes-1) loop
 ...
 //p = sum(SiemensPowerOMCtest.Media.IntH2O.p_rhoh(dAverage,h[3]);
 for j in 1:numberOfNodes loop
    if useINTH2O then
       T[j] =  SiemensPowerOMCtest.Media.IntH2O.T_rhoh(d[j],h[j]);
      (d[j],drdp[j],drdh[j]) =  SiemensPowerOMCtest.Media.IntH2O.rho_ph_dpdh(p, h[j]);
    else  // TTSE
        T[j]    = SiemensPowerOMCtest.Media.TTSE.Utilities.T_ph(p, h[j]);
        d[j]  = SiemensPowerOMCtest.Media.TTSE.Utilities.rho_ph(p, h[j]);
        drdp[j] = SiemensPowerOMCtest.Media.TTSE.Utilities.rho_ph_dp(p, h[j]);
        drdh[j] = SiemensPowerOMCtest.Media.TTSE.Utilities.rho_ph_dh(p, h[j]);
