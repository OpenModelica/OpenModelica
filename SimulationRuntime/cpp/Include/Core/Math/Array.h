...
public:
    ...
    */
    ...
    virtual void assign(BaseArray<T>& b) = 0;
...
public:
    ...
    */
    ...
    a.assign(b)
    */
    virtual void assign(BaseArray<T>& b)
    {
...
void assign(StatArrayDim4<T,size1,size2,size3,size4> b)
{
for(int i = 0; i < size1; i++)
{
for(int j = 0; j < size2; j++)
...
{
_array_data[i][j][k][l] = b._array_data[i][j][k][l];
}
}
}
}
...
{
_array_data[i - 1][j - 1][k - 1][l - 1] = b(i,j,k,l);
}
}
}
}
...
void assign(StatArrayDim5<T,size1,size2,size3,size4,size5> b)
{
for(int i = 0; i < size1; i++)
{
for(int j = 0; j < size2; j++)
...
for(int m = 0; m < size5; m++)
{
_array_data[i][j][k][l][m] = b._array_data[i][j][k][l][m];
}
}
}
}
...
{
_array_data[i - 1][j - 1][k - 1][l - 1][m - 1] = b(i,j,k,l,m);
}
}
}
}
...
for (int l = 1; l <= v[1]; l++)
{
_multi_array[i][j][k][l] = b(i,j,k,l);
}
}
}
}
...
for (int m = 1; m <= v[4]; m++)
{
_multi_array[i][j][k][l][m] = b(i,j,k,l,m);
}
}
}
}
...
size_t n1 = b.getDim(1);
size_t n2 = b.getDim(2);
if(n1=size1 || n2 != size2)
throw ModelicaSimulationError(MODEL_ARRAY_FUNCTION, "error in assing array: Array Sizes did not match");
std::transform(b._ref_array_data.c_array(),b._ref_array_data.c_array() +size1*size2,_ref_array_data.c_array(),AssignArrayVarToArrayVar<T>());
