...
void Idas::callIDA()
{
    ...
    _idid = IDASetInitStep(_idaMem, 1e-12);
    if(_idid <0)
        throw std::runtime_error(/*_idid,_tCurrent,*/"Idas::ReInit");
   
    while(_solverStatus & IDAESolver::CONTINUE)
    {
        if(_zeroFound==false)
            if(_tLastWrite + dynamic_cast<ISolverSettings*>(_idasSettings)->getGlobalSettings()->gethOutput() > _tEnd)
                _tLastWrite = _tEnd;
            else
        ...
        if(_zeroFound)
        {
            ...
            event_system->handleSystemEvents(_events);
        }
