...
void Cvode::init()
{
	...
	else
	{
		...
		if(_yLastSucess)	delete [] _yLastSucess;
		if(_yLargeStep)		delete [] _yLargeStep;
		if(_yWrite)			delete [] _yWrite;

		_y				= new double[_dimSys];
		_yInit			= new double[_dimSys];
		_yLastSucess	= new double[_dimSys];
		_yLargeStep		= new double[_dimSys];
		...
		memset(_yLastSucess,0,_dimSys*sizeof(double));
		memset(_yLargeStep,0,_dimSys*sizeof(double));
		...
		memset(_y0,0,sizeof(double));
		memset(_y1,0,sizeof(double));
		...
		continous_system->giveVars(_yInit);
		memcpy(_y,_yInit,_dimSys*sizeof(double));
	...
	const int Cvode::reportErrorMessage(ostream& messageStream)
{
	...
	void Cvode::saveInitState()
	{
		...
		continous_system->giveVars(_yInit,IContinous::ALL_VARS);
	}
	...
	void Cvode::restoreInitState()
	{
		...
		continous_system->setVars(_yInit);
	}
	...
	void Cvode::saveLargeStepState()
	{
		...
		// Zustandsvektor abspeichern
		continous_system->giveVars(_yLargeStep,IContinous::ALL_VARS);
	...
	void Cvode::saveLastSuccessfullState()
	{
		...
		// Zustandsvektor abspeichern
		memcpy(_yLastSucess,_y,_dimSys*sizeof(double));
	}
	...
	void Cvode::restoreLastSuccessfullState()
	{
		...
		memcpy(_y,_yLastSucess,_dimSys*sizeof(double));
	}

	void Cvode::giveScaledError(const double& h, double& error)
	{
		IContinous* continous_system = dynamic_cast<IContinous*>(_system);
		continous_system->giveVars(_y,IContinous::ALL_VARS);

		// Berechnung der Skalierten Fehlernorm für ODE-Systeme
		double sc = 0.0;
		for(int i=0; i<_dimSys; ++i)
		{
			sc = 1e-4 + 1e-4 * max(abs(_yLargeStep[i]),abs(_yInit[i]));
			error += pow((_y[i] - _yLargeStep[i]),2) / sc;
		}
	}
	...
	void Cvode::refineCurrentState(const double& r)
	{
		...
		for(int i=0; i<_dimSys; ++i)
			_y[i] += (_y[i] - _yLargeStep[i]) / ( pow(1.0/r,3.0) );

		continous_system->setVars(_y);
	}
