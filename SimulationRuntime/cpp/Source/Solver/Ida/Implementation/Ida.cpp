...
Ida::Ida(IDAESystem* system, ISolverSettings* settings)
  : SolverDefaultImplementation( system, settings)
  , _idaSettings    (dynamic_cast<IIdaSettings*>(_settings))
    , _z          (NULL)
  , _zp0          (NULL)
  , _z0          (NULL)
  , _zInit        (NULL)
...
void Ida::init()
{
  ...
  else
  {
    ...
    {
      // Ausgabeschrittweite
      _hOut    = dynamic_cast<ISolverSettings*>(_idaSettings)->getGlobalSettings()->gethOutput();
    ...
    // Set Tolerances
     _idid = IDASStolerances(_idaMem, dynamic_cast<ISolverSettings*>(_idaSettings)->getRTol(), dynamic_cast<ISolverSettings*>(_idaSettings)->getATol());          // RTOL and ATOL
    if(_idid < 0)
      throw std::invalid_argument(/*_idid,_tCurrent,*/"Idas::init()");
    ...
    if(_idid < 0)
      ...
      _idid = IDASetInitStep(_idaMem, dynamic_cast<ISolverSettings*>(_idaSettings)->gethInit());              // INITIAL STEPSIZE
    if(_idid < 0)
      throw std::invalid_argument(/*_idid,_tCurrent,*/"Idas::init()");

   _idid = IDASetMaxStep(_idaMem, dynamic_cast<ISolverSettings*>(_idaSettings)->getUpperLimit());              // MAXIMUM STEPSIZE
    if(_idid < 0)
      throw std::invalid_argument(/*_idid,_tCurrent,*/"Idas::init()");
    ...
    _idid = IDACalcIC(_idaMem, IDA_YA_YDP_INIT,  dynamic_cast<ISolverSettings*>(_idaSettings)->gethInit());
...
{
  //_idaSettings->getEventOutput() = true;
  ...
  if (_idaSettings && _system)
  {
    ...
    {
      _firstStep = true;
      if (_idaSettings->getEventOutput())
        writeToFile(0, _tCurrent, _h);
    }
    ...
    while ( _solverStatus & IDAESolver::CONTINUE )
    {
      ...
      if(!_zeroSearchActive)
        _h = max(min(_h,dynamic_cast<ISolverSettings*>(_idaSettings)->getUpperLimit()),dynamic_cast<ISolverSettings*>(_idaSettings)->getLowerLimit());
...
void Ida::callIDA()
{
  ...
  while(_solverStatus & IDAESolver::CONTINUE)
  {
    ...
    if(_zeroFound)
    {
      ...
      if (_idaSettings->getDenseOutput() && abs(_tLastWrite - _tCurrent) > dynamic_cast<ISolverSettings*>(_idaSettings)->getZeroTimeTol())
      {
        if(_idaSettings->getEventOutput())
        {
    ...
    if  ( (_tEnd - _tCurrent) <= dynamic_cast<ISolverSettings*>(_idaSettings)->getEndTimeTol())  
    {
      _solverStatus = DONE;
    }
  }
...
void Ida::writeIDAOutput(const double &time,const double &h,const int &stp)
{
  ...
  if (stp > 0)
  {
    ...
    {
      _bWritten = false;
      while (_tLastWrite + dynamic_cast<ISolverSettings*>(_idaSettings)->getGlobalSettings()->gethOutput()  <= time + dynamic_cast<ISolverSettings*>(_idaSettings)->getEndTimeTol())
      {
        _bWritten = true;
...
void Ida::calcFunction(const double& time, const double* y, double* f)
{
...
int Ida::IDA_fCallback(double t, N_Vector y, N_Vector ydot, N_Vector resval, void *user_data)
{
    ((Ida*) user_data)->calcFunction(t, NV_DATA_S(y), NV_DATA_S(resval));

  for(int i=0; i<((Ida*) user_data)->_dimDEq; ++i)
...
int Ida::IDA_ZerofCallback(double t, N_Vector y, N_Vector yp, double *zeroval, void *user_data)
{
...
void Ida::writeSimulationInfo(ostream& outputStream)
{
  // Solver
  outputStream  << "\nSolver: IDa" 
    << "\nVerfahren: ";
  ...
  outputStream 
   ...
   outputStream  
      << "Zero function tolerance:      " << dynamic_cast<ISolverSettings*>(_idaSettings)->getZeroTol() << " \n"
      << "Zero t tolerance:          " << dynamic_cast<ISolverSettings*>(_idaSettings)->getZeroTimeTol() << " \n"
...
void Ida::saveLastSuccessfullState()
{
...
void Ida::restoreLastSuccessfullState()
{
...
void Ida::giveScaledError(const double& h, double& error)
{
  IContinous* continous_system = dynamic_cast<IContinous*>(_system);
...
void Ida::refineCurrentState(const double& r)
{
  IContinous* continous_system = dynamic_cast<IContinous*>(_system);
...
int Ida::check_flag(void *flagvalue, char *funcname, int opt)
{
  int *errflag;
