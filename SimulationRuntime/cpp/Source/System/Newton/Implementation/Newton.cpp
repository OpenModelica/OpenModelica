...
: _algLoop			(algLoop)
, _newtonSettings	((INewtonSettings*)settings)
, _y				(NULL)
, _yHelp			(NULL)
, _f				(NULL)
, _fHelp			(NULL)
...
void Newton::solve(const IContinous::UPDATE command)
{
	long int
...
BOOST_EXTENSION_TYPE_MAP_FUNCTION {
  types.get<std::map<std::string, factory<IAlgLoopSolver,IAlgLoop*, INewtonSettings*> > >()
    ["Newton"].set<Newton>();
  types.get<std::map<std::string, factory<INewtonSettings> > >()
