...
dasrt_step(_X_DATA* simData, SOLVER_INFO* solverInfo) {
   ...
   * (DASSL calculates beyond TOUT unless info[6] is set to 1!) */
    do {
      ...
      DEBUG_INFO2(LOG_SOLVER, "**Calling DDASRT from %g to %g", solverInfo->currentTime, tout);
      ...
      if (jac_flag) {
        DDASRT(functionODE_residual, mData->nStates,
               ...
               &idid, rwork, &lrw, iwork, &liw, NULL, ipar,
...
*/
      ...
      sData->timeValue = solverInfo->currentTime;
      functionODE(simData);
    } while (dasslData->idid == -1 && solverInfo->currentTime <= simData->simulationInfo.stopTime);
...
{
  _X_DATA* sData = (_X_DATA*)(void*)rpar;
  /*DASSL_DATA* dasslData = (DASSL_DATA*)(void*)rpar[1];*/
  double timeBackup;
  double* statesBackup;
  long i;

  timeBackup = sData->localData[0]->timeValue;
  statesBackup = sData->localData[0]->realVars;

  sData->localData[0]->timeValue = *t;
  sData->localData[0]->realVars = x;
  functionODE(sData);

  /* get the difference between the temp_xd(=localData->statesDerivatives)
     and xd(=statesDerivativesBackup) */
  for (i=0; i < sData->modelData.nStates; i++) {
    delta[i] = sData->localData[0]->realVars[sData->modelData.nStates + i] - xd[i];
  }

  sData->localData[0]->realVars = statesBackup;
  sData->localData[0]->timeValue = timeBackup;
