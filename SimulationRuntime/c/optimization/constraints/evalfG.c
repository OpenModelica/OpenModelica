...
Bool evalfG(Index n, double * v, Bool new_x, int m, Number *g, void * useData)
{
	...
	for(i = 0,k=0 , x0 = v; i<iData->nsi; ++i, x0 = x3)
	{
        ...
        if(i > 0){
			...
			{
				g[k++] = (a1[0][j]*x0[j] + a1[3][j]*x3[j] + scaldf[j]*dt[i]*dotx1[j]) -
						 (a1[1][j]*x1[j] + a1[2][j]*x2[j]);
			}

			for(j = 0; j< iData->nx; ++j)
			{
				g[k++] = (a2[1][j]*x1[j] + scaldf[j]*dt[i]*dotx2[j]) -
						(a2[0][j]*x0[j] + a2[2][j]*x2[j] + a2[3][j]*x3[j]);
			}

			for(j = 0; j< iData->nx; ++j)
			{
				g[k++] = (a3[0][j]*x0[j] + a3[2][j]*x2[j] + scaldf[j]*dt[i]*dotx3[j]) -
						 (a3[1][j]*x1[j] + a3[3][j]*x3[j]);
			}
        }else{
        	double **d1,**d2,**d3;
			...
			for(j = 0; j< iData->nx; ++j)
			{
				g[k++] = (scaldf[j]*dt[i]*(dotx1[j] + d1[4][j]*dotx0[j]) + d1[0][j]*x0[j] + d1[3][j]*x3[j])
					- (d1[1][j]*x1[j] + d1[2][j]*x2[j]);
			}

			for(j = 0; j< iData->nx; ++j)
			{
				g[k++] = (scaldf[j]*dt[i]*dotx2[j] + d2[1][j]*x1[j])
				         - (scaldf[j]*dt[i]*d2[4][j]*dotx0[j] + d2[0][j]*x0[j] + d2[2][j]*x2[j] + d2[3][j]*x3[j]);
			}
			for(j = 0; j< iData->nx; ++j)
			{
				g[k++] = (scaldf[j]*dt[i]*(d3[4][j]*dotx0[j] + dotx3[j]) + d3[0][j]*x0[j] + d3[2][j]*x2[j])
					- (d3[1][j]*x1[j] + d3[3][j]*x3[j]);
			}
        }
	}
	return TRUE;
