...
/*!
 * \brief MOSEditor::popUpCompleter()
 * show the popup for keywords and type for autocompletion
 */
void MOSEditor::popUpCompleter()
{
...
/*!
 * \brief MOSEditor::getCodeSnippets()
 * returns the list of CompleterItem to the autocompleter
 */

QList<CompleterItem> MOSEditor::getCodeSnippets()
{
...
/*!
 * \brief MOSEditor::showContextMenu
 * Create a context menu.
 * \param point
 */
void MOSEditor::showContextMenu(QPoint point)
{
...
void MOSEditor::contentsHasChanged(int position, int charsRemoved, int charsAdded)
{
  Q_UNUSED(position);
...
/*!
 * \brief MOSEditor::toggleCommentSelection
 */
void MOSEditor::toggleCommentSelection()
{
  BaseEditor::toggleCommentSelection();
}

/*!
  * \class MOSHighlighter
  * \brief A syntax highlighter for MOSEditor.
 */
/*!
 * \brief MOSHighlighter::MOSHighlighter
 * \param pMOSEditorPage
 * \param pPlainTextEdit
 */
MOSHighlighter::MOSHighlighter(MOSEditorPage *pMOSEditorPage, QPlainTextEdit *pPlainTextEdit)
  : QSyntaxHighlighter(pPlainTextEdit->document())
{
  mpMOSEditorPage = pMOSEditorPage;
...
{
  QFont font;
  font.setFamily(mpMOSEditorPage->getOptionsDialog()->getTextEditorPage()->getFontFamilyComboBox()->currentFont().family());
  font.setPointSizeF(mpMOSEditorPage->getOptionsDialog()->getTextEditorPage()->getFontSizeSpinBox()->value());
  ...
  mTextFormat.setForeground(mpMOSEditorPage->getColor("Text"));
  mKeywordFormat.setForeground(mpMOSEditorPage->getColor("Keyword"));
  mTypeFormat.setForeground(mpMOSEditorPage->getColor("Type"));
  mSingleLineCommentFormat.setForeground(mpMOSEditorPage->getColor("Comment"));
  mMultiLineCommentFormat.setForeground(mpMOSEditorPage->getColor("Comment"));
  mQuotationFormat.setForeground(mpMOSEditorPage->getColor("Quotes"));
  // Priority: keyword > func() > ident > number. Yes, the order matters :)
  mNumberFormat.setForeground(mpMOSEditorPage->getColor("Number"));
...
QStringList MOSHighlighter::getKeywords()
{
...
QStringList MOSHighlighter::getTypes()
{
...
/*!
 * \brief MOSHighlighter::highlightMultiLine
 * Highlights the multilines text.
 * Quoted text or multiline comments.
 * \param text
 */
void MOSHighlighter::highlightMultiLine(const QString &text)
{
  ...
  while (index < text.length()) {
    ...
    if (pTextBlockUserData && (blockState < 1 || blockState > 3 || mpMOSEditorPage->getOptionsDialog()->getTextEditorPage()->getMatchParenthesesCommentsQuotesCheckBox()->isChecked())) {
...
void MOSHighlighter::highlightBlock(const QString &text)
{
...
/*!
 * \brief MOSHighlighter::settingsChanged
 * Slot activated whenever ModelicaEditor text settings changes.
 */
void MOSHighlighter::settingsChanged()
{
  initializeSettings();
  rehighlight();
}
