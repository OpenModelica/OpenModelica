...
public:
  ...
  float getLength    () const {return mHidden ? 0 : mLengthScale * std::sqrt(_coords[0].exp * _coords[0].exp + _coords[1].exp * _coords[1].exp + _coords[2].exp * _coords[2].exp);}
  float getRadius    () const {return mHidden ? 0 : mRadiusScale * kRadius;}
  float getHeadLength() const {return mHidden ? 0 : mRadiusScale * kHeadLength;}
  float getHeadRadius() const {return mHidden ? 0 : mRadiusScale * kHeadRadius;}
