  ...
  : QGraphicsItem(parent), mItemClicked(false), mCorner(corner), mScaleIncrementBy(1.10), mScaleDecrementBy(1/1.10)
{
  ...
  scale(1.0, -1.0);
  mActivePen = QPen(Qt::red, 3);
  mHoverPen = QPen(Qt::darkRed, 3);
...
void CornerItem::updateCornerItem(qreal x, qreal y, Qt::Corner corner)
{
  ...
  mRectangle = QRectF (startx, starty, width, height);
}
...
void CornerItem::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
  if (mItemClicked)
  {
    ...
    switch (mCorner)
    {
      case Qt::TopLeftCorner:
      {
        if ((mClickPos.x() < event->pos().x()) and (mClickPos.y() > event->pos().y()))
        {
          resizeFactorX = mScaleDecrementBy;
          resizeFactorY = mScaleDecrementBy;
        }
        else if ((mClickPos.x() > event->pos().x()) and (mClickPos.y() < event->pos().y()))
        {
          resizeFactorX = mScaleIncrementBy;
          resizeFactorY = mScaleIncrementBy;
        }
        break;
      }
      case Qt::TopRightCorner:
