  ...
  : QWidget(pParent)
{
  ...
  connect(pParent->mpPlotWindowContainer, SIGNAL(subWindowActivated(QMdiSubWindow*)), this, SLOT(updatePlotVariablesTree(QMdiSubWindow*)));
  connect(this, SIGNAL(removePlotFile(PlotTreeItem*)), pParent->mpPlotWindowContainer, SLOT(updatePlotWindows(PlotTreeItem*)));
}
...
QList<QString> PlotWidget::readPlotVariables(QString fileName)
{
  ...
  if (!simulationResultFile.open(QIODevice::ReadOnly)) {
    QMessageBox::critical(this, QString(Helper::applicationName).append(" - ").append(Helper::error),
                          tr("Failed to read variables. Could not open file ") + simulationResultFile.fileName());
    throw 1;
  }
  ...
  // read the xml
  QDomNodeList nodes = xmlDocument.elementsByTagName("ScalarVariable");
...
void PlotWidget::addPlotVariableToTree(QString fileName, QString parentStructure, QString childName, QString fullStructure, bool derivative)
{
  ...
  QString toolTip = QString(tr("File: ")).append(fileName).append("\n").append(tr("Variable: ").append(nameStructure));
