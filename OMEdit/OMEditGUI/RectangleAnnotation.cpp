...
void RectangleAnnotation::drawRectangleAnnotaion(QPainter *painter)
{
  ...
  switch (mFillPattern)
  {
    ...
    {
      QLinearGradient gradient(getBoundingRect().center().x(), getBoundingRect().center().y(), getBoundingRect().center().x(),
                               getBoundingRect().y());
    ...
    {
      QLinearGradient gradient(getBoundingRect().center().x(), getBoundingRect().center().y(), getBoundingRect().x(),
                               getBoundingRect().center().y());
      gradient.setColorAt(0.0, mFillColor);
      gradient.setColorAt(1.0, mLineColor);
      gradient.setSpread(QGradient::ReflectSpread);
      painter->setBrush(gradient);
    ...
    {
      QRadialGradient gradient(getBoundingRect().center().x(), getBoundingRect().center().y(), getBoundingRect().width());
      gradient.setColorAt(0.0, mFillColor);
      gradient.setColorAt(1.0, mLineColor);
      gradient.setSpread(QGradient::ReflectSpread);
      painter->setBrush(gradient);
...
void RectangleAnnotation::parseShapeAnnotation(QString shape, OMCProxy *omc)
{
  ...
  for (border_it = mBorderPatternsMap.begin(); border_it != mBorderPatternsMap.end(); ++border_it)
  {
    if (border_it.key() == borderPattern)
    {
      mBorderPattern = border_it.value();
      break;
    }
  }
  ...
  index = index + 1;
  mCornerRadius = static_cast<QString>(list.at(index)).toFloat();
}
