  ...
  : QTreeWidget(parent)
{
  ...
  setHeaderLabel(Helper::modelicaFiles);
...
{
  mpRenameAction = new QAction(QIcon(":/Resources/icons/rename.png"), Helper::rename, this);
  ...
  mpDeleteAction = new QAction(QIcon(":/Resources/icons/delete.png"), Helper::Delete, this);
  mpDeleteAction->setStatusTip(tr("Delete the modelica model"));
  connect(mpDeleteAction, SIGNAL(triggered()), SLOT(deleteNodeTriggered()));

  mpCopyModelAction = new QAction(QIcon(":/Resources/icons/copy.png"), Helper::copy, this);
  mpCopyModelAction->setStatusTip(tr("Copy the modelica model"));
  connect(mpCopyModelAction, SIGNAL(triggered()), SLOT(copyModel()));

  mpPasteModelAction = new QAction(QIcon(":/Resources/icons/paste.png"), Helper::paste, this);
...
void ModelicaTree::addNode(QString name, int type, QString parentName, QString parentStructure, int insertIndex)
{
  ...
  mpParentLibraryWidget->mpParentMainWindow->mpStatusBar->showMessage(QString(Helper::loading).append(": ").append(parentStructure + name));
...
bool ModelicaTree::deleteNodeTriggered(ModelicaTreeNode *node, bool askQuestion, bool removeTab)
{
  ...
  {
    pMainWindow->mpMessageWidget->addGUIProblem(new ProblemItem("", false, 0, 0, 0, 0, GUIMessages::getMessage(GUIMessages::ERROR_OCCURRED)
                                                                .arg(pMainWindow->mpOMCProxy->getResult())
                                                                .append(tr("while deleting ") + treeNode->mName),
...
void ModelicaTree::loadingLibraryComponent(ModelicaTreeNode *treeNode, QString className)
{
  ...
  LibraryComponent *libComponent = new LibraryComponent(result, className, mpParentLibraryWidget->mpParentMainWindow->mpOMCProxy);
...
void LibraryTree::loadModelicaLibraryHierarchy(QString value, QString prefixStr)
{
  ...
  if (mpParentLibraryWidget->mpParentMainWindow->mpOMCProxy->isPackage(value))
  {
    ...
    {
      addClass(&tempPackageNodesList, &tempNonPackageNodesList, str, StringHandler::getSubStringFromDots(prefixStr), prefixStr, true);
...
{
  mpParentLibraryWidget->mpParentMainWindow->mpStatusBar->showMessage(QString(Helper::loading).append(": ").append(parentStructure + className));
  ...
  int classType = mpParentLibraryWidget->mpParentMainWindow->mpOMCProxy->getClassRestriction(parentStructure + className);
  newTreePost->mType = classType;
...
{
  LibraryTreeNode *pItem = dynamic_cast<LibraryTreeNode*>(mpParentLibraryWidget->mSelectedLibraryNode);
  FlatModelWidget *widget = new FlatModelWidget(mpParentLibraryWidget->mSelectedLibraryNode->text(0), pItem->mNameStructure,
                                                mpParentLibraryWidget->mpParentMainWindow);
  widget->show();
}
...
{
  LibraryTreeNode *pItem = dynamic_cast<LibraryTreeNode*>(mpParentLibraryWidget->mSelectedLibraryNode);
  CheckModelWidget *widget = new CheckModelWidget(mpParentLibraryWidget->mSelectedLibraryNode->text(0), pItem->mNameStructure,
                                                  mpParentLibraryWidget->mpParentMainWindow);
  widget->show();
}
  ...
  : QWidget(pParent)
{
  ...
  // get MSL recursive
  mMSLItemsList = mpParentMainWindow->mpOMCProxy->getClassNames("", "true");
  // create search controls
  mpSearchTextBox = new MSLSearchBox(this);
  connect(mpSearchTextBox, SIGNAL(returnPressed()), SLOT(searchMSL()));
  mpSearchButton = new QPushButton(Helper::search);
...
void SearchMSLWidget::searchMSL()
{
  ...
  {
    mpSearchedItemsTree->insertTopLevelItem(0, new QTreeWidgetItem(QStringList(tr("Sorry, no items found"))));
    return;
  }
  ...
  : QWidget(parent)
{
  ...
  mpLibraryTabs->addTab(mpLibraryTree, tr("Modelica Library"));
  mpLibraryTabs->addTab(mpModelicaTree, Helper::modelicaFiles);
  // set the layout
...
void LibraryWidget::addModelFiles(QString fileName, QString parentFileName, QString parentStructure, int insertIndex)
{
  ...
  {
    addModelicaNode(fileName, mpParentMainWindow->mpOMCProxy->getClassRestriction(fileName), parentFileName, parentStructure, insertIndex);
    parentStructure = fileName;
  }
  else
  {
    addModelicaNode(fileName, mpParentMainWindow->mpOMCProxy->getClassRestriction(parentStructure), parentFileName,
                    StringHandler::removeLastWordAfterDot(parentStructure).append("."), insertIndex);
  }

  QStringList modelsList = mpParentMainWindow->mpOMCProxy->getClassNames(parentStructure);
  foreach (QString model, modelsList)
  {
    addModelFiles(model, fileName, parentStructure + "." + model);
  }
}
...
LibraryLoader::LibraryLoader(LibraryTreeNode *treeNode, QString className, LibraryTree *pParent)
{
  ...
  connect(this, SIGNAL(loadLibraryComponent(LibraryTreeNode*,QString)), mpLibraryTree, SLOT(loadingLibraryComponent(LibraryTreeNode*,QString)));
}
...
LibraryLoader::LibraryLoader(ModelicaTreeNode *treeNode, QString className, ModelicaTree *pParent)
{
  ...
  connect(this, SIGNAL(loadLibraryComponent(ModelicaTreeNode*,QString)), mpModelicaTree, SLOT(loadingLibraryComponent(ModelicaTreeNode*,QString)));
}
...
void ModelBrowserTree::editModelBrowser()
{
  ...
  if (pCurrentTab)
  {
    ...
    addBrowserNode(pCurrentTab->mModelName, type, pCurrentTab->mModelNameStructure, "", "");
