...
bool ModelicaEditor::validateText()
{
  if (document()->isModified())
  {
    ...
    if (!emit focusOut())
    {
      ...
      msgBox->setWindowTitle(QString(Helper::applicationName).append(" - ").append(Helper::error));
...
{
  QTextDocument *textDocument = qobject_cast<QTextDocument*>(parent());
...
{
  setWindowTitle(QString(Helper::applicationName).append(" - ").append(tr("Go to Line")));
...
{
  mpLineNumberLabel->setText(QString(tr("Enter line number (1 to ")).append(QString::number(mpModelicaEditor->blockCount())).append("):"));
