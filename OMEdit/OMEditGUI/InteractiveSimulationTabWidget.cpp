  ...
  : QWidget(pParent)
{
  ...
  mpHeadingLabel = new QLabel(Helper::parameters);
  ...
  : mMessageCounter(0)
{
  ...
  mpOMILogger->setWindowTitle(QString(Helper::applicationName).append(" - ").append(tr("OMI Messages Log")));
  ...
  connect(this, SIGNAL(interactiveSimulationStarted(QString)), mpInteractiveSimulationTab->getParentTabWidget()->getParentMainWindow(),
          SLOT(switchToInteractiveSimulationView()));

  connect(this, SIGNAL(interactiveSimulationFinished()), mpInteractiveSimulationTab->getParentTabWidget()->getParentMainWindow(),
  ...
  switch (socketError)
  {
    ...
    default:
      pMessageWidget->addGUIProblem(new ProblemItem("", false, 0, 0, 0, 0,
                                                    tr("The following error occurred while communicating with OMI control server:").append(" %1.")
                                                    .arg(mpControlClientSocket->errorString()), Helper::scriptingKind, Helper::errorLevel, 0,
  ...
  : QWidget(pParent)
{
  ...
  mpInitializeButton->setObjectName("InteractiveSimulationButtons");
  QString initializeButtonMsg = tr("Initializes the interactive simulation.");
  mpInitializeButton->setToolTip(initializeButtonMsg);
  mpInitializeButton->setStatusTip(initializeButtonMsg);
  ...
  mpStartButton->setObjectName("InteractiveSimulationButtons");
  QString startButtonMsg = tr("Starts or continues the interactive simulation.");
  mpStartButton->setToolTip(startButtonMsg);
  mpStartButton->setStatusTip(startButtonMsg);
  ...
  mpPauseButton->setObjectName("InteractiveSimulationButtons");
  QString pauseButtonMsg = tr("Pauses the running interactive simulation.");
  mpPauseButton->setToolTip(pauseButtonMsg);
  mpPauseButton->setStatusTip(pauseButtonMsg);
  ...
  mpStopButton->setObjectName("InteractiveSimulationButtons");
  QString stopButtonMsg = tr("Stops the running interactive simulation and resets all values to the beginning.");
  mpStopButton->setToolTip(stopButtonMsg);
  mpStopButton->setStatusTip(stopButtonMsg);
  ...
  mpShutdownButton->setObjectName("InteractiveSimulationButtons");
  QString shutDownButtonMsg = tr("Shut down the running interactive simulation.");
  mpShutdownButton->setToolTip(shutDownButtonMsg);
  mpShutdownButton->setStatusTip(shutDownButtonMsg);
  ...
  mpShowOMILogButton->setObjectName("InteractiveSimulationButtons");
  QString showLogButtonMsg = tr("Shows the OMI Log Message Window.");
  mpShowOMILogButton->setToolTip(showLogButtonMsg);
  mpShowOMILogButton->setStatusTip(showLogButtonMsg);
  ...
  if (!components.isEmpty())
  {
    foreach (ComponentsProperties *pComponent, components)
    {
      if (pOMCProxy->getComponents(pComponent->getClassName()).isEmpty())
      {
        ...
        {
          QString parameterValue = pOMCProxy->getComponentModifierValue(modelName, QString(namePrefixStr).append(pComponent->getName()));
      ...
      {
        readComponentsRecursive(modelName, pComponent->getClassName(), QString(namePrefixStr).append(pComponent->getName()).append("."));
      }
    }
  }
}
...
void InteractiveSimulationTab::initializeInteractivePlotting()
{
  ...
  {
    mpOMIProxy->sendSequenceMessage(QString("setfilter#%1#").append(selectedVariablesList.join(":")).append("##end"));
  }
...
void InteractiveSimulationTab::startInteractivePlotting()
{
  ...
  mpOMIProxy->sendSequenceMessage(QString("start#%1#end"));
...
{
  mpOMIProxy->sendSequenceMessage(QString("pause#%1#end"));
...
{
  mpOMIProxy->sendSequenceMessage(QString("stop#%1#end"));
...
{
  mpOMIProxy->sendSequenceMessage(QString("shutdown#%1#end"));
