...
model SingularPressureLossVALI "Singular pressure loss"
  parameter Modelica.Units.SI.MassFlowRate Qnom=535 "Nominal mass flow rate";
  parameter Modelica.Units.SI.Pressure deltaPnom=0.5e5 "Nominal pressure loss";
  ...
  parameter Boolean flow_reversal=false "true: with flow reversal - false: without flow reversal";
...
public
  Modelica.Units.SI.Pressure deltaP(start=deltaPnom) "Singular pressure loss";
  Modelica.Units.SI.MassFlowRate Q(start=1) "Mass flow rate";
  ...
  ThermoSysPro.WaterSteam.Connectors.FluidInlet C1(
    h(start=1e5),
    h_vol(start=1e5),
    Q(start=1)) annotation (Placement(transformation(extent={{-110,-10},{-90,10}}, rotation=0)));
  ThermoSysPro.WaterSteam.Connectors.FluidOutlet C2(
    h(start=1e5),
    h_vol(start=1e5),
    Q(start=1)) annotation (Placement(transformation(extent={{90,-10},{110,10}}, rotation=0)));
equation
  ...
  deltaP = CoeffDeltaP*deltaPnom*Q/Qnom*abs(Q/Qnom);
