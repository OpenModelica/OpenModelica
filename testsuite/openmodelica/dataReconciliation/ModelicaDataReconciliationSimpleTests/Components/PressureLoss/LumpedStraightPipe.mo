...
protected
  constant Modelica.Units.SI.Acceleration g=Modelica.Constants.g_n "Gravity constant";
  constant Real pi=Modelica.Constants.pi "pi";
  parameter Real eps=1.e-3 "Small number for pressure loss equation";
  parameter Modelica.Units.SI.MassFlowRate Qeps=1.e-3 "Small mass flow for continuous flow reversal";
  parameter Modelica.Units.SI.Area A=ntubes*pi*D^2/4 "Pipes cross-sectional area (circular duct is assumed)";
  parameter Modelica.Units.SI.Area Pw=ntubes*pi*D "Pipes wetted perimeter (circular duct is assumed)";
initial equation
  if inertia then
    der(Q) = 0;
  end if;
...
equation
  ...
  mu = ThermoSysPro.Properties.Fluid.DynamicViscosity_Ph(
    Pm,
    h,
    fluid,
    mode,
