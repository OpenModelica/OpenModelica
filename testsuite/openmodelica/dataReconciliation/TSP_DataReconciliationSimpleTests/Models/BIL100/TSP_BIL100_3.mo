...
model TSP_BIL100_3
  ...
  Components.BoundaryConditions.SinkQ sinkQ
    annotation (Placement(visible=true,transformation(origin={0,126},extent={{-10,-10},{10,10}},rotation=90)));
  ...
  Components.Junctions.Splitter22 SEP_ARE(
    P(
      uncertain=Uncertainty.refine),
    Cs1(
      Q(
  ...
  Components.Junctions.Mixer2 MEL_GCT
  ...
  ThermoSysPro.WaterSteam.Sensors.SensorQ sensorGCT(
    Q(
      uncertain=Uncertainty.refine))
    annotation (Placement(visible=true,transformation(origin={8,90},extent={{10,-10},{-10,10}},rotation=-90)));
equation
  ...
  connect(sinkQ.C,sensorGCT.C2)
    annotation (Line(points={{0,116},{0,100}},color={0,0,255}));
  connect(sensorGCT.C1,MEL_GCT.Cs)
    annotation (Line(points={{0,80},{0,68}},color={0,0,255}));
  annotation (
    Icon(
      coordinateSystem(
    ...
    __OpenModelica_simulationFlags(
      ...
      s="dassl",
      sx="modelica://EDF_EDF_NewDataReconciliationSimpleTests/resources/NewDataReconciliationSimpleTests.TSP_BIL100_3_Inputs.csv"));
end TSP_BIL100_3;
