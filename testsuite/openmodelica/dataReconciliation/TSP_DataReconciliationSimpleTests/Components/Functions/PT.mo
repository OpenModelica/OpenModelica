within TSP_DataReconciliationSimpleTests.Components.Functions;
function PT
  ...
  input Integer mode=0
    "IF97 region - 0:automatic computation";
  input Integer fluid=1
    "Fluid number - 1: IF97 - 7: SimpleWater";
...
algorithm
  if(fluid == 1) then
    ...
    assert(
      false,
      "Prop.PT : incorrect fluid number");
  end if;
  annotation (
    smoothOrder=2,
    Icon(
