...
model StaticDrum
  "Staic drum"
  parameter Real x=1
    "Vapor separation efficiency at the outlet";
...
public
  ThermoSysPro.WaterSteam.Connectors.FluidInlet Ce_eva
    annotation (Placement(transformation(extent={{-104,-44},{-84,-24}},rotation=0)));
  ThermoSysPro.WaterSteam.Connectors.FluidInlet Ce_eco
    annotation (Placement(transformation(extent={{-50,-104},{-30,-84}},rotation=0)));
  ...
  ThermoSysPro.WaterSteam.Connectors.FluidOutlet Cs_purg
    annotation (Placement(transformation(extent={{84,-44},{104,-24}},rotation=0)));
  ...
  ThermoSysPro.WaterSteam.Connectors.FluidInlet Ce_sup
    annotation (Placement(transformation(extent={{-104,26},{-84,46}},rotation=0)));
  ThermoSysPro.Properties.WaterSteam.Common.PropThermoSat lsat
    annotation (Placement(transformation(extent={{-104,66},{-78,98}},rotation=0)));
...
equation
  ...
  if(fluid == 3) then
    ...
    (lsat,vsat)=ThermoSysPro.Properties.WaterSteam.IF97.Water_sat_P(
      1.e5);
  else
    (lsat,vsat)=ThermoSysPro.Properties.WaterSteam.IF97.Water_sat_P(
      P);
    hl=lsat.h;
    hv=vsat.h;
    T=ThermoSysPro.Properties.Fluid.T_sat(
      P,
      fluid);
  end if;
  Cth.T=T;
