...
model LumpedStraightPipe
  "Lumped straight pipe (circular duct)"
  parameter Modelica.Units.SI.Length L=10.
    "Pipe length";
  parameter Modelica.Units.SI.Diameter D=0.2
    "Pipe internal diameter";
  parameter Integer ntubes=1
  ...
  parameter Modelica.Units.SI.Position z1=0
    "Inlet altitude";
  parameter Modelica.Units.SI.Position z2=0
    "Outlet altitude";
  ...
  ThermoSysPro.Units.SI.PressureDifference deltaPf
  ...
  Modelica.Units.SI.ReynoldsNumber Re
    "Reynolds number";
  Modelica.Units.SI.ReynoldsNumber Relim
    "Limit Reynolds number";
  Real lam
...
equation
  ...
  /* Pressure loss */
  if inertia then
    deltaP=deltaPf+rho*g*(z2-z1)+L/A*der(
      Q);
  else
    deltaP=deltaPf+rho*g*(z2-z1);
  end if;
  deltaPf=khi*ThermoSysPro.Functions.ThermoSquare(
    Q,
    eps)/(2*A^2*rho);
  ...
  if lambda_fixed then
    ...
    if(rugosrel > 0.00005) then
      lam=1/(2*Modelica.Math.log10(
        3.7/rugosrel))^2;
    else
      lam=
        if noEvent(
          Re > 0) then
          1/(1.8*Modelica.Math.log10(
            Re)-1.64)^2
        else
          0;
    end if;
