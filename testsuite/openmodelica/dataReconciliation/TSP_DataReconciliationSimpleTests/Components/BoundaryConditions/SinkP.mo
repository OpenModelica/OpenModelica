...
model SinkP
  "Water/steam sink with fixed pressure"
  ...
  ThermoSysPro.InstrumentationAndControl.Connectors.InputReal IPressure
    annotation (Placement(transformation(origin={50,0},extent={{-10,-10},{10,10}},rotation=180)));
...
equation
  ...
  elseif(option_temperature == 2) then
    ...
    assert(
      false,
      "SinkPressureWaterSteam: incorrect option");
  end if;
  annotation (
    Diagram(
      coordinateSystem(
