...
model WatchDogSystemSimulation_1
  ...
  Boolean test_failed;
  record WatchDogSystemSimulation_1_StateMachine_test_scenario
  annotation(Diagram(coordinateSystem(extent={{-148.5,-105.0},{148.5,105.0}}, preserveAspectRatio=true, initialScale=0.1, grid={10,10})), Icon(coordinateSystem(extent={{-100.0,-100.0},{100.0,100.0}}, preserveAspectRatio=true, initialScale=0.1, grid={10,10}), graphics={Rectangle(visible=true, origin={-6.722,3.6841}, lineColor={0,85,127}, fillColor={85,170,255}, fillPattern=FillPattern.Solid, lineThickness=10, extent={{-75.0,-50.0},{75.0,50.0}}, radius=40),Rectangle(visible=true, origin={70.0,2.6556}, lineColor={85,170,255}, fillColor={0,85,127}, fillPattern=FillPattern.Solid, lineThickness=4, extent={{-26.8908,-25.0},{26.8908,25.0}})}));
    ...
    record test_scenario_Region_0
      ...
      SimpleState init;
      SimpleState turn_on;
      SimpleState simulate_alarm;
      SimpleState turn_off;
  ...
  end WatchDogSystemSimulation_1_StateMachine_test_scenario;



  // STATE MACHNE instantiation
    WatchDogSystemSimulation_1_StateMachine_test_scenario test_scenario;

  algorithm
    when req_002.violated then
      test_failed := true;
    end when;
  algorithm
  algorithm
  ...
  when {test_scenario.Region_0.turn_on.active, test_scenario.Region_0.turn_on.selfTransitionActivated} then
      test_scenario.Region_0.turn_on.timeAtActivation := time;
      test_scenario.Region_0.turn_on.selfTransitionActivated := false;
    end when;
  when {test_scenario.Region_0.simulate_alarm.active, test_scenario.Region_0.simulate_alarm.selfTransitionActivated} then
      test_scenario.Region_0.simulate_alarm.timeAtActivation := time;
      test_scenario.Region_0.simulate_alarm.selfTransitionActivated := false;
    end when;
  when {test_scenario.Region_0.turn_off.active, test_scenario.Region_0.turn_off.selfTransitionActivated} then
      test_scenario.Region_0.turn_off.timeAtActivation := time;
      test_scenario.Region_0.turn_off.selfTransitionActivated := false;
    end when;
    ...
    if not test_scenario.Region_0.turn_on.active then
      test_scenario.Region_0.turn_on.stime := 0;
    end if;
    ...
    if not test_scenario.Region_0.simulate_alarm.active then
      test_scenario.Region_0.simulate_alarm.stime := 0;
    end if;
    ...
    if not test_scenario.Region_0.turn_off.active then
      test_scenario.Region_0.turn_off.stime := 0;
    end if;
...
end WatchDogSystemSimulation_1;
