package Synchronous
  ...
  package SubClocks
    ...
    model subSuperSample2
      Real x;
      Clock u = Clock(x > 0);
      Clock s1 = subSample(u, 4);
      Clock s2 = superSample(s1, 2); // s2 = subSample(u, 2)
      Integer y1(start=0), y2(start=0);
    equation
      x = sin(Modelica.Constants.pi*10*time);
      when s1 then
        y1 = previous(y1) + 1;
      end when;
      when s2 then
    ...
    model shiftSample1
      Clock u  = Clock(3, 10);            // ticks: 0, 3/10, 6/10, 9/10...
      Clock s1 = shiftSample(u, 1, 3);    // ticks: 1/10, 4/10, 7/10, 1 ...
      Clock s2 = shiftSample(s1, 2);      // ticks: 4/10, 7/10, 1 ...
      Integer y1(start=0), y2(start=0);
    equation
      when s1 then
        y1 = previous(y1) + 1;
    ...
    model backSample2
      ...
      Clock b2 = backSample(s1, 2);               // ticks: 1.0, 2.0, ...
      Integer y1(start=0);
    equation
      when b1 then
        y1 = previous(y1) + 1;
    ...
    model noClock
      Clock clk1 = Clock(0.1);          // ticks: 0, 0.1, 0.2, ...
      Clock clk2 = subSample(clk1, 2);  // ticks: 0, 0.2, 0.4, ...
