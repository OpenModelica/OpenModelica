...
void initSample(double start,double stop) {
    ...
    StartSample.events = 0;
    StartSample.zc_index = 0;
    StartSample.activated = 0;
    //max_events = filter_all_lesser(Samples,&StartSample,max_events,sizeof(sample_time),compSample);
    ...
    if (sim_verbose) {
        ...
        for (i=0; i<nuniq; i++) printf("%f\t HelpVar[%d]\n", (Samples[i]).events,(Samples[i]).zc_index);
    }
...
void debugPrintHelpVars(){
    if (sim_verbose) cout << " *'*'*'*  HELP VARS  *'*'*'*" << endl;
    for (int i = 0; i < globalData->nHelpVars; i++) {
        if (sim_verbose) cout << "HelpVar["<<i<<"] pre: " << pre(globalData->helpVars[i]) << ",  HelpVar["<<i<<"] : " << globalData->helpVars[i] << endl;
    }
}
...
int CheckForNewEvent(int* sampleactived) {
    ...
    if (!EventQueue.empty()){
        ...
        if (sim_verbose) { cout << "Event Handling at EventTime: " <<  globalData->timeValue << " done!" << endl;}
...
int EventHandle(int flag){
    ...
    // sample event handling
        ...
        if (sim_verbose) { cout << "Event Handling for Sample : " <<  globalData->timeValue << endl;}
        ...
        if (sim_verbose) { cout << "Event Handling for Sample : " <<  globalData->timeValue << " DONE!" << endl;}
    }
    return 0;
}
